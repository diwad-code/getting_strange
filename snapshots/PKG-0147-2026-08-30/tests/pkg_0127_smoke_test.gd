...
func _test_bidirectional_graph_and_continuity() -> void:
	...
	# 4. Save and Restore (station_26 lives outside every P7 migration range,
	# so the checkpoint itself must survive reload untouched).
	...
	_expect(gsm.last_checkpoint_station == &"station_26", "Checkpoint station must be station_26")
