...
func _test_campaign_frame_budget() -> void:
	...
	for station_id in ALL_CAMPAIGN_SCENE_IDS:
		...
		# Cameras are resolved by type, not by node name: the delivered campaign uses
		# "Camera" in stations 01-32 and "Camera2D" in stations 33-43. The contract
		# that matters is that exactly one CinematicCamera exists and keeps snapping.
