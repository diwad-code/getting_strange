...
func _audit_station(station_id: StringName) -> void:
	...
	for prop in props:
		...
		prop.trigger_interaction()
	await process_frame
	await _exercise_pkg_0148_station(station_id, station)
...
func _exercise_pkg_0148_station(station_id: StringName, station: Node) -> void:
	var state := root.get_node_or_null("GameStateManager")
	match station_id:
		&"station_26":
			if state:
				state.record_decision(&"p7.three_place_proofs.trace", "world_recognized_and_search_committed")
			station.call("observe_interrupted_ucp_log")
			station.call("synchronize_sample_clock")
			station.call("synchronize_ucp_command_clock")
			station.call("synchronize_local_generator_clock")
			station.call("reconstruct_ucp_intervention")
		&"station_27":
			if state:
				state.record_decision(&"p7.interrupted_trial_and_small_cost.ucp_intervention_reconstructed", true)
			station.call("calibrate_pulse_reference")
			station.call("send_first_identical_pulse")
			station.call("send_second_identical_pulse")
			station.call("send_deliberate_error_pulse")
			station.call("compare_response_correction")
		&"station_28":
			if state:
				state.record_decision(&"p7.interrupted_trial_and_small_cost.ucp_intervention_reconstructed", true)
				state.record_decision(&"p7.interrupted_trial_and_small_cost.local_lena_signal_confirmed", true)
			station.call("observe_transfer_constraint")
			station.call("inspect_home_sample_trace")
			station.call("inspect_local_lena_signal_trace")
			station.call("disclose_transfer_price")
			station.call("perform_shared_drift_yield")
		&"station_29":
			if state:
				state.record_decision(&"p7.interrupted_trial_and_small_cost.trace", "shared_drift_yield")
			station.call("inspect_ucp_jakub_contrast_proposal")
			station.call("observe_jakub_ordinary_life_scope")
			station.call("disclose_jakub_signal_risk")
			station.call("disable_jakub_transmitter")
			station.call("record_jakub_consent", &"refused")
		&"station_30":
			if state:
				state.record_decision(&"p7.interrupted_trial_and_small_cost.trace", "shared_drift_yield")
				state.record_decision(&"jakub_consent_state", "refused")
			station.call("build_force_home_forecast")
			station.call("build_close_equal_recover_local_forecast")
			station.call("build_mutual_passage_forecast")
