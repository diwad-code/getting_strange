# Prompt dla następnej sesji: PKG-0069
...
- Baseline po PKG-0068: Wszystkie 43 przestrzenie fabularne i epilog (Sceny 01..43) są w 100% zaimplementowane w silniku Godot 4.7.
- Oficjalny portal webowy w `web/` posiada:
  - symulator fizyki Canvas 2D (7 komór: 01..07, w tym Sala Modeli i Podstruktura -40m, profile A/B/C, dotyk, fullscreen, real-time HUD);
  - galerię 43 kadrów z wyszukiwarką na żywo oraz interaktywnym planem sektorów Równi i schematem Linii 4;
  - aparaturę audio z master volume, mute, analizatorem widma 512 FFT, generatorami tła, syntezatorem polifonicznym `PolyphonicRetroSynth` (4 motywy, 16 diod LED, tempo, filtr, arp), `CustomSignalDesigner` z eksportem 16-bit WAV PCM oraz 32-padowym dotykowym soundboardem proceduralnym;
  ...
  - odtajnione akta archiwalne IKP/UCP (6 dokumentów: doc1..doc6 z filtrami kategorii) oraz oficjalny Generator Certyfikatów Weryfikacji IKP (Canvas 2D z eksportem PNG i podpisem SHA-256);
...
- Snapshot produkcyjny: `snapshots/PKG-0068-2026-08-22`.

## KRYTERIA AKCEPTACJI

1. Wykonać świeżą weryfikację za pomocą `pwsh -NoProfile -File .\tools\verify.ps1`.
2. Zrealizować zadania przewidziane dla PKG-0069.
3. Zaktualizować dokumentację (`docs/CURRENT_STATE.md`, `docs/SESSION_LOG.md`, `docs/NEXT_SESSION_PROMPT.md`).
4. Zamrozić pakiet poleceniem `pwsh -NoProfile -File .\tools\snapshot.ps1 -Package PKG-0069`.

## KONIEC PAKIETU JEST OBOWIAZKOWY

Pakiet PKG-0069 musi zostać zweryfikowany przez `tools/verify.ps1`, zaktualizowany w dokumentacji projektu oraz zamrożony za pomocą polecenia:
`pwsh -NoProfile -File .\tools\snapshot.ps1 -Package PKG-0069`.
