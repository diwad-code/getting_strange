...
## PRZESZKODA — dlaczego to tu jest: węzeł przekaźnikowy zmienia konfigurację,
## bo maszyna świadków rozdziela zasilanie poszczególnych zeznań.
## PRZESZKODA — czego wymaga od Leny: zakotwiczenia konkretnej konfiguracji,
## zanim korekta przestawi mapę przewodów.
## PRZESZKODA — koszt porażki: korekta odsyła Lenę do checkpointu i wygasza
## najostrzejszy przewód mapy świadectw.
...
@onready var witness_relay: AnchorableObject = get_node_or_null("Geometry/WitnessRelayBank") as AnchorableObject
...
var is_witness_relay_anchored: bool = false
var witness_relay_correction_count: int = 0
var witness_relay_detail_faded: bool = false
var last_witness_relay_correction_target: AnchorableObject.RealityState = AnchorableObject.RealityState.STATE_A
...
func _physics_process(_delta: float) -> void:
	if witness_relay and player:
		witness_relay.update_player_distance(player.global_position)


func _setup_witness_relay() -> void:
	if witness_relay == null:
		return
	witness_relay.anchor_state_changed.connect(_on_witness_relay_anchor_changed)
	witness_relay.apply_reality_shift(AnchorableObject.RealityState.STATE_A, false)
	var state := get_node_or_null("/root/GameStateManager")
	if state and state.has_method("has_recorded_decision"):
		witness_relay_detail_faded = state.has_recorded_decision(&"station_30_witness_relay_corrected")
	elif state:
		var decisions: Variant = state.get("decisions")
		if decisions is Dictionary:
			witness_relay_detail_faded = (decisions as Dictionary).has(&"station_30_witness_relay_corrected")


func _on_witness_relay_anchor_changed(anchored: bool) -> void:
	is_witness_relay_anchored = anchored
	if anchored:
		var state := get_node_or_null("/root/GameStateManager")
		if state and state.has_method("record_decision"):
			state.record_decision(&"station_30_anchored_witness_relay", true)
	queue_redraw()


func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed(&"trigger_correction") and not event.is_echo():
		run_witness_relay_correction_pass()
	...
	if event.is_action_pressed("interact") or event.is_action_pressed("ui_accept"):
		...
		if dialogue_active:
			...
			get_viewport().set_input_as_handled()
			return
	if event.is_action_pressed(&"interact") and witness_relay and witness_relay.is_player_in_range:
		witness_relay.toggle_anchor()
		get_viewport().set_input_as_handled()
...
func run_witness_relay_correction_pass() -> void:
	if witness_relay == null:
		return
	last_witness_relay_correction_target = (
		AnchorableObject.RealityState.STATE_B
		if witness_relay.current_reality == AnchorableObject.RealityState.STATE_A
		else AnchorableObject.RealityState.STATE_A
	)
	witness_relay.apply_reality_shift(last_witness_relay_correction_target, false)
	if not witness_relay.is_anchored:
		_apply_witness_relay_correction()
	queue_redraw()


func _apply_witness_relay_correction() -> void:
	...
	var relay_prop := props.get_node_or_null("WitnessRelayBank") as MemoryResonancePoint if props else null
	if relay_prop:
		relay_prop.prop_subtitle = "Maszyna świadków — przewód mapy wyblakł po korekcie"
	if witness_relay:
		witness_relay.set_anchored(false)
	if player:
...
func _draw_state_layer() -> void:
	...
	var relay_color := VectorStageStyle.ANCHOR_CYAN if is_witness_relay_anchored else VectorStageStyle.HUMAN_AMBER
	if witness_relay_detail_faded:
		relay_color = VectorStageStyle.CORRECTION_OXIDE
	draw_line(Vector2(112.0, 78.0), Vector2(516.0, 78.0), relay_color, 2.0)
	for bus_index in range(4):
		var bus_x := 140.0 + float(bus_index) * 120.0
		draw_line(Vector2(bus_x, 82.0), Vector2(bus_x, 202.0), relay_color, 1.0)
	if is_breaker_thrown:
		draw_line(Vector2(262.0, 220.0), Vector2(330.0, 190.0), VectorStageStyle.ANCHOR_CYAN, 3.0)
	if is_exit_unlocked:
		draw_line(Vector2(548.0, 224.0), Vector2(606.0, 224.0), VectorStageStyle.HUMAN_AMBER, 2.0)


func _draw_background_environment_legacy() -> void:
