...
func _check_helper_inventory() -> void:
	...
	# PKG-0200 (D-218): slice2 extended the helper from 130 pilot renderers to
	# the full 203 (130 pilot 67..196 + 73 slice2 0..66,197..202). Pin the full
	# total here; the 130 pilot names are still pinned one-by-one below, so this
	# control is equivalent or stronger, never a lowered threshold.
	_expect(_count(helper, "static func draw_") == 203, "helper must own exactly 203 static renderers (130 pilot + 73 slice2)")
...
func _check_dispatch() -> void:
	...
	var helper := _read(HELPER_PATH)
	# PKG-0200 (D-218): facade now delegates the full 203 types (130 pilot +
	# 73 slice2); the pilot 130 are still pinned one-by-one below.
	_expect(_count(facade, "MrpLegacyRenderer.draw_") == 203, "facade must delegate 203 types to the helper (130 pilot + 73 slice2)")
	for pname in PILOT_NAMES:
		var short := pname.to_lower()
...
func _finish() -> void:
	if _failures.is_empty():
		# PKG-0200: helper now holds the full 203 (130 pilot pinned here + 73 slice2).
		print("PKG-0199 PILOT PASS: facade intact, 203 stateless renderers (130 pilot pinned), dispatch 1:1, runtime bridge GREEN.")
		quit(0)
	else:
