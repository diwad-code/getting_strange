...
- Ostatni zamkniety pakiet: `PKG-0015`, 2026-08-20
...
- Zamrozenia: `snapshots/PKG-NNNN-DATA/` przez `tools/snapshot.ps1` (D-017);
  ostatnie: `snapshots/PKG-0014-2026-08-20`
...
- Zaimplementowano `MovableAnchorableProp` (`scripts/interactables/movable_anchorable_prop.gd`) — fizycznie przemieszczana skrzynia laboratoryjna rozszerzająca `CharacterBody2D`: grawitacja (640 px/s²), pchanie przez gracza (PUSH_CONTACT_DISTANCE = 36 px, push_speed_max = 64 px/s), selektywne zakotwiczenie zamrażające prędkość, opór przed falą korekty konsensusu, cząsteczki i dźwięk proceduralny. Używa `AnchorableObject.RealityState` — brak duplikatu enum.
- Scena `scenes/prototype/anchor_lab.tscn` zawiera pełny 3-komorowy kompleks o szerokości 1920 px (Komora 1: Nauka, Komora 2: Zastosowanie z podnośnikiem i antresolą, Komora 3: Komplikacja z bramą bezpieczeństwa i stacją pomiarową celu) oraz skrzynię laboratoryjną `Chamber2Crate` (`MovableAnchorableProp`, pozycja `800, 304`, 28×28 px) do puzzla z podnośnikiem.
...
- Rozszerzono `tests/smoke_test.gd` o automatyczną weryfikację kamery kinowej, granic komorowych, przejść, wstrząsów, audio proceduralnego i fizyki/logiki kotwiczenia.
- Wygenerowano zrzuty kontrolne `reports/movement_lab.png`, `reports/anchor_lab.png`, `reports/anchor_lab_ch2.png` oraz `reports/anchor_lab_ch3.png`.
...
Potwierdzony wynik finalnej weryfikacji PKG-0015:
...
- Automatyczne testy potwierdzaja kontrakty plikow, generowanie buforów audio, zachowanie kamery kinowej, grayboxu oraz logikę kotwiczenia, nie subiektywną satysfakcję (game feel) gracza.
- `DOCS PASS` sprawdza istnienie plikow i obecnosc naglowkow.
- Cofanie jest ograniczone do snapshotów (`tools/snapshot.ps1`).
- `AnchorLab` weryfikuje bazową mechanikę i opór obiektów (AnimatableBody2D) w 3 pełnych komorach ze sprzężeniem audiowizualnym i kinową kamerą.

## Nastepny pakiet

`PKG-0016: Rozszerzenie interakcji i integracja układu puzzle w Komorze 2`

Cel: Dopracowanie puzzla skrzynia+podnośnik w Komorze 2 (integracja Chamber2Crate z Chamber2Lift), weryfikacja przejścia od Komory 1 do 3, ewentualnie kolejna skrzynia lub mechanika wymagająca sekwencji zakotwiczenia. Możliwa rozbudowa wizualna skrzyni o etykiety laboratoryjne.
...
plik i `NEXT_SESSION_PROMPT.md`, a nastepnie przechodzi do realizacji PKG-0016.
