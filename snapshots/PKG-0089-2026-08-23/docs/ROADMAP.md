...
## P4. Produkcja i Szlif Gry w Silniku Godot 4.7 (Pure Godot Game Focus, D-089) + Priorytet Ukończenia Portalu Web (D-090)

Status: **W TOKU** (równoległe tory: szlif gry w Godot 4.7 oraz dokończenie Portalu Web — właściciel projektu zarządził w D-090, że strona jest celem nadrzędnym i wszystkie zadeklarowane przez nią funkcje muszą działać; PKG-0089 naprawił krytyczny błąd ładujący `app.js` i domknął czytnik 100 akt, projektant sygnałów oraz szybkie komendy CLI; bramka web objęła audyty martwych kontrolek/eksportów i runtime smoke test w Node)

Główne filary produkcji w Godot:

1. **Szlif Sterowania i Game Feel (PrototypePlayer)**:
   - Kalibracja reakcji poziomej, coyote time, jump buffering, przyspieszenia i opadania.
   - Wdrożenie klatek squash-and-stretch, pyłu przy skoku i lądowaniu (`CPUParticles2D`), dynamicznego pochylenia postaci przy zwrocie.
   - Kinowa kamera 2D z płynnym śledzeniem, dynamicznym deadzone i mikro-wstrząsami przy anomaliach.

2. **Oprawa Wizualna Poziomów i Oświetlenie 2D (Station 01..43)**:
   - Dressing wizualny architektury (modernistyczne płytki, kasetony sufitowe, rury, żeliwo, jarzeniówki 100 Hz).
   - Oświetlenie 2D (`PointLight2D`, `DirectionalLight2D`) z cieniami okluzyjnymi, pulsującymi neonami i wolumetryczną mgłą.
   - Efekty anomalii czasoprzestrzennych (przesunięcia cienia, restless grid, cynobrowe szczeliny).

3. **Interfejs Dialogowy i Narracyjny w Silniku (In-Engine Dialogue UI)**:
   - Stylowe ramki dialogowe w estetyce CRT / Teletype 1978.
   - Portrety postaci (Lena Wolska, Marta Kurek, Jakub Wolski, dr Helena Wierzbicka, Szymon Bera, Strażnik).
   - Efekt maszyny do pisania zsynchronizowany z proceduralnymi blipami głosu postaci.
   - Historia dialogu (backlog) i płynne przewijanie.

4. **Mechaniki Zagadek i Pętla Rozgrywki (Anchor / Yield / Memory Props)**:
   - Dopracowanie zagadek środowiskowych (przesuwanie skrzyń, mosty dwustanowe, windy, bramy).
   - Dźwiękowe i wizualne sprzężenie zwrotne przy badaniu 117 rekwizytów pamięci.
   - Wdrożenie Menu Głównego, ekranu pauzy, ustawień dźwięku/sterowania i automatycznego menedżera zapisu/punktów kontrolnych (Checkpoint System) łączącego przejście przez wszystkie 43 przestrzenie i 3 rodziny zakończeń.
...
- Statyczna strona dystrybucyjna jako launcher/wizytówka bez zbędnego narzutu.
