...
func _run() -> void:
	...
	if state:
		...
		_expect(state.has_reached_station(&"station_42a"), "completing station 18 must unlock finale")
		state.complete_station(&"station_19", false)
		_expect(state.has_reached_station(&"station_20"), "completing legacy station 19 must unlock station 20")
		state.complete_station(&"station_20", false)
		_expect(state.has_reached_station(&"station_21"), "completing legacy station 20 must unlock station 21")
		_expect(
