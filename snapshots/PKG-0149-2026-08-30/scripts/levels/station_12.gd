...
func _on_prop_resonance_triggered(id: String, prop_type: int, prop: MemoryResonancePoint) -> void:
	match id:
		...
		"answering_machine":
			listen_to_message()
		"phone_handset", "caller_id_list":
			verify_caller_identity()
		"note_pad":
			prepare_independent_questions()
...
func _is_resolved(id: String) -> bool:
	match id:
		"balcony_handle": return is_balcony_closed
		"answering_machine": return is_message_heard
		"phone_handset", "caller_id_list": return is_caller_verified
		"note_pad": return are_questions_prepared
	return false
...
func _draw() -> void:
	...
	var balcony_color := VectorStageStyle.LIGHT_PLANE if is_balcony_closed else VectorStageStyle.CORRECTION_OXIDE
	var voice_color := VectorStageStyle.HUMAN_AMBER if is_message_heard else VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.5)
	var caller_color := VectorStageStyle.ANCHOR_CYAN if is_caller_verified else VectorStageStyle.HUMAN_AMBER
	var note_color := VectorStageStyle.ANCHOR_CYAN if are_questions_prepared else VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.35)
	draw_line(Vector2(520.0, 168.0), Vector2(520.0, 296.0), balcony_color, 2.0)
	draw_rect(Rect2(Vector2(228.0, 240.0), Vector2(38.0, 18.0)), voice_color, false, 1.0)
	draw_rect(Rect2(Vector2(396.0, 228.0), Vector2(22.0, 12.0)), caller_color, false, 1.0)
	draw_rect(Rect2(Vector2(340.0, 250.0), Vector2(26.0, 12.0)), note_color, true)
	var exit_color := VectorStageStyle.ANCHOR_CYAN if is_exit_unlocked else VectorStageStyle.HUMAN_AMBER
	draw_line(Vector2(590.0, 180.0), Vector2(590.0, 296.0), exit_color, 2.0)
