...
## PKG-0203 — sciezka C: oczny ogld skal 85/115 w pelnej rozdzielczosci,
## domkniecie dla pozostalych adresow wycinka 2.
## Po PKG-0202 (08/11/14/43) zostaje 16 adresow z zestawu 0198
## (02/03/04/05/07/09/10/12/13/15/16/17/18/42a/42b/42c):
## prompt mowi „15", ale inwentarz frames.tsv 0198 liczy 19 adresow,
## z czego 0202 domknal ocznie 3 (11/14/43) — reszta to 16, nie 15.
## Uruchamiac normalnym sterownikiem Windows (headless wiesza sie na
## frame_post_draw; fakt z PKG-0197). Zapis: 96 PNG 640x360
## (16 adresow x 3 skale x pelny/bez tekstu) + frames.tsv
## pod reports/pkg_0203/visual. Zero zmian w scripts/scenes; tylko odczyt
...
const OUT_DIR := "res://reports/pkg_0203/visual"
...
func _run() -> void:
	...
	if state == null:
		push_error("CAPTURE 0203: no GameStateManager autoload")
		quit(1)
		return
	...
	for si in range(STATIONS.size()):
		...
		if packed == null:
			push_error("CAPTURE 0203: cannot load %s" % station_id)
			continue
		for sci in range(SCALE_LABELS.size()):
			...
			if full_image != null:
				var full_name := "%s__pkg0203_s%d_full.png" % [station_id, label]
				full_image.save_png(out_path.path_join(full_name))
				print("CAPTURE 0203 PASS: %s" % full_name)
				index.append("%s\t%d\tfull\t%s" % [station_id, label, full_name])
			else:
				push_error("CAPTURE 0203: no full image for %s s%d" % [station_id, label])
			var notext_image := await _capture_variant(packed, station_id, true)
			if notext_image != null:
				var notext_name := "%s__pkg0203_s%d_notext.png" % [station_id, label]
				notext_image.save_png(out_path.path_join(notext_name))
				print("CAPTURE 0203 PASS: %s" % notext_name)
				index.append("%s\t%d\tnotext\t%s" % [station_id, label, notext_name])
			else:
				push_error("CAPTURE 0203: no notext image for %s s%d" % [station_id, label])
