...
func _test_runtime_untouched() -> void:
	# Grounding for the interaction-budget dry-run FAIL: the current runtime
	# exceeds the three-interaction budget on the first address of the game.
	var station_01 := _read("res://scenes/levels/station_01.tscn")
	var count := station_01.count("resonance_id")
	_expect(count >= 4, "grounding check failed: station_01 resonance prop count unexpectedly low (%d)" % count)
