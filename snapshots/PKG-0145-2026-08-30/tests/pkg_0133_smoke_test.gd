...
func _test_station_01_door_and_one_shot() -> void:
	...
	if sensor:
		sensor.is_player_in_range = true
		sensor.trigger_interaction()
		_expect(station.sensor_checked, "first sensor reading recorded")
		_expect(not sensor.is_activated, "first sensor reading must not lock the prop")
		sensor.trigger_interaction()
		_expect(station.second_reading_done, "second sensor reading recorded")
		_expect(sensor.is_activated and sensor.is_one_shot, "second sensor reading must lock the prop")

	if bag:
		bag.is_player_in_range = true
		bag.trigger_interaction()
		_expect(station.gear_packed, "bag after second reading packs gear")
		_expect(station.is_procedure_completed, "packing completes the exit procedure")
