...
const DIALOGUE_LINES: Array[Dictionary] = [
	{"speaker": "LENA", "text": "Przepływ zamknięty. Próg cichy. Ona jest w środku."},
	{"speaker": "MARTA", "text": "Odzyskała swój dzień. Gdzie pójdziesz ty?"},
	{"speaker": "LENA", "text": "Przed siebie. Bez numeru w rejestrze."},
	{"speaker": "JAKUB", "text": "Obie jesteście prawdziwe. Każda w swoim miejscu."},
]
...
func _setup_guidance() -> void:
	...
	_register_beat(&"s42b_recovered_seen", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Ona jest u siebie. To jest skutek wyboru, nie nagroda.", "She is home. That is the consequence of choice, not a reward.", &"", "")
...
func _draw_local_lena_recovered() -> void:
	...
	# Wnętrze mieszkania 14: odzyskana miejscowa Lena (spokojna ciepła sylwetka w głębi).
	draw_rect(Rect2(318.0, 196.0, 28.0, 86.0), Color(VectorStageStyle.HUMAN_AMBER, 0.35))
	draw_circle(Vector2(332.0, 208.0), 6.0, VectorStageStyle.HUMAN_AMBER)
	draw_line(Vector2(332.0, 214.0), Vector2(332.0, 260.0), VectorStageStyle.HUMAN_AMBER, 2.0)
