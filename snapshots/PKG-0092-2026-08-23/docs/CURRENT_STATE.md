...
-   Nie ma repozytorium, galezi, commita ani historii. Nie uruchamiamy `git`. Ostatni zamkniety pakiet: `PKG-0092`, 2026-08-23
...
-   ostatnie: `snapshots/PKG-0092-2026-08-23`
...
Finalna weryfikacja PKG-0092 zakończyła się 2026-08-23 wynikiem PASS: kontrakt dokumentacji, statyczny/runtimeowy kontrakt istniejącego zamrożonego showcase'u, import Godot 4.7 oraz pełny smoke test Station 01..43. Dodatkowy kontrakt `tests/pkg_0091_smoke_test.gd` przechodzi dla nowych systemów.
...
`PKG-0093: Rozszerzenie Pętli Gry — trwały zapis, menu poziomów i integracja dialogów Station 06..43`

Cel: kontynuacja wyłącznie gry Godot (D-091) — zbudować używalną pętlę kampanii na fundamentach PKG-0092:
1. Dodać wersjonowany zapis `user://` i bezpieczne odtworzenie `GameStateManager`; podłączyć akcję `restart` do checkpointu.
2. Dostarczyć dostępne menu pauzy oraz ekran wyboru 43 poziomów, używające listy `GameStateManager` i przejść fade-to-black.
3. Zintegrować checkpointy, poszlaki i decyzje z istniejącymi interakcjami `Station01`..`Station43`; rozszerzyć CRT dialogue cues z otwarć na kluczowe sceny z `DIALOGUE_SCRIPT.md`.
4. Rozszerzyć rig atmosfery systematycznie poza Station 05, z budżetem emisji i renderami kontrolnymi dla każdego aktu.
5. Uruchomić pełną weryfikację, zaktualizować dokumentację i zamrozić `PKG-0093`.
...
plik i `NEXT_SESSION_PROMPT.md`, a następnie przechodzi do realizacji PKG-0093 w silniku Godot 4.7.
