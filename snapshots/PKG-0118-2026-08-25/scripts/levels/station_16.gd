...
func _draw_state_layer() -> void:
	...
	# The repaired cup is a small amber witness on the table, never a second backdrop.
	if is_cup_inspected:
	...
	# The ring disposition is one point of relation, not a full-screen moral colour.
	if is_ring_chosen:
		var ring_color := VectorStageStyle.HUMAN_AMBER
		if ring_disposition == "leave":
			ring_color = VectorStageStyle.LIGHT_PLANE
		elif ring_disposition == "sample":
			ring_color = VectorStageStyle.ANCHOR_CYAN
		draw_circle(Vector2(370.0, 238.0), 5.0, ring_color)
		draw_arc(Vector2(370.0, 238.0), 7.0, 0.0, TAU, 16, ring_color, 1.0)

	if is_balcony_unlocked:
		draw_line(Vector2(560.0, 270.0), Vector2(610.0, 270.0), VectorStageStyle.ANCHOR_CYAN, 2.0)
