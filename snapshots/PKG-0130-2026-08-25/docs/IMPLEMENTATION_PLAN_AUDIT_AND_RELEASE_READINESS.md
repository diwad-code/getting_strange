...
Ten dokument pozostaje audytem historycznego runtime PKG-0113–0115. Jego
trasa „R2 content lock → R3 build” nie jest już aktywnym planem. Kontrolowana
przebudowa zachowuje technicznie domknięte R0/R1 (shell, topologię, zapis,
ustawienia), ale ponownie tworzy treść 01–43, Lenę, prowadzenie i powierzchnię
obrazu przed nowym content lockiem 2.0. Obowiązuje
`docs/CREATIVE_REBUILD_PLAN.md` i `docs/ROADMAP.md`; buildy są zablokowane do
PKG-0122. Poniższa macierz służy wyłącznie jako dowód tego, co istniało przed
D-113.
...
R1 jest zamknięte technicznie. `SettingsOverlay` współdzieli shell i pauza,
`GameStateManager` utrzymuje osobny schema ustawień, trwałość Master/tempa/
skali/fullscreen/locale, bezpieczny fallback oraz kontrolowany remap pięciu
akcji z konfliktem i defaults. Semantyczny fokus działa w shellu, ustawieniach
i pauzie, a PL/EN obejmuje UI i komunikaty systemowe. Gate PKG-0115 oraz siedem
normal-driver capture'ów potwierdzają kontrakty i rozmiar 640x360; nie są
dowodem ergonomii, czytelności przez nową osobę ani odbioru. Dialog narracyjny
pozostaje poza mechaniczną lokalizacją do osobnego audytu.
...
### R1 — opcje, pełny pad, dostępność i lokalizacja (P1, technicznie zamknięte PKG-0115)
...
- przełączalne PL/EN dla shellu, ustawień, pauzy, CRT i komunikatów systemowych;
...
Następny pakiet powinien wykonać R2 jako jeden spójny mega-pakiet: content lock
obrazu, dialogu i dźwięku, z ręcznym audytem wskazanych kadrów, portretów,
historii dialogu oraz miksu. Nie powinien wracać do R0/R1, eksportować buildów
ani dodawać przeszkód; bez zamrożonej treści nie ma stabilnego materiału do
pakowania w R3.
