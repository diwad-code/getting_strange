...
Status: **KANON PRODUKCYJNY; SZKIELET RIGA Z PKG-0117 WYMAGA ITERACJI W PKG-0118**

## 1. Diagnoza obecnego runtime

`scripts/player/prototype_player.gd` pozostaje właścicielem fizyki, a
`scripts/player/lena_visual_rig.gd` oraz `lena_animation_state.gd` zostały
już od niego oddzielone. Zastany rig ma trzynaście nazwanych stanów, głowę,
tułów, kończyny, torbę i kontaktowy cień.

Implementacja nadal generuje większość ruchu przez sinus, przesunięcia faset i
kąty kończyn. Nie ma danych autorskich key poses, pełnego kontaktu stopy,
bezwładności torby ani opóźnienia kolejnych segmentów ciała. Świeży capture
pokazuje czytelną ludzką figurę, lecz nadal mały proceduralny manekin. To dobra
architektura `KEEP` i prezentacja `ADAPT`, nie postać produkcyjna.

Nie dopracowujemy jej przez dodawanie kolejnych przypadkowych kresek. PKG-0118
zachowuje separację od fizyki i zastępuje skrót proceduralny czytelnymi pozami
oraz timingiem.
