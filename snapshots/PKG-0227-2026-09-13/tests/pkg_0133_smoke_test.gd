...
func _test_station_01_door_and_opening_actions() -> void:
	...
	# PKG-0176: Station 01 otwiera sie w warstwie B zimnego otwarcia. Rejestrator
	# jest wtedy jedyna dzialajaca czynnoscia i NIE zostaje rozwiazany — ta sama
	# obejma sluzy pozniej do powtorki. Trzy ograniczone czynnosci otwarcia
	# zaczynaja obowiazywac dopiero po otwarciu rozwidlenia.
	if measurement and station.is_cold_open_active():
		_expect(measurement.trigger_interaction(), "MeasurementRig must start the PKG-0176 cold open measurement")
		for _frame in range(900):
			if not station.is_cold_open_active():
				break
			await physics_frame
		_expect(not station.is_cold_open_active(), "PKG-0176 cold open must resolve before the bounded opening actions")
		_expect(not measurement.is_resolved, "the cold open measurement must leave the rig available for the repeat")
