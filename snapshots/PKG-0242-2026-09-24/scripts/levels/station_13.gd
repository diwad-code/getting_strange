...
## Station 13 — P9, synteza przy wspolnym stole (S-13, PKG-0230).
## Zrodlo Marty, zrodlo UCP i jawne zestawienie: Lena kladzie wlasny nosnik
## obok wyciagu, slyszy trzeci glos Jakuba przez lacze i wypowiada wniosek
## (marta_source → institution_source → synthesize).
## Marta wrocila z wlasnym sprawdzeniem (zawias P1-2: druga kwestia cue).
## Warstwa dawcy P7 (szuflada, dokumenty) jest nieaktywna w trasie, ale
## callable dla testow — patrz werby nizej.

## PRZESZKODA — dlaczego to tu jest: Wniosek wymaga trzech niezaleznych zrodel i obu znacznikow na jednym stole; bez kompletu synteza sie nie domyka.
## PRZESZKODA — czego wymaga od Leny: wylozenia zrodel, wysluchania Marty i wypowiedzenia wniosku wlasnymi slowami.
## PRZESZKODA — koszt porażki: wniosek bez kompletu zostawia luke w GapLedger, nie blokade.

## PKG-0192 (D-205 §4) — jawna decyzja dla `DeskDrawer`: ZOSTAJE jako
## wezel w drzewie, nie jest bramka syntezy ani cialem na drodze
## (PKG-0234 A4). Synteza P9 zostaje na marta_source → institution_source
## → synthesize. Metody
...
func _setup_guidance() -> void:
	...
	# PKG-0233 (B4, S-13): jeden fizyczny korelat syntezy — czytnik terenowy
	# i wyciag UCP nosza te sama niemozliwa luke. Dwa urzadzenia, jeden brak;
	# dopiero to (nie sama pamiec) domyka wniosek. Bez nowego werbu i faktu:
	# korelat wynika ze wspolobecnosci obu zrodel wymaganych bramka syntezy.
	_register_beat(&"s13_device_correlate", GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT, &"interpretation", &"fallible", "Czytnik i wyciąg pokazują tę samą lukę. Dwa urządzenia, jeden niemożliwy brak.", "The reader and the extract show the same gap. Two devices, one impossible absence.", &"", "")
	# PKG-0230 (P1-1): zdanie wyjscia — 13 → 14: wyciag z 11 wskazuje sekcje
	# rozdzielni; Lena wychodzi z mieszkania i schodzi wlazem serwisowym
	# (HATCH Bindera, MAPA). Bez przedwczesnej wiedzy z 15: o zewnetrznym
	# przerwaniu proby dowiaduje sie dopiero dziennik petli (PKG-0233, C1).
	_register_beat(&"s13_exit_to_switchyard", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Wychodzę z mieszkania. Wyciąg wskazuje sekcję rozdzielni — zejdę włazem serwisowym.", "I leave the flat. The extract points at the switchyard section — I go down the service hatch.", &"", "")
...
func mark_marta_source_seen() -> bool:
	...
	_maybe_announce_device_correlate()
	queue_redraw()
	return true
...
## PKG-0233 (B4, S-13): korelat odzywa sie raz, przy domknieciu drugiego
## zrodla — bez wzgledu na kolejnosc wykladania. Bez nowych faktow.
func _maybe_announce_device_correlate() -> void:
	if not (_decision_bool(P9_MARTA_SOURCE_SEEN) and _decision_bool(P9_INSTITUTION_SOURCE_SEEN)):
		return
	if guidance_service:
		guidance_service.trigger_beat(&"s13_device_correlate")
...
func _draw() -> void:
	# PKG-0218 (V1): stage apron first (D-136: dialogue offset 36 spends
	# painted floor, STAGE_APRON 40, never engine clear colour).
	...
	# PKG-0233 (B3, S-09): czytnik lezy na stole z pokwitowaniem UCP.
	# _has (nie _decision_bool): wartoscia pokwitowania jest String.
	if _decision_bool(P9_MARTA_SOURCE_SEEN) and _has(&"p9.mystery.institution.reader_custody_receipt"):
		draw_rect(Rect2(300, 246, 18, 5), Color("8b9a94"))
		draw_line(Vector2(303, 248.0), Vector2(315, 248.0), Color("292932"), 2.0)
	# PKG-0233 (B4, S-13): fizyczny korelat syntezy — czytnik terenowy
	# i wyciag UCP z ta sama niemozliwa luka. Dwa identyczne wyciecia na
	# obu nosnikach plus linia powiazania; warunek = oba zrodla na stole.
	if _decision_bool(P9_MARTA_SOURCE_SEEN) and _decision_bool(P9_INSTITUTION_SOURCE_SEEN) and _decision_bool(&"recognition_evidence_public"):
		var gap_color := Color("e4bd82")
		draw_line(Vector2(304.0, 230.0), Vector2(314.0, 230.0), gap_color, 2.0)
		draw_line(Vector2(362.0, 236.0), Vector2(392.0, 236.0), gap_color, 2.0)
		draw_line(Vector2(318.0, 230.0), Vector2(358.0, 236.0), Color(gap_color, 0.5), 2.0)
...
func synthesize_world_difference() -> bool:
	...
	if guidance_service:
		guidance_service.trigger_beat(&"s13_exit_to_switchyard")
	_unlock_exit()
	queue_redraw()
	return true
