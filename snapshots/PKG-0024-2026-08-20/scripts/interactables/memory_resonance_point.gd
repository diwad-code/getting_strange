...
func _process(delta: float) -> void:
	...
	if _resonance_flash > 0.0:
		...
		queue_redraw()
	elif is_player_in_range or prop_type == PropType.DESK_TELEPHONE or prop_type == PropType.DOOR_CARD_READER or prop_type == PropType.GOLD_RING or prop_type == PropType.BUS_SPEAKER:
		queue_redraw()
