...
func _test_m1_continuous_playthrough(state: Node) -> void:
	...
	# Krok 4: Pętla przez stacje 02 .. 18 -> 42a -> 43
	for index in range(1, CAMPAIGN_20_ROUTE.size()):
		var target_id := CAMPAIGN_20_ROUTE[index]
...
func _drive_station_step(st: Node2D, station_id: StringName, state: Node) -> void:
	...
	match sid:
		...
		"station_18":
			...
			await _press_semantic_action(&"interact")
			await _advance_all_dialogue(st)
			# Słupek zatwierdzenia na x = 484; stajemy na x = 450 (< 484 - 24) -> wybór force_home
			await _walk_right_to(player, 450.0, 240)
			await _press_semantic_action(&"interact")
			await _advance_all_dialogue(st)
			if is_instance_valid(st):
				await _walk_right_to(player, 585.0, 240)
