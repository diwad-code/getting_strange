...
func _setup_guidance() -> void:
	...
	if guidance:
		...
		beat.beat_id = &"s03_marta_message"
		beat.scene_id = &"station_03"
		beat.tier = GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT
		beat.thought_kind = &"intention"
		beat.text_pl = "Wracam do domu. Marta czeka."
		beat.text_en = "Heading home. Marta is waiting."
		beat.cooldown_s = 8.0
...
func _draw_state_layer() -> void:
	...
	var clue_count := int(cups_inspected) + int(phone_inspected) + int(roster_inspected)
	var clue_col := VectorStageStyle.CORRECTION_OXIDE
	if clue_count >= 2:
		clue_col = VectorStageStyle.HUMAN_AMBER
	if is_door_unlocked:
		clue_col = VectorStageStyle.ANCHOR_CYAN
	draw_rect(Rect2(138.0, 226.0, 96.0, 5.0), VectorStageStyle.INK)
	draw_rect(Rect2(138.0, 226.0, 24.0 * float(clue_count), 5.0), clue_col)
	draw_line(Vector2(518.0, 154.0), Vector2(518.0, 248.0), clue_col, 3.0)
	if card_reader_inspected:
		draw_rect(Rect2(508.0, 166.0, 20.0, 8.0), VectorStageStyle.ANCHOR_CYAN)
