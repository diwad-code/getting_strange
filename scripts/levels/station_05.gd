...
## PKG-0225 (K6): ulica-rym 05/18 — szyld nocnych prac UCP (tracker C-05:
## obecność przed zagadką, nowy wykonawca) i bazowy cykl lampy; 18 bierze ten
## sam cykl z opóźnieniem jednej klatki (RHYME_FRAME_DELAY w station_18).
...
func _physics_process(delta: float) -> void:
	_rhyme_phase = fmod(_rhyme_phase + delta, TAU)
...
func _draw() -> void:
	...
	# Signpost / Street sign at x=160 — PKG-0225 (K6): szyld nocnych prac UCP
	# (tracker C-05), ta sama płyta co w 18 (RHYME_SIGN_SIZE) z tym samym
	# odpryskiem w prawym górnym rogu; kolor niesie stan sprawdzenia trasy.
	...
	draw_rect(Rect2(146.0, 230.0, RHYME_SIGN_SIZE.x, RHYME_SIGN_SIZE.y), sign_color, false, 1.5)
	draw_colored_polygon(
		PackedVector2Array([Vector2(168.0, 230.0), Vector2(174.0, 230.0), Vector2(174.0, 236.0)]),
	...
	# Crossing signal at x=480 — PKG-0225 (K6): bazowy cykl rymu ulicy;
	# latarnia 18 bierze ten sam cykl z opóźnieniem jednej klatki.
	...
	var rhyme_blink := 0.75 + 0.25 * sin(_rhyme_phase * RHYME_BLINK_SPEED)
	draw_circle(Vector2(480.0, 226.0), 5.0, Color(signal_color, rhyme_blink))
