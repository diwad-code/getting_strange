...
func _test_finale_branch_flows(state: Node) -> void:
	for branch in ["A", "B", "C"]:
		...
		if finale_station != null:
			match finale_id:
				&"station_42a":
					_expect(_call_bool(finale_station, &"inspect_cups", "station_42a missing inspect_cups"), "station_42a cups inspection must succeed")
				...
				&"station_42b":
					_expect(_call_bool(finale_station, &"inspect_doorstep", "station_42b missing inspect_doorstep"), "station_42b doorstep inspection must succeed")
