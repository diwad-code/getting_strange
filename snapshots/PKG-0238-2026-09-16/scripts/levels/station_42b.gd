...
## o wiedzę przed testem, a perspektywa przybyłej Leny stoi w progu mieszkania 14
## bez indeksu (wiata linii 03 należy do epilogu 43B, PKG-0237 D4 / PKG-0238 E3).
## Bez hasła o dwóch prawdziwych; bez oceny moralnej. Dostarcza lokalny prezenter
## CRT (wzór CR-A/CR-B) z `creative_scene_lines.gd`.
...
func execute_close_flow() -> bool:
	if is_flow_closed and _decided(FACT_FLOW_CLOSED):
		return false
...
func read_local_lena_recovered() -> bool:
	if is_local_lena_recovered and _decided(FACT_LOCAL_RECOVERED):
		return false
	if not _has_close_equal():
		_record_feedback(&"method_close_equal_required")
		return false
	...
	if not _decided(FACT_LOCAL_RECOVERED):
...
func read_household_consequence() -> bool:
	...
	jakub_consent_state = _read_jakub_consent()
	payoff_cost = _read_small_cost()
	# PKG-0230 (P0-1, S-02): jak w 42A/42C — pusta zgoda jest luka
	# (consent_scope_required), nie domyslnym SCOPE_LIMITED.
	...
	household_consequence = {
		...
		# bramka 0195 czyta tylko klucze marta/jakub/local_lena/arrived_lena).
		"cost": payoff_cost,
	}
...
func _trigger_level_completion() -> void:
	...
	if not ((is_flow_closed or _decided(FACT_FLOW_CLOSED)) \
			and (is_local_lena_recovered or _decided(FACT_LOCAL_RECOVERED)) \
...
func _draw_dawn_apartment_b() -> void:
	...
	# PKG-0225 (K1): puste miejsce po kurtce — hak na ścianie między progiem
	# a stołem, kurtki nie ma (beat s42b_hook). Prymitywy, zero assetów.
	draw_line(Vector2(400.0, 210.0), Vector2(400.0, 228.0), VectorStageStyle.MID_PLANE, 2.0)
	draw_arc(Vector2(400.0, 232.0), 4.0, 0.0, PI, 8, VectorStageStyle.MID_PLANE, 1.5)
...
func _draw_household_table() -> void:
	...
	# PKG-0225 (K5): blizna ciałem — koszula ściągnięta w dół na oparciu krzesła,
	# dół poniżej listwy oparcia (beat s42b_shirt). Jakub zasłania bok, nie
	# pokazuje (granica z 12); prymitywy, zero assetów.
	draw_colored_polygon(
		PackedVector2Array([Vector2(510.0, 262.0), Vector2(534.0, 262.0), Vector2(532.0, 270.0), Vector2(512.0, 270.0)]),
		VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.5),
	)
	draw_line(Vector2(510.0, 270.0), Vector2(534.0, 270.0), VectorStageStyle.LIGHT_PLANE, 1.0)
