...
const OUTPUT_PATH := "res://reports/pkg_0183/performance.tsv"
...
func _run() -> void:
	...
	print("PKG-0183 PERFORMANCE: driver=%s renderer=%s gpu=%s" % [driver, renderer, RenderingServer.get_video_adapter_name()])
	if driver == "headless":
	...
	if _failures.is_empty():
		print("PKG-0183 PERFORMANCE PASS: %d active addresses measured on normal driver." % ACTIVE_IDS.size())
		quit(0)
	else:
		for failure in _failures:
			push_error("PKG-0183 PERFORMANCE: " + failure)
		quit(1)
