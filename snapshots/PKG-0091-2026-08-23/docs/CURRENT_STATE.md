...
`PKG-0091: Szlif Sterowania, Game Feel, Oświetlenie 2D i Interfejs Dialogowy w Grze Godot 4.7`

Cel: Kontynuacja wyłącznie gry Godot (D-091) — dokończenie P4 w silniku:
1. Kalibracja fizyki gracza `PrototypePlayer` (coyote time, buforowanie skoku, dynamiczny squash-and-stretch, pochylenie przy zwrocie, cząsteczki kurzu `CPUParticles2D` przy odbiciu i lądowaniu);
2. Wdrożenie kinowego oświetlenia 2D `PointLight2D` i cieni okluzyjnych w kluczowych poziomach `scenes/levels/station_01.tscn` .. `station_05.tscn` z efektem jarzeniówek 100 Hz;
3. Rozbudowa interfejsu dialogowego w świecie gry o stylowe ramki CRT/Teletype 1978 z portretami postaci i płynnym efektem maszyny do pisania;
4. Wdrożenie globalnego menedżera stanu gry i punktów kontrolnych `GameStateManager` (autoload) z zapisem postępu, restartem od checkpointa i płynnymi przejściami między 43 przestrzeniami;
5. Automatyczne testy i weryfikacja: `tools/verify.ps1`, aktualizacja dokumentacji i zamrożenie snapshotu PKG-0091.
...
plik i `NEXT_SESSION_PROMPT.md`, a nastepnie przechodzi do realizacji PKG-0091 w silniku Godot 4.7.
