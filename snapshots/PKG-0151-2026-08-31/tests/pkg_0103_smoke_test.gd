...
func _run() -> void:
	...
	if _failures.is_empty():
		print("PKG-0103 SMOKE PASS: Act III Vector-Stage route, R5/R1 mechanisms and production chain")
		quit(0)
	else:
		...
		if geometry:
			...
			for child in geometry.get_children():
				if child is AnimatableBody2D:
					obstacle_count += 1
					_expect(
						(expected_number == 26 and child.name == "AdaptiveIsolationPartition"),
						"unexpected physical obstacle in Station %d: %s" % [expected_number, child.name]
					)
			_expect(
				obstacle_count == (1 if expected_number == 26 else 0),
...
func _check_station_26(station: Station26, state: Node) -> void:
	...
	_expect(not station.is_ucp_intervention_reconstructed, "Station 26 diagnostic trial must start unresolved")
	_expect(station.has_method(&"reconstruct_ucp_intervention"), "Station 26 must expose its diagnostic reconstruction verb")
	if state:
		state.record_decision(&"p7.three_place_proofs.trace", "world_recognized_and_search_committed")
	station.observe_interrupted_ucp_log()
	station.synchronize_sample_clock()
	station.synchronize_ucp_command_clock()
	station.synchronize_local_generator_clock()
	station.reconstruct_ucp_intervention()
	if partition != null:
		var collision := partition.get_node_or_null("CollisionShape2D") as CollisionShape2D
		_expect(collision != null, "Station 26 adaptive partition must retain its collider node")
		if collision != null:
			_expect(collision.disabled, "Station 26 partition collider must be disabled after diagnostic unlock")


func _check_station_30(station: Station30, _state: Node) -> void:
	_expect(station != null, "Station 30 script class is missing")
	if station == null:
		return
	_expect(station.get_node_or_null("Geometry/WitnessRelayBank") == null, "Station 30 legacy witness relay must be removed after forecast cutover")
	_expect(not station.are_forecast_consent_dependencies_compared, "Station 30 forecast comparison must start unresolved")
	_expect(station.has_method(&"compare_forecast_consent_dependencies"), "Station 30 must expose its diagnostic forecast verb")
...
func _check_campaign_boundary(state: Node) -> void:
	...
	_expect(state.has_reached_station(&"station_26"), "PKG-0103 must preserve the production chain beyond Station 25")
