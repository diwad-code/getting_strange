...
func _check_obstacle(expected_number: int, station: Node2D, state: Node) -> void:
	...
	match expected_number:
		17:
			...
			if station_17:
				station_17.apply_intercom_setback()
				_expect(station_17.intercom_setback_count > 0, "Station 17 setback must count a lost attempt")
				_expect(station_17.timestamp_smudged, "Station 17 setback must mark timestamp smudged")
				_expect(state != null and state.decisions.has(&"station_17_alarm_delayed"), "Station 17 setback must be recorded")
				station_17.read_incident_report()
				station_17.copy_report_header()
				station_17.pull_vent_lever()
				_expect(station_17.is_header_copied and station_17.is_vent_pulled, "Station 17 route must be cleared")
		19:
			var station_19 := station as Station19
			if station_19:
				station_19.apply_phone_setback()
				_expect(station_19.phone_setback_count > 0, "Station 19 setback must count a lost attempt")
				_expect(station_19.line_interrupted, "Station 19 setback must mark line interrupted")
				_expect(state != null and state.decisions.has(&"station_19_call_interrupted"), "Station 19 setback must be recorded")
				station_19.answer_phone()
				for i in range(station_19.phone_dialogue_lines.size()):
					station_19.advance_phone_dialogue()
				_expect(station_19.is_phone_completed, "Station 19 phone dialogue must complete")
		20:
			var station_20 := station as Station20
			if station_20:
				station_20.apply_boundary_setback()
				_expect(station_20.boundary_setback_count > 0, "Station 20 setback must count a lost attempt")
				_expect(station_20.jakub_stepped_back, "Station 20 setback must mark Jakub stepped back")
				_expect(state != null and state.decisions.has(&"station_20_boundary_violated"), "Station 20 setback must be recorded")
				station_20.start_meeting_dialogue()
				for i in range(station_20.meeting_dialogue_lines.size()):
					station_20.advance_meeting_dialogue()
				_expect(station_20.is_dialogue_completed, "Station 20 meeting must complete")
