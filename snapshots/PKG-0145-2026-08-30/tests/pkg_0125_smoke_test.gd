...
func _test_lena_feminine_rig_and_proportions() -> void:
	...
	rig.set_mechanical_state(Vector2(80, 0), true, false, false)
	_expect(rig.get_active_state_name() == &"walk", "Mechanical state (80, grounded, no sprint) should be walk")

	rig.set_mechanical_state(Vector2(80, 0), true, false, true)
	_expect(rig.get_active_state_name() == &"run", "Mechanical state (80, grounded, sprint) should be run")
