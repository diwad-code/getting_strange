...
## PRZESZKODA — dlaczego to tu jest: akustyczna przegroda pracuje, bo strefa
## łagodnej izolacji realokuje funkcje pomieszczeń po komunikacie PA.
## PRZESZKODA — czego wymaga od Leny: rozpoznania, że zmiana układu jest pracą
## urządzenia, a nie zaproszeniem do skoku ani pościgu.
## PRZESZKODA — koszt porażki: korekta odsyła Lenę do checkpointu i wygasza
## najostrzejszą krawędź wskaźnika funkcji.
...
@onready var adaptive_partition: AnimatableBody2D = $Geometry/AdaptiveIsolationPartition
...
const PARTITION_CLOSED_POSITION := Vector2(360.0, 170.0)
const PARTITION_OPEN_POSITION := Vector2(548.0, 170.0)
const PARTITION_CYCLE_LENGTH := 4.0
...
func _ready() -> void:
	_setup_station()
	_connect_signals()
	_restore_partition_detail_state()
	station_entered.emit()


func _process(delta: float) -> void:
	_pulse_time += delta
	partition_cycle_time = fposmod(partition_cycle_time + delta, PARTITION_CYCLE_LENGTH)
...
func _update_partition_cycle() -> void:
	if adaptive_partition == null:
		return
	var phase := fposmod(partition_cycle_time, PARTITION_CYCLE_LENGTH)
	var open_progress := 0.0
	if phase >= 1.5 and phase < 2.0:
		open_progress = inverse_lerp(1.5, 2.0, phase)
	elif phase >= 2.0 and phase < 3.0:
		open_progress = 1.0
	elif phase >= 3.0 and phase < 3.5:
		open_progress = 1.0 - inverse_lerp(3.0, 3.5, phase)
	is_partition_open = current_room_state == RoomState.SEDATION and open_progress >= 0.92
	if current_room_state != RoomState.SEDATION:
		open_progress = 0.0
	adaptive_partition.position = PARTITION_CLOSED_POSITION.lerp(PARTITION_OPEN_POSITION, open_progress)


func _restore_partition_detail_state() -> void:
	var state := get_node_or_null("/root/GameStateManager")
	if state and state.has_method("has_recorded_decision"):
		partition_detail_faded = state.has_recorded_decision(&"station_26_isolation_partition_corrected")
	elif state:
		var decisions: Variant = state.get("decisions")
		if decisions is Dictionary:
			partition_detail_faded = (decisions as Dictionary).has(&"station_26_isolation_partition_corrected")
	if adaptive_partition:
		adaptive_partition.position = PARTITION_CLOSED_POSITION
...
func _apply_isolation_correction() -> void:
	...
	var designator_prop := props.get_node_or_null("RoomDesignator") as MemoryResonancePoint if props else null
	if designator_prop:
		designator_prop.prop_subtitle = "Wskaźnik funkcji — krawędź odczytu wyblakła po korekcie"
	if player:
		player.reset_to(Vector2(50.0, 240.0))
	queue_redraw()
...
func _draw_state_layer() -> void:
	...
	if adaptive_partition:
		var partition_center := adaptive_partition.position
		var partition_color := VectorStageStyle.ANCHOR_CYAN if is_partition_open else VectorStageStyle.MID_PLANE
		if partition_detail_faded:
			partition_color = VectorStageStyle.shade(partition_color, 0.28)
		var half_size := Vector2(12.0, 110.0)
		VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
			partition_center + Vector2(-half_size.x, -half_size.y),
			partition_center + Vector2(half_size.x, -half_size.y),
			partition_center + Vector2(half_size.x + 8.0, half_size.y),
			partition_center + Vector2(-half_size.x, half_size.y),
		]), partition_color, 1.0)
		if is_partition_open:
			draw_line(partition_center + Vector2(0.0, -92.0), partition_center + Vector2(0.0, 92.0), VectorStageStyle.LIGHT_PLANE, 1.0)
	var state_color := VectorStageStyle.CORRECTION_OXIDE if partition_detail_faded else VectorStageStyle.HUMAN_AMBER
	draw_line(Vector2(276.0, 54.0), Vector2(364.0, 54.0), state_color, 2.0)
