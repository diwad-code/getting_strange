...
# PKG-0162 normal-driver evidence. Run without --headless.
# Three textless M2 frames of Station 14 (neutral / anchor / yield) plus one
# true-grayscale M3 structure frame of the technical family.const PACKAGE_ID := "PKG-0162"
const LOGICAL_SIZE := Vector2i(640, 360)
const OUTPUT_ROOT := "res://reports/pkg_0162"
const REPORT_PATH := "res://reports/pkg_0162/visual_evidence_report.txt"
const WAIT_FRAMES := 20

const CASES: Array[Dictionary] = [
	{"id": "neutral", "mono": false},
	{"id": "anchor", "mono": false},
	{"id": "yield", "mono": false},
...
func _capture_one(capture: Dictionary, state: Node) -> void:
	...
	if packed == null:
		_failures.append("cannot load station_14")
		return
	var station := packed.instantiate() as CanvasItem
	if station == null:
		_failures.append("station_14 root is not CanvasItem")
		return
	...
	var relative_path := OUTPUT_ROOT.path_join("station_14_%s.png" % String(capture["id"]))
...
func _write_report() -> void:
	...
	var lines := PackedStringArray([
		"PKG-0162 VISUAL EVIDENCE REPORT",
		"Renderer: %s" % RenderingServer.get_video_adapter_name(),
		"Viewport: 640x360; normal Windows display driver; script was not headless.",
		"Three textless M2 frames of Station 14: neutral, held version A, yielded version B.",
		"One M3 true-grayscale structure frame of the technical family (machine >= 1/3 width).",
