...
static func install(station: Node) -> void:
	...
	_rewire(station)
	if bool(station.get("is_exit_unlocked")):
		zone.is_open = true
...
static func spec_for(station_id: String) -> Dictionary:
	...
	match id:
		...
		"station_12":
			return {"family": ThresholdZone.Family.DOOR, "width": 48.0, "height": 112.0, "door": "Geometry/BalconyDoor"}
...
static func complete_from_test(station: Node, player: Node2D = null) -> bool:
	...
	if zone == null:
		if station.has_method("_trigger_level_completion"):
			...
			return bool(station.get("is_level_completed"))
		return false
...
static func _rewire(station: Node) -> void:
	...
	if zone == null:
		return
	if not zone.crossed.is_connected(func(): _complete(station)):
		zone.crossed.connect(func(): _complete(station))
