...
| R-018 | Sceny istnieją, ale produkt nie ma produkcyjnego wejścia ani pełnego ciągu do napisów | **pewne — zmaterializowane w PKG-0113** | blokujące wydanie | `run/main_scene` wskazuje Movement Lab, limit centralnych przejść wynosi 25, a 42A/B/C i 43 nie są spięte z normalną kampanią. Następny pakiet wykonuje R0: menu główne, pełną topologię i test end-to-end przed eksportem |
| R-019 | Zielony smoke deklaruje szerszy zakres niż rzeczywiście wywołuje | **zmaterializowane, ograniczone w PKG-0113** | bardzo wysoki | główny smoke kończył się po Station 19 mimo funkcji 20..43. Dodano wszystkie wywołania, naprawiono prawdziwe granice Anchor Station 32/38 i gate źródłowy PKG-0113 pilnujący 01..43 |
...
| R-021 | Build może działać w edytorze, lecz zawieść jako publiczny pakiet PC | wysokie | blokujące wydanie | brak `export_presets.cfg`, buildów, czystej instalacji, pełnego pada/ustawień, kompletnej lokalizacji, credits i manifestu praw. R3/R4 w poprawionej roadmapie są obowiązkowe przed nazwaniem produktu gotowym |
