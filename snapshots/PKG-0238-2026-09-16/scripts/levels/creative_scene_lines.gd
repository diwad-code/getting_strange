...
const LINES := {
	...
	"identity_card": [["Wierzbicka", "Stan: niezgodność zakresu. W dopuszczalnym odchyleniu wymagane potwierdzenie biometryczne."], ["Lena", "A teraz?"], ["Wierzbicka", "Stan: Lena Wolska, zespół UCP-4. Zgodność w dopuszczalnym zakresie, stabilność potwierdzona."], ["Lena", "Proszę pokazać historię wejść."]],
	...
	"minimal_report": [["Wierzbicka", "Stan: wyciąg w zakresie procedury. Czytnik pozostaje do sprawdzenia zgodności."], ["Lena", "Zostaje ze mną. Potrzebuję godziny próby i numeru urządzenia."], ["WYCIĄG UCP", "20:40 — próba równoległa. Numer czytnika terenowego: brak w rejestrze."], ["Lena", "Zabieram tylko tę stronę i kontakt do warsztatu."]],
	...
	"synthesize": [["Lena", "Czytnik jest mój. Wasze zapisy i wspomnienia do niego nie pasują. Jakub żyje."], ["Lena", "To nie jest mój świat."], ["Marta", "Więc gdzie jest ona?"], ["Lena", "Nie wiem."], ["JAKUB (ŁĄCZE)", "Tego numeru nie ma w naszej bazie. Mój czas się zgadza."], ["Lena", "Sprawdzę ją. Najpierw muszę ją odnaleźć."]],
	...
	"loop_logbook": [["DZIENNIK PĘTLI", "20:40 — próba miejscowej. Kontakt przy pierwszym odczycie."], ["Lena", "Powtórka dała pełną próbkę. Wyjście na czas: sam czytnik, bez próbki. Próbę ktoś przerwał z zewnątrz."]],
	...
	"abort_note": [["NOTATKA SERWISOWA", "Brak odpowiedzi w trzy sekundy — przerwij (na marginesie). Bez jej zgody nie powtarzaj."], ["Lena", "Zostawiła warunek przerwania. To była decyzja, nie wypadek."], ["Lena", "Przerwę tak samo: po braku odpowiedzi. Za nią działać nie będę."]],
	...
	"cost_selector_sample_full": [["Lena", "Wybieram sekundę surowej próbki. Zapis pokaże lukę tam, gdzie była pełnia."], ["ANALIZATOR", "Sekunda 20:40:07 — ostrość utracona. Oś: pik spada. Reszta śladu czytelna."], ["Lena", "Pamięć Marty zostaje cała. Płacę zapisem, nie nią."]],
	"cost_selector_sample_buffer": [["Lena", "Wybieram sekundę z bufora czytnika. Próbki nie zabezpieczyłam — płacę tym, co mam."], ["ANALIZATOR", "Sekunda 20:40:07 — ostrość utracona. Oś: pik spada. Reszta śladu czytelna."], ["Lena", "Pamięć Marty zostaje cała. Płacę zapisem, nie nią."]],
	...
	"cost_ledger_console": [["REJESTR UCP", "Rejestr 04/17: dwa końce pary — stabilna Równia i pogrzeb Jakuba. Wpisane jedną ręką."], ["Lena", "Równia. Tym słowem podpisali to miejsce. Moje nie miało nazwy na papierze."], ["Lena", "Ktoś utrzymuje zapis. Ktoś drugi zapłacił. Korelacja to nie dowód powiązanego kosztu."]],
	"adaptation_offer_terminal": [["WIERZBICKA", "Stan: wpis w zakresie miejscowej. Stabilność: dom, Marta, żywy Jakub."], ["Lena", "Głos z lady. Bez kosztu? Bez cudzej pamięci w mojej głowie?"], ["WIERZBICKA", "Procedura zachowuje rolę w dopuszczalnym odchyleniu. Wspomnienia zostałyby wygładzone."], ["Lena", "Odmawiam. Nie będę wygodnym zastępstwem."]],
	...
	"consent_scope_desk_granted": [["Lena", "Potrzebuję twojej ręki na nadajniku. Dziesięć sekund, ty na wyłączniku."], ["JAKUB (ŁĄCZE)", "Ten napęd muszę oddać przed końcem zmiany. Potem dziesięć sekund."], ["JAKUB (ŁĄCZE)", "Mój nadajnik, moja ręka na wyłączniku."], ["Lena", "Dziesięć."]],
	"consent_scope_desk_limited": [["Lena", "Potrzebuję twojej ręki na nadajniku przy próbie."], ["JAKUB (ŁĄCZE)", "Nadajnika do mnie nie podłączysz. Sprawdzę wskazania."], ["JAKUB (ŁĄCZE)", "Ten napęd oddaję przed końcem zmiany. Tyle mogę."], ["Lena", "Tyle wystarczy. Resztę zrobię sama."]],
	"consent_scope_desk_refused": [["Lena", "Potrzebuję twojej ręki na nadajniku przy próbie."], ["JAKUB (ŁĄCZE)", "Przyszłaś po odczyt, a teraz każesz mi podpisać protokół in blanco. Nie ze mną, Lena."], ["Lena", "Ten napęd i tak musisz oddać przed końcem zmiany. Nie zatrzymuję cię."]],
	...
	"forecast_comparator_granted": [["REJESTR UCP", "Wymuszenie domu: chroni mój powrót. Strata: jej miejsce. 20:40, brak danych: czy kanał da się zamknąć."], ["ANALIZATOR", "Zamknięcie z odzyskaniem miejscowej: chroni jej powrót. Strata: adres. Oś spada — brak danych: gdzie jestem."], ["NOTATKA", "Przejście wzajemne — chroni obie (na marginesie). Strata: przeciek pamięci. Brak danych: co połączy się na stałe."]],
	...
	"method_commit_post_force_home": [["Lena", "Zatwierdzam wymuszenie domu. Staję przy słupku sama."], ["Lena", "Kładę rękę na słupku. Braki zostają nazwane."]],
	"method_commit_post_close_equal_recover_local": [["Lena", "Zatwierdzam zamknięcie z odzyskaniem miejscowej. Oddaję jej miejsce."], ["Lena", "Bez jej adresu nie wracam. Czekam na jej klucz."]],
	"method_commit_post_mutual_passage": [["Lena", "Zatwierdzam przejście wzajemne. Otwieram, nie zabieram."], ["Lena", "Przeciek przyjmuję z góry. Mostu nie gaszę."]],
	...
	"memory_leak": [["JAKUB (ECHO)", "Przy imadle — przez chwilę prosektorium. Potem znowu imadło."], ["Lena", "To mój pogrzeb w twojej głowie. Nie twój."], ["JAKUB (ECHO)", "Wracam do napędu. To mija."], ["EKRAN CZYTNIKA", "Oba czytniki: ta sama brakująca sekunda po rozdzieleniu."]],
	...
	"household_c_full": [["Marta domowa", "Znam ten kubek."], ["Lena", "Nie masz go."], ["Lena", "Obie patrzymy na pustą półkę."], ["Marta", "A kto odpowie za przeciek?"], ["Miejscowa Lena", "Obie. Przed własnymi domami."], ["DWA ZAPISY", "Ten sam ubytek po obu stronach. UCP nie ma wyłączności na opis."], ["EKRAN CZYTNIKA", "Dalszy dryf: wspólny ubytek czasu po rozdzieleniu."], ["Marta domowa", "Znam detal, którego nie przeżyłam. Zostaje na półce."]],
	"household_c_partial": [["Marta domowa", "Znam ten kubek."], ["Lena", "O półce powiem, kiedy będę umiała."], ["Marta", "A kto odpowie za przeciek?"], ["Miejscowa Lena", "Obie. Przed własnymi domami."], ["DWA ZAPISY", "Ten sam ubytek po obu stronach. UCP nie ma wyłączności na opis."], ["EKRAN CZYTNIKA", "Dalszy dryf: wspólny ubytek czasu po rozdzieleniu."], ["Marta domowa", "Półkę opiszesz, kiedy będziesz umiała. Czekam."]],
	"household_c_withheld": [["Marta domowa", "Milczysz."], ["Lena", "Przyjdę z zapisem. Nie z legendą."], ["Marta", "A kto odpowie za przeciek?"], ["Miejscowa Lena", "Obie. Przed własnymi domami."], ["DWA ZAPISY", "Ten sam ubytek po obu stronach. UCP nie ma wyłączności na opis."], ["EKRAN CZYTNIKA", "Dalszy dryf: wspólny ubytek czasu po rozdzieleniu."], ["Marta domowa", "Milczysz. Półka zostaje pusta do czasu zapisu."]],
}
...
static func lines_for(id: String, decisions: Dictionary, station: Node = null) -> Array:
	...
	if id == "method_commit_post" and decisions.get(&"world_recognized", false) == true:
		...
		# PKG-0226 (N6-reszta): stół sześciu rzeczy (FULL_STORY §39) — przegląd
		# konkretnego stanu sześciu pozycji przy zatwierdzeniu. Wyłącznie
		# odczyt istniejących decyzji, zero nowych faktów/flag; brak to luka,
		# nie domysł (wzór forecast_comparator_missing). Kopia robocza, żeby
		# nie mutować const LINES (append działa na kopii).
		pairs = pairs.duplicate()
...
static func _commit_review_table(decisions: Dictionary) -> Array:
	# PKG-0226 (N6-reszta): sześć pozycji stołu z FULL_STORY §39 — próbka
	# domowa, sygnał miejscowej, Marta, Jakub, rejestr UCP, węzeł. Każda para
	# nazywa konkretny stan ze spare'owanych decyzji; głos Leny, bo to jej
	# przegląd przed nieodwracalnym wynikiem. Bez tez (D-214/D-219).
	var table: Array = []
	if _truthy(decisions.get(&"home_sample_preserved", false)):
		table.append(["Lena", "Próbka domowa: surowa próbka z powtórzonego pomiaru."])
	else:
		table.append(["Lena", "Próbka domowa: tylko zapis z bufora czytnika."])
	if _truthy(decisions.get(&"p9.mechanics.mutual_signal.log_reconstructed", false)):
		table.append(["Lena", "Sygnał miejscowej: kontakt przy pierwszym odczycie."])
	else:
		table.append(["Lena", "Sygnał miejscowej: brak kontaktu w dzienniku."])
	match String(decisions.get(&"marta_truth_state", decisions.get(&"p9.method_commitment.marta_truth_state", ""))):
		"full":
			table.append(["Lena", "Marta: pełny zapis przekazany."])
		"withheld":
			table.append(["Lena", "Marta: zapis wstrzymany."])
		"partial":
			table.append(["Lena", "Marta: część zapisana, reszta wstrzymana."])
		_:
			table.append(["Lena", "Marta: stan nieustalony."])
	match _read_consent(decisions):
		"granted":
			table.append(["Lena", "Jakub: pełna zgoda, jego ręka na wyłączniku."])
		"limited":
			table.append(["Lena", "Jakub: zgoda ograniczona, tylko wskazania."])
		"refused":
			table.append(["Lena", "Jakub: odmowa, relacja zamknięta."])
		_:
			table.append(["Lena", "Jakub: zakres nieustalony."])
	if _truthy(decisions.get(&"p9.consent_and_cost.cost_ledger_read", false)):
		table.append(["Lena", "Rejestr UCP: para zdarzeń odczytana."])
	else:
		table.append(["Lena", "Rejestr UCP: luka, brak odczytu."])
	table.append(["Lena", _knot_state_line(decisions)])
	return table


static func _knot_state_line(decisions: Dictionary) -> String:
	# Węzeł: ile z trzech dróg ma lukę w istniejącym słowniku prognoz
	# p9.method_commitment.forecasts; brak słownika to jawna luka.
	var forecasts: Variant = decisions.get(&"p9.method_commitment.forecasts", null)
	if not (forecasts is Dictionary):
		return "Węzeł: prognozy niezestawione."
	var gaps := 0
	for route_id: String in ["force_home", "close_equal_recover_local", "mutual_passage"]:
		var entry: Variant = (forecasts as Dictionary).get(route_id, {})
		if not (entry is Dictionary and bool((entry as Dictionary).get("available", false))):
			gaps += 1
	if gaps == 0:
		return "Węzeł: wszystkie drogi policzone."
	return "Węzeł: %d z 3 dróg z luką." % gaps
