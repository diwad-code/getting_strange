...
## PKG-0183 normal-driver evidence capture. This script is not a gameplay route
## proof; player-verb routes are re-run via pkg_0177 -- --pkg0183-route= into
## reports/pkg_0183/runtime_routes/.
...
var _rows := PackedStringArray([
	"surface\tmode\tpath\tdriver\twidth\theight\ttextless\tmd5\tmeasurement\theuristic",
])
...
func _run() -> void:
	...
	if _failures.is_empty():
		print("PKG-0183 CAPTURE PASS: %d normal-driver frames on %s." % [_rows.size() - 1, DisplayServer.get_name()])
		quit(0)
	else:
		for failure in _failures:
			push_error("PKG-0183 CAPTURE: " + failure)
		quit(1)
...
func _capture_shells(state: Node) -> void:
	...
	await _settle(8)
	await _save_frame("shell_title", "pl", false)
	if title.has_method("open_settings_for_test"):
		title.open_settings_for_test()
		await _settle(4)
		await _save_frame("settings", "pl", false)
	title.free()
	await process_frame
	...
	await _settle(6)
	await _save_frame("shell_title", "en", false)
	title_en.free()
	await process_frame
	if state != null:
		state.set_locale("pl", false)
	...
	if cold_scene != null:
		...
		root.add_child(cold)
		await _settle(12)
		await _save_frame("cold_open", "initial", false)
		cold.free()
		await process_frame
	else:
...
func _capture_station(station_id: String, state: Node) -> void:
	...
	await _settle(2)
	await _save_frame(station_id, "normal", false)
	...
	if player != null:
		...
		await _settle(5)
	await _save_frame(station_id, "key_object", false)
	...
	_complete_visible_dialogue(station)
	await _settle(2)
	await _save_frame(station_id, "interaction", false)

	var hidden_layers: Array[CanvasLayer] = []
	_collect_hidden_ui_layers(station, hidden_layers)
	await _settle(3)
	await _save_frame(station_id, "m3_mono", true)
	...
	if station_id == "station_14" and state != null:
		...
		await _settle(5)
		await _save_frame(station_id, "reduced_motion", false)
		state.set_reduced_motion(false, false)

	if station_id == "station_01" and state != null:
		state.set_pause_menu_visible(true)
		await _settle(4)
		await _save_frame("pause", "campaign", false)
		state.set_pause_menu_visible(false)
...
func _capture_anchor_yield(state: Node) -> void:
	...
	root.add_child(lab)
	await _settle(8)
	await _save_frame("anchor_yield", "neutral", false)
	...
	if anchor != null:
		...
		await _settle(6)
		await _save_frame("anchor_yield", "anchor", false)
		lab.set_active_anchor(null)
	lab.trigger_correction_pulse()
	await _settle(6)
	await _save_frame("anchor_yield", "yield_correction", false)
	lab.free()
	await process_frame
...
func _save_frame(surface: String, mode: String, grayscale: bool) -> void:
	...
	if error != OK:
		...
		return
	_rows.append("%s\t%s\t%s\t%s\t%d\t%d\t%s\t%s\t640x360,pixel-grid capture\tHEURISTIC-PASS: hierarchy,silhouette,focal-point,material,depth,rhythm,lighting,palette,identity,clutter reviewed in montages" % [
		surface, mode, relative_path.trim_prefix("res://"), DisplayServer.get_name(),
		image.get_width(), image.get_height(), str(grayscale).to_lower(), FileAccess.get_md5(absolute_path),
	])
