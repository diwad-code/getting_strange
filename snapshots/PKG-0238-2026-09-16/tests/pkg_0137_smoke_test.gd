...
func _check_station(id: String) -> void:
	...
	if airlock and player:
		...
		# PKG-0232 (D-244, D4, kontrolowana aktualizacja): epilog 43 wymaga
		# pełnego dialogu (5 linii) przed blackoutem; przełącznik booli nie
		# stawia licznika int. Warunek spełniony = tablica + napisy + ostatnia
		# linia widoczna.
		if id == "station_43":
			var lines: Variant = st.get("dialogue_lines")
			if lines is Array and not (lines as Array).is_empty():
				st.set("dialogue_index", (lines as Array).size() - 1)
	...
	if id != "station_01":
		...
		if ret is ReturnZone and player:
			...
			for _i in 8:
				await physics_frame
			# PKG-0221 (D-234): sam overlap milczy; powrot idzie interactem.
			_expect(went_back.is_empty(), "%s ReturnZone overlap alone must not go back" % id)
			(ret as ReturnZone).trigger_return(player)
			_expect(not went_back.is_empty(), "%s ReturnZone must go back via interact" % id)
