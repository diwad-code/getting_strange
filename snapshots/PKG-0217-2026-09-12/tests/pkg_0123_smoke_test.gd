...
func _test_scene_node_stacks() -> void:
	...
	for path in SCENE_PATHS:
		...
		_expect(instance.get_node_or_null("Camera") != null, "%s missing Camera" % path)
...
func _test_interaction_flows() -> void:
	...
	root.add_child(s38)
	await process_frame
	s38.inspect_radio_receiver()
	s38.inspect_local_procedure_record()
	s38.inspect_jakub_switchboard()
	s38.anchor_rescue_tether()
	s38.disclose_marta_truth(&"full")
	_expect(s38.is_exit_unlocked, "Station 38 exit must unlock after diagnostic disclosure")
	s38.queue_free()
	await process_frame
