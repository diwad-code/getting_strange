...
## PKG-0166 normal-driver evidence for Station 18.
## Three textless M2 state frames (neutral / forecasts compared / marta partial)
## and one true-grayscale M3 structure frame after method_committed on the
## limited-consent path. This is structural evidence only; it does not claim
## human comprehension, emotion, comfort or PRODUCT GO.

const PACKAGE_ID := "PKG-0166"
const LOGICAL_SIZE := Vector2i(640, 360)
const OUTPUT_ROOT := "res://reports/pkg_0166"
const REPORT_PATH := "res://reports/pkg_0166/visual_evidence_report.txt"
const WAIT_FRAMES := 20

const CASES: Array[Dictionary] = [
	{"id": "neutral", "mono": false},
	{"id": "forecasts_compared", "mono": false},
	{"id": "marta_partial", "mono": false},
...
func _capture_one(capture: Dictionary, state: Node) -> void:
	...
	if packed == null:
		_failures.append("cannot load station_18")
		return
	var station := packed.instantiate() as CanvasItem
	if station == null:
		_failures.append("station_18 root is not CanvasItem")
		return
	root.add_child(station)
	for _frame in range(WAIT_FRAMES):
		await physics_frame
	if not _apply_state(station, String(capture["id"])):
		_failures.append("cannot apply station_18/%s" % String(capture["id"]))
	else:
...
func _save_frame(image: Image, capture: Dictionary, hidden_items: int) -> void:
	...
	var relative_path := OUTPUT_ROOT.path_join("station_18_%s.png" % String(capture["id"]))
...
func _apply_state(station: CanvasItem, state_id: String) -> bool:
	match state_id:
		"neutral":
			return true
		"forecasts_compared":
			return bool(station.call("compare_forecast_consent_dependencies"))
		"marta_partial":
			return bool(station.call("compare_forecast_consent_dependencies")) \
				and bool(station.call("disclose_marta_truth_partial"))
		"structure_mono":
			return bool(station.call("compare_forecast_consent_dependencies")) \
				and bool(station.call("disclose_marta_truth_partial")) \
				and bool(station.call("commit_close_equal"))
	return false
...
func _write_report() -> void:
	...
	var lines := PackedStringArray([
		"PKG-0166 VISUAL EVIDENCE REPORT",
		"Renderer: %s" % RenderingServer.get_video_adapter_name(),
		"Viewport: 640x360; normal Windows display driver; script was not headless.",
		"Three textless M2 frames of Station 18: neutral, forecasts compared, Marta partial truth.",
		"One M3 true-grayscale structure frame after method_committed on the limited-consent path.",
