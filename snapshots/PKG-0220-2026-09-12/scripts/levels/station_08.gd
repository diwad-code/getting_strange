...
## PKG-0198 (ZERO wycinek 2, D-215): słyszalne źródło klatki w kadrze.
## Kinkiety (y=130) mają własny szum — warstwa czysto wizualno-dźwiękowa:
## zero writerów, flag i sygnałów gry.
var _stairwell_hum: AudioStreamPlayer2D = null
...
func _ready() -> void:
	...
	_connect_npc_rig()
	_setup_stairwell_hum()
	queue_redraw()


## PKG-0198: słyszalne źródło klatki w kadrze (między kinkietami).
## Szum kinkietów gra od wejścia, niezależnie od czynności gracza.
func _setup_stairwell_hum() -> void:
	if has_node("StairwellWorkHum"):
		_stairwell_hum = get_node("StairwellWorkHum") as AudioStreamPlayer2D
		return
	_stairwell_hum = AudioStreamPlayer2D.new()
	_stairwell_hum.name = "StairwellWorkHum"
	_stairwell_hum.position = Vector2(320.0, 130.0)
	_stairwell_hum.volume_db = -28.0
	_stairwell_hum.stream = ProceduralAudio.get_cached_sound(&"s08_stairwell_hum", ProceduralAudio.create_act1_fluorescent_ballast_hum_sound)
	add_child(_stairwell_hum)
	_stairwell_hum.play()


## PKG-0197 (ZERO REWIZJA, D-214): sąsiadka odpowiada ciałem przy swojej
## domowej czynności, nie stoi jak słup. Wcześniej rig `neighbour` tkwił
## w `idle` przez całą stację, choć ma klatki talk_0/talk_1. Sterownik
## wizualny (zero writerów/flag/sygnałów gry): linia sąsiadki → talk,
...
func _npc_rig() -> CharacterVisualRig:
	var rig := get_node_or_null("LandingBlockout/Neighbour") as CharacterVisualRig
	if rig != null:
		return rig
...
func _on_npc_dialogue_line(speaker: StringName, _text: String) -> void:
	...
	if s.contains("siad") or s == "neighbour":
		rig.set_state(&"talk")
	else:
...
func speak_with_neighbour() -> bool:
	...
	# CR-D (PKG-0196 §6): sąsiadka nie ogłasza — odpowiada na pytanie
	# kontrolne zadane wprost, przy swojej domowej czynności.
	_present([
		{"speaker": "LENA", "text": "Dobry wieczór. Kto mieszka pod dwunastką?"},
		{"speaker": "SĄSIADKA", "text": "Pan Kowalczyk, od lat. A pani z Martą pod czternastką — ona wróciła godzinę temu."},
		{"speaker": "LENA", "text": "Dziękuję pani."},
	])
	queue_redraw()
	return true
...
func unlock_apartment_fourteen() -> bool:
	...
	# CR-D (PKG-0196 §6): po kluczu cisza — bez długiego opisu zapadki.
	_present([
		{"speaker": "LENA", "text": "Klucz działa. Wchodzę bez słowa."},
	])
	_unlock_exit()
	queue_redraw()
	return true
...
func _draw() -> void:
	...
	# PKG-0197: cień kontaktowy biegu schodów i donicy — kinkiety na y=130
	# są nazwanym źródłem, cień kładzie się w prawo i w dół, jak w 01/06.
	draw_colored_polygon(PackedVector2Array([
		Vector2(224.0, 294.0), Vector2(452.0, 290.0),
		Vector2(458.0, 297.0), Vector2(230.0, 301.0),
	]), Color(VectorStageStyle.INK, 0.48))
	# Potted fern planter on landing
	# PKG-0198 (ZERO wycinek 2): donica ma pracę — po rozmowie z sąsiadką
	# (domowa czynność przy donicy) paprotka prostuje się lekko w górę,
	# zamiast być wieczną kropką cyjanu. Stan czytelny bryłą, nie kolorem.
	var fern_lift := 2.0 if is_neighbour_spoken_to else 0.0
	var pot_color := VectorStageStyle.ANCHOR_CYAN if is_door_twelve_inspected else VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.50)
	draw_rect(Rect2(366.0, 256.0, 14.0, 16.0), pot_color)
	draw_circle(Vector2(373.0, 252.0 - fern_lift), 6.0, VectorStageStyle.ANCHOR_CYAN)
