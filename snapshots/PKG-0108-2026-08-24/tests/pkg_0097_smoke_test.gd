...
func _run() -> void:
	...
	if state:
		...
		_expect(
			...
			"delivered campaign transition limit must be 25"
		)
		...
		_expect(
			not state.has_reached_station(&"station_26"),
			"station 25 must not unlock the unconverted next batch (limit 25)"
		)
		_expect(
			state.get_next_campaign_station(&"station_25").is_empty(),
			"station 25 must report no delivered successor"
		)
	...
	for expected_number in range(21, 26):
		...
		if state:
			...
			if expected_number < 25:
				...
				_expect(
					not state.has_reached_station(next_id),
					"Station 25 completion must stop at the delivered limit"
				)
		station.queue_free()
		await process_frame
