...
func _build_audio_soundscape() -> void:
	...
	if station_number >= 31 and station_number <= 41:
		...
		_substructure_player.stream = ProceduralAudio.create_cooling_chamber_drone_sound()
	...
	_unease_player.stream = ProceduralAudio.create_unease_tinnitus_sound()
...
func _select_primary_soundscape() -> AudioStreamWAV:
	if station_number <= 7:
		return ProceduralAudio.create_fluorescent_hum_sound()
	elif station_number <= 13:
		return ProceduralAudio.create_residential_ambience_sound()
	elif station_number <= 23:
		return ProceduralAudio.create_terminal_hum_sound()
	elif station_number <= 30:
		return ProceduralAudio.create_tunnel_rumble_sound()
	elif station_number <= 37:
		return ProceduralAudio.create_substructure_ambient_sound()
	elif station_number <= 39:
		return ProceduralAudio.create_high_voltage_hum_sound()
	elif station_number <= 41:
		return ProceduralAudio.create_hydraulic_echo_sound()
	elif station_number == 42:
		var parent_node := get_parent()
		if parent_node != null:
			var pname := parent_node.name.to_lower()
			if "42a" in pname:
				return ProceduralAudio.create_finale_42a_forced_return_sound()
			elif "42b" in pname:
				return ProceduralAudio.create_finale_42b_closure_sound()
			elif "42c" in pname:
				return ProceduralAudio.create_finale_42c_reciprocal_passage_sound()
		return ProceduralAudio.create_finale_42a_forced_return_sound()
	else:
		return ProceduralAudio.create_dawn_quietude_sound()
