...
func _physics_process(_delta: float) -> void:
	if is_instance_valid(player) and is_instance_valid(hallway_sideboard) and not hallway_sideboard.is_anchored:
		...
		var vertical_overlap := absf(diff.y) < (hallway_sideboard.crate_size.y * 0.5 + 40.0)
		if horizontal_dist < 46.0 and vertical_overlap:
			var push_dir := signf(player.velocity.x)
			if push_dir > 0.0 and signf(diff.x) == push_dir:
				hallway_sideboard.receive_push(push_dir)
