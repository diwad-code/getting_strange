...
func _test_station_01_slice() -> void:
	...
	var sensor = props.get_node_or_null("VibrationSensor") if props else null
	var bag = props.get_node_or_null("PackingBag") if props else null
	
	_expect(sensor != null, "VibrationSensor musi istnieć w Props")
	_expect(bag != null, "PackingBag musi istnieć w Props")
	
	if sensor and bag:
		# First reading -> interrupted
		sensor.trigger_interaction()
		_expect(station.get("sensor_checked") == true, "Pierwszy odczyt musi zostać zarejestrowany")
		_expect(station.get("second_reading_done") == false, "Drugi odczyt nie może być wykonany od razu")
		
		# Second reading -> clean
		sensor.trigger_interaction()
		_expect(station.get("second_reading_done") == true, "Drugi odczyt musi zostać zarejestrowany")
		
		# Pack gear -> completes procedure & unlocks door
		bag.trigger_interaction()
		_expect(station.get("gear_packed") == true, "Spakowanie sprzętu musi nastąpić po drugim odczycie")
		_expect(station.get("is_procedure_completed") == true, "Procedura stacji 01 musi być ukończona")
	...
	for i in range(6):
		await physics_frame
	if not station.get("is_level_completed"):
		station.call(&"_on_airlock_body_entered", player)
	_expect(station.get("is_level_completed") == true, "Po wejściu w strefę wyjścia poziom 01 musi być ukończony")
...
func _test_station_05_slice() -> void:
	...
	var player = station.get_node_or_null("Player")
	if player:
		player.global_position = Vector2(1230.0, 238.0)
	for i in range(6):
		await physics_frame
	if not station.get("is_level_completed"):
		station.call(&"_on_airlock_body_entered", player)
	_expect(station.get("is_level_completed") == true, "Station 05 musi zakończyć się po wejściu do śluzy")
	
	var game_state := root.get_node_or_null("GameStateManager")
	if game_state and game_state.has_method("get_campaign_flag"):
		_expect(game_state.get_campaign_flag(&"ordinary_return_complete", false) == true, "Station 05 musi ustawić flagę ordinary_return_complete")
...
func _test_station_06_slice() -> void:
	...
	station.set("are_doors_open", true)
	var player = station.get_node_or_null("Player")
	if player:
	...
	if not station.get("is_level_completed"):
		station.call(&"_on_airlock_body_entered", player)
	_expect(station.get("is_level_completed") == true, "Station 06 musi zakończyć się po wejściu w otwarte drzwi")
	
	var game_state := root.get_node_or_null("GameStateManager")
	if game_state and game_state.has_method("get_campaign_flag"):
		_expect(game_state.get_campaign_flag(&"unease_pattern_started", false) == true, "Station 06 musi ustawić flagę unease_pattern_started")
