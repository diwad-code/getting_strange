...
- logiczny viewport 640x360, integer scaling i nearest filtering dla UI/bitmap;
  renderowane proceduralnie płaszczyzny `Vector-Stage` pozostają niezależne od
  techniki pixel artu;
- fizyka 60 Hz;
...
- Krita/Inkscape lub własne procedury `CanvasItem._draw()` dla finalnych
  płaszczyzn wektorowo-wielokątnych; Aseprite wyłącznie dla bitmapowego UI lub
  drobnych masek, nie jako kanoniczna technika świata;
...
- każda generacja użyta poza eksperymentem zapisuje: pełny prompt, model,
  ...
  dopóki nie przejdą ręcznej adaptacji do Rówień Vector-Stage;
...
### Kontrakt Rówień Vector-Stage

- warstwa prezentacji nie zmienia `CollisionShape2D`, fizyki ani zasięgu
  interakcji;
- postać ma maksymalnie 9 widocznych płaszczyzn sylwetki w pozie bazowej;
- każdy kadr używa palety zdefiniowanej w `VectorStageStyle` oraz najwyżej
  jednego akcentu stanu (cyjan **lub** cynober) poza akcentem ludzkim;
- duże płaszczyzny, czytelne krawędzie i negatywna przestrzeń mają pierwszeństwo
  przed teksturą, szumem, gradientowym „plastikiem” i gęstą siatką paneli;
- wszystkie przebudowane stacje instancjonują `VectorStageEnvironment` i są
  objęte smoke testem oraz renderem kontrolnym.
