...
func _handle_player_anchor_toggle() -> void:
	...
	for child in anchorables.get_children():
		if child is AnchorableObject:
			...
			var prop := child as MovableAnchorableProp
			var dist := prop.global_position.distance_to(player.global_position)
	...
	if anchorables:
		for child in anchorables.get_children():
			if child is AnchorableObject:
				...
				var prop := child as MovableAnchorableProp
				prop.is_anchored = false
				prop.velocity = Vector2.ZERO
				# Restore spawn position stored in metadata if present
				if prop.has_meta(&"spawn_position"):
					prop.global_position = prop.get_meta(&"spawn_position")
