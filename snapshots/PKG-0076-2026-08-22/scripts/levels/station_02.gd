...
const COLOR_AMBER := Color("d39a62") # Muted amber (Lena / life)
const COLOR_CYAN := Color("75c7c3") # Cool cyan (Vacuum / correlation)
const COLOR_CORRECTION := Color("c65d58") # Oxide cinnabar
const COLOR_DARK_STEEL := Color("263943")
const COLOR_FLOOR := Color("1b272e")
...
func _draw() -> void:
	# 1. Architectural structure and wall panels
	_draw_architecture()
	
	# 2. Symmetrical correlation containment chamber & optical axis
	_draw_correlation_chamber()
	
	# 3. Telemetry panel & strip chart printer (WYNIK ZGODNY)
	_draw_telemetry_and_printer()
	
	# 4. Work lighting fixtures
	_draw_lighting_fixtures()
	
	# 5. Exit airlock frame
	_draw_airlock_frame()
