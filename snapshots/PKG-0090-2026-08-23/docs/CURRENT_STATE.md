...
- **Wersjonowanie: brak (D-016).** Pliki na dysku sa jedynym stanem projektu.
  Nie ma repozytorium, galezi, commita ani historii. Nie uruchamiamy `git`. Ostatni zamkniety pakiet: `PKG-0090`, 2026-08-23
...
- Zamrozenia: `snapshots/PKG-NNNN-DATA/` przez `tools/snapshot.ps1` (D-017);
  ostatnie: `snapshots/PKG-0090-2026-08-23`
...
Faza **`P4: Produkcja i Szlif Gry (Pure Godot Game Focus, D-089) + Priorytet Ukończenia Portalu Web (D-090)`**.
Zgodnie z decyzją D-090 (2026-08-23, polecenie właściciela projektu), plan wdrożenia zostaje przekierowany na ukończenie gry przez dokończenie strony: Portal Web (`web/`) jest równoprawnym torem produkcyjnym, a **wszystkie funkcje zadeklarowane przez stronę muszą działać**. W PKG-0089 naprawiono krytyczny błąd ładujący `app.js` (ReferenceError w bloku eksportów `window.*`), który dezaktywował 23 klasy silników i 341 eksportów; wdrożono czytnik 100 akt, kontroler projektanta sygnałów i szybkie komendy CLI; bramka `verify_web.ps1` objęła audyty martwych kontrolek/eksportów oraz runtime smoke test w Node. W PKG-0090 wdrożono pełną pętlę progresji 20 komór symulatora Canvas 2D (zapis stanu w `localStorage`, odblokowania, baner ukończenia kampanii, naprawa błędu `% 15` i telemetrii HUD `XX/20`) oraz zsynchronizowano telemetrię portalu (CLI `status`, manifesty, stopka, cache PWA v2). Równolegle obowiązuje D-089 — szlif gry w silniku Godot 4.7:
...
Potwierdzony wynik finalnej weryfikacji PKG-0090:
...
`PKG-0091: Pełna Dostępność Offline PWA, Rozszerzony Runtime E2E Kluczowych Ścieżek i Audyt Dostępności Portalu Web`

Cel: Kontynuacja priorytetu D-090 — strona kompletna również bez sieci i pod kontrolą testów behawioralnych:
1. Rozszerzenie cache service workera o komplet 92 kadrów `assets/reports` i krytyczne assety, tak aby Portal Web działał w 100% offline po pierwszej wizycie;
2. Rozszerzenie `tools/web_runtime_smoke.js` o fazę behawioralną: symulacja kliknięć kluczowych ścieżek (czytnik akt `openDossierModal('doc1')`, projektant sygnałów, komendy CLI `help`/`status`/`dossier 5`, reset progresji) w harnessie DOM;
3. Audyt dostępności i responsywności (focus states, kontrast, aria-labels przycisków dotykowych, media queries) z poprawkami w `style.css`;
4. Synchronizacja telemetrii portalu ze stanem PKG-0091.

Pakiet jest realizowany w 100% autonomicznie w trybie Wysokoprzepustowych Mega-Pakietów (2x–5x Batch Size, D-085, D-090).

## Punkt przekazania

Nowa sesja zaczyna od swiezej weryfikacji (`tools/verify.ps1`), czyta `AGENTS.md`, `INDEX.md`, ten
plik i `NEXT_SESSION_PROMPT.md`, a nastepnie przechodzi do realizacji PKG-0091 (Portal Web, D-090) z równoległym torem szlifu gry Godot 4.7 (D-089).
