...
##    - 15: notatki terenowe, dwa konkrety wyprawy, obserwacja zabezpieczonego telefonu, prośba o własny zapis pracy (S06)
##    - 16: biometryka, obcy numer karty, historia aktywności, próba instytucjonalna przy kołowrocie (S06)
##    - 17: raport incydentu, sygnatura vs karta, interkom jako obserwacja, rygiel wentylacji, minimalny zakres kopiowania (S06)
##    - 18: rejestr miejski, szpitalny, karta pracownicza, numer sprawy, próba publiczna na mikrofiszach (S07)
...
const REQUIRED_HYPOTHESES := {
	14: "memory_gap",
	15: "memory_manipulation",
	16: "falsified_record",
	17: "consistent_foreign_biography",
	18: "error_copy",
	19: "impostor",
	20: "impostor",
...
func _test_station_15() -> void:
	...
	if packed == null:
		...
		return
	var state := root.get_node_or_null("GameStateManager")
	_reset_with(state, {"p7.marta_threshold.trace": "relationship_difference_explicit"})
	var station := packed.instantiate() as Station15
	root.add_child(station)
	await process_frame
	_expect(not station.request_own_work_record(), "Prośba o zapis pracy bez dwóch konkretów musi pozostać bezpieczna")
	if state:
		_expect(
			state.decisions.get(&"p7.work_history_and_record.safe_trial_feedback", "") == "expedition_details_incomplete",
			"Błędna bezpieczna próba musi zostawić fakt"
		)
	_expect(station.observe_expedition_notes(), "Notatki terenowe muszą się otworzyć")
	_expect(station.compare_rain_detail(), "Konkret deszczu musi zostać porównany")
	_expect(station.compare_fence_detail(), "Konkret ogrodzenia musi zostać porównany")
	_expect(station.observe_secured_phone(), "Zabezpieczenie telefonu przez Martę musi zostać zaobserwowane")
	_expect(station.request_own_work_record(), "Prośba o własny zapis pracy musi się udać")
	if state:
		_expect(state.decisions.get(&"marta_memories_conflict", false) == true, "Konflikt wspomnień musi powstać po wykonanym porównaniu")
	_expect(station.get("is_exit_unlocked") == true, "Wyjście musi się otworzyć po prośbie")
	_expect(not station.is_level_completed, "Prośba sama nie kończy sceny")
	station.queue_free()
	for frame in range(3):
		await process_frame
...
func _test_station_16() -> void:
	...
	root.add_child(station)
	await process_frame
	_expect(not station.compare_field_biometrics_and_audit(), "Próba bez trzech odczytów musi pozostać bezpieczna")
	_expect(station.scan_biometric_profile(), "Profil biometryczny musi zostać rozpoznany")
	_expect(station.compare_card_number(), "Numer karty musi zostać porównany")
	_expect(station.read_activity_history(), "Historia aktywności musi zostać odczytana")
	_expect(station.compare_field_biometrics_and_audit(), "Próba instytucjonalna musi rozstrzygnąć")
	if state:
		_expect(state.decisions.get(&"local_lena_ucp_profile_found", false) == true, "Profil UCP-4 musi powstać po wykonanej próbie")
		_expect(not state.decisions.has(&"mechanic_cost_observed"), "Station 16 nie może ustawić kosztu metody")
	_expect(station.get("is_exit_unlocked") == true, "Kołowrót musi zwolnić wyjście po próbie")
...
func _test_station_17() -> void:
	print("TEST: Station 17 (Zakres kopiowania raportu)...")
	var packed := load("res://scenes/levels/station_17.tscn") as PackedScene
	if packed == null:
		_expect(false, "station_17.tscn musi istnieć")
		return
	var state := root.get_node_or_null("GameStateManager")
	_reset_with(state, {"p7.work_history_and_record.institution_trial_result": "persistent_chronology_confirmed"})
	var station := packed.instantiate() as Station17
	root.add_child(station)
	await process_frame
	_expect(station.read_incident_report(), "Raport incydentu musi zostać otwarty")
	_expect(station.compare_signature_with_card(), "Sygnatura musi zostać porównana z kartą")
	_expect(station.listen_intercom_warning(), "Interkom musi zostać wysłuchany jako obserwacja")
	_expect(station.release_service_route(), "Rygiel wentylacji musi zwolnić drogę serwisową")
	_expect(station.copy_report_header(), "Nagłówek raportu musi zostać skopiowany w minimalnym zakresie")
	if state:
		_expect(state.decisions.get(&"p7.work_history_and_record.report_scope", "") == "minimum_for_test", "Zakres kopiowania musi być jawny")
		_expect(state.decisions.get(&"parallel_test_trace_found", false) == true, "Ślad równoległej próby musi powstać po działaniu")
		_expect(
			state.decisions.get(&"p7.work_history_and_record.trace", "") == "institutional_source_and_document_conflict",
			"S06 musi zostawić źródło instytucjonalne i sprzeczność dokumentów"
		)
	_expect(station.get("is_exit_unlocked") == true, "Właz serwisowy musi otworzyć wyjście")
	_expect(not station.is_level_completed, "Kopiowanie samo nie kończy sceny")
	station.queue_free()
	for frame in range(3):
		await process_frame
...
func _test_station_18() -> void:
	...
	if packed == null:
		...
		return
	var state := root.get_node_or_null("GameStateManager")
	_reset_with(state, {"p7.work_history_and_record.trace": "institutional_source_and_document_conflict"})
	var station := packed.instantiate() as Station18
	root.add_child(station)
	await process_frame
	_expect(not station.compare_independent_registries(), "Porównanie bez trzech rejestrów musi pozostać bezpieczne")
	if state:
		_expect(
			state.decisions.get(&"p7.three_place_proofs.safe_trial_feedback", "") == "registry_sources_incomplete",
			"Błędna bezpieczna próba musi zostawić fakt"
		)
	_expect(station.search_municipal_death_registry(), "Rejestr miejski musi zostać sprawdzony")
	_expect(station.search_hospital_registry(), "Rejestr szpitalny musi zostać sprawdzony")
	_expect(station.verify_employment_card(), "Karta pracownicza musi zostać potwierdzona")
	_expect(station.read_disaster_case_file(), "Numer sprawy katastrofy musi zostać odczytany")
	_expect(station.compare_independent_registries(), "Próba publiczna musi rozstrzygnąć")
	if state:
		_expect(state.decisions.get(&"jakub_public_history_verified", false) == true, "Publiczna historia Jakuba musi powstać po próbie")
		_expect(state.decisions.get(&"recognition_evidence_public", false) == true, "Dowód publiczny musi zostać utrwalony")
	_expect(station.get("is_exit_unlocked") == true, "Wyjście musi się otworzyć po próbie publicznej")
	_expect(not station.is_level_completed, "Próba sama nie kończy sceny")
	station.queue_free()
	for frame in range(3):
		await process_frame
