...
## Station 12 — P9, dowod osobowy: warsztat Jakuba (S-13, PKG-0230).
## Lacze przy wejsciu, czlowiek przy imadle: Lena zadaje dwa pytania
## kontrolne, spotyka go i przyjmuje odmowe pokazania blizny
## (jakub_questions → jakub_meeting → jakub_refusal).
## Warstwa dawcy P7 (balkon, sekretarka) jest nieaktywna w trasie, ale
## callable dla testow — patrz werby nizej.

## PRZESZKODA — dlaczego to tu jest: Warsztat oddaje naped przed koncem zmiany, wiec Jakub rozmawia w pracy, nie po niej.
## PRZESZKODA — czego wymaga od Leny: pytania przez lacze, podejscia do stolu i przyjecia odmowy bez nacisku.
## PRZESZKODA — koszt porażki: nacisk zamiast przyjecia zostawia luke w GapLedger, nie blokade.

## PKG-0192 (D-205 §4) — jawna decyzja dla `BalconyDoor`: ZOSTAJE jako
## wezel w drzewie, nie jest blocking_body progu ani cialem na prawej
## krawedzi (PKG-0234 A3). Wyjscie warsztatu to drzwi serwisowe (Binder
## door="", rodzina DOOR). Metody
...
const BALCONY_OPEN_X := 72.0
const BALCONY_CLOSED_X := 156.0
...
func _setup_guidance() -> void:
	...
	_register_beat(&"s12_voice_contact", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Przy wejściu jest łącze do warsztatu.", "There is a workshop link by the entrance.", &"", "")
	_register_beat(&"s12_staging_hypothesis", GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT, &"interpretation", &"fallible", "To może być nagranie. Zapytam o coś spoza rejestru.", "It could be a recording. I will ask something outside the record.", &"staging", "ask_jakub_control_questions")
	_register_beat(&"s12_verify_voice", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Zapytam przez łącze, potem podejdę do stołu.", "I will ask over the link, then approach the bench.", &"", "")
	# PKG-0233 (B3, S-09): czytnik niesiony na pokwitowaniu z UCP — status
	# proceduralny z 11 pamietany w decyzji i widoczny przy pierwszym pytaniu.
	_register_beat(&"s12_reader_receipt", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Czytnik mam na pokwitowaniu z UCP. Pokażę go, nie oddam.", "The reader travels on a UCP receipt. I will show it, not surrender it.", &"", "")
	# PKG-0230 (P1-1+P1-4): zdanie wyjscia — 12 → 13 to powrot do mieszkania,
	# do wspolnego stolu (wejscie z prawej, P2-1).
	_register_beat(&"s12_exit_back_home", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Wracam do mieszkania, do wspólnego stołu. Ułożę trzy źródła obok siebie.", "Back to the flat, to the shared table. I lay the three sources side by side.", &"", "")
	_register_beat(&"s12_system_hint", GuidanceBeat.Tier.L4_RESCUE_HINT, &"system_hint", &"system", "WSKAZÓWKA: Łącze, spotkanie przy imadle, czytnik do sprawdzenia.", "HINT: Link, meeting at the vice, reader inspection.", &"", "")
...
func _draw() -> void:
	...
	# PKG-0233 (B3, S-09): czytnik na stole z pokwitowaniem UCP — ten sam
	# rekwizyt, ktory 11 wydal warunkowo, a Jakub sprawdzi w 12.
	if _has(&"p9.mystery.institution.trace"):
		draw_rect(Rect2(388.0, 240.0, 26.0, 10.0), Color("d3d0c5"))
		draw_rect(Rect2(388.0, 250.0, 26.0, 6.0), Color("8b9a94"))
		draw_line(Vector2(392.0, 253.0), Vector2(410.0, 253.0), Color("1b2529"), 1.0)
	...
	# PKG-0234 (A3): drzwi serwisowe warsztatu na prawej krawedzi — rodzina
	# DOOR, nie skrzydlo balkonu. Binder nie wskazuje BalconyDoor.
	draw_rect(Rect2(536.0, 184.0, 54.0, 112.0), Color("3b4d54"))
	draw_rect(Rect2(542.0, 192.0, 42.0, 96.0), Color("1d2b37"))
	draw_rect(Rect2(554.0, 236.0, 18.0, 8.0), Color("d39a62"))
	draw_rect(Rect2(0.0, 304.0, 640.0, 56.0), Color("1b2529"))
	draw_line(Vector2(0.0, 304.0), Vector2(640.0, 304.0), Color("82949a"), 2.0)
	# PKG-0198 (ZERO wycinek 2): cień kontaktowy warsztatu w prawo, 0.48,
	# zgodnie z lampą stanowiska i konwencją 01/06/08.
	draw_colored_polygon(PackedVector2Array([
		Vector2(56.0, 300.0), Vector2(452.0, 296.0),
		Vector2(460.0, 304.0), Vector2(64.0, 308.0),
...
func ask_jakub_control_questions() -> bool:
	...
	# PKG-0233 (B3, S-09): pierwsze pytanie przez lacze odbywa sie z czytnikiem
	# na pokwitowaniu — jawny status, nie domyslne posiadanie.
	if guidance_service:
		guidance_service.trigger_beat(&"s12_reader_receipt")
	queue_redraw()
	return true
...
func accept_jakub_refusal() -> bool:
	...
	if guidance_service:
		guidance_service.trigger_beat(&"s12_exit_back_home")
	_unlock_exit()
	queue_redraw()
	return true
