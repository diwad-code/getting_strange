...
func _test_opening_lines_match_live_stations() -> void:
	...
	# PKG-0193: old tests described dormant P7 helpers. CR-A protects the live
	# three-point P9 scene and also rejects those obsolete opening directions.
	_expect(
		line_11.to_lower().contains("czytnik kart") and line_11.contains("Wierzbicka") and not line_11.to_lower().contains("prywatn"),
		"station_11 opening must introduce card admission at Wierzbicka's counter (got: %s)" % line_11
	)
	_expect(
		line_12.contains("łącze") and line_12.contains("imadle") and not line_12.to_lower().contains("balkon") and not line_12.to_lower().contains("sekretark"),
		"station_12 opening must introduce a workshop link and a working person, never dormant answering-machine tasks"
	)
	_expect(not line_13.contains("w terenie"), "station_13 must not keep the leftover field-companion line")
	_expect(
		line_13.contains("Marta") and line_13.contains("torbę") and line_13.contains("stole") and not line_13.to_lower().contains("szuflad"),
		"station_13 opening must stage Marta and the carried material at the shared table (got: %s)" % line_13
	)
	var required_points := {
	...
	for number in required_points:
		var scene_source := FileAccess.get_file_as_string("res://scenes/levels/station_%s.tscn" % number)
		for point_id in required_points[number]:
			_expect(scene_source.contains('resonance_id = "%s"' % point_id), "opening must have its actual P9 source point: " + point_id)
