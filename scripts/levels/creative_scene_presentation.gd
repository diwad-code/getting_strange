...
func _on_line(speaker: StringName, _text: String) -> void:
	...
	for rig in _rigs:
		...
		if rig.character_id == &"jakub" and _active_id == "jakub_questions":
			...
			# PKG-0230 (P1-3, S-07): didaskalia z dopiskiem ("Marta domowa",
			# "JAKUB (ŁĄCZE)") naleza do tej samej postaci co czysty identyfikator.
			var speaker_lower := String(speaker).to_lower()
			rig.set_state(&"talk" if speaker_lower.begins_with(String(rig.character_id)) else &"listen")
