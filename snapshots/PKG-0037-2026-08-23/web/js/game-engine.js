/**
 * GETTING STRANGE — Movement & Anchor HTML5 Canvas Mini-Engine (20 Chambers)
 * Interaktywny symulator fizyki (640x360 logiczne) z profilami ruchu A/B/C, 20 komorami testowymi,
 * pętlą progresji z zapisem stanu w localStorage (PKG-0090), systemem cząsteczek,
 * wstrząsem ekranu (screen-shake) i bezpośrednią integracją WebProceduralAudio.
 */
...
class GettingStrangeMiniEngine {
  constructor(canvasId) {
    ...
    // Progression loop state (PKG-0090): 20 chambers, persistent unlocks
    this.totalChambers = 20;
    this.completionBannerTimer = 0;

    // Level geometry & anchorables
    this.loadChamber(1);

    // Restore persistent progression and resume from last reached chamber
    this.progress = this.loadProgress();
    if (this.progress.lastChamber > 1 && this.progress.lastChamber <= this.totalChambers) {
      this.loadChamber(this.progress.lastChamber);
    }
  ...
  /* ========================================================================
     PROGRESSION LOOP (PKG-0090) — persistent 20-chamber campaign state
     ======================================================================== */
  loadProgress() {
    const fallback = {
      completed: [],
    ...
    try {
      ...
      const raw = localStorage.getItem('getting_strange_progress_v1');
  ...
  saveProgress() {
    try {
      ...
      localStorage.setItem('getting_strange_progress_v1', JSON.stringify(this.progress));
    } catch (e) { /* storage unavailable — progression stays in-memory */ }
  }
  ...
  resetProgress() {
    ...
    this.completionBannerTimer = 0;
    this.loadChamber(1);
    this.syncChamberToolbar(1);
    if (window.proceduralAudio) window.proceduralAudio.playUnanchorSound();
    if (window.showToast) {
      const lang = window.i18n ? window.i18n.currentLang : 'pl';
      window.showToast(lang === 'en'
        ? 'Chamber progress reset (01/20)'
        : 'Zresetowano postęp komór (01/20)');
    }
  }

  syncChamberToolbar(chamberIndex) {
    ...
    chamberButtons.forEach((b, idx) => {
      b.classList.toggle('active', idx + 1 === chamberIndex);
    });
  }
  ...
  update(dt) {
    ...
    if (this.goal && !this.goal.isReached) {
      if (this.player.x + this.player.width > this.goal.x &&
        ...
        // Progression loop (PKG-0090): record completion, persist, advance
        const finishedChamber = this.currentChamber;
        if (this.progress) {
          if (!this.progress.completed.includes(finishedChamber)) {
            this.progress.completed.push(finishedChamber);
          }
          this.progress.unlocked = Math.max(
            this.progress.unlocked,
            Math.min(finishedChamber + 1, this.totalChambers)
          );
          if (finishedChamber >= this.totalChambers) {
            this.progress.runsFinished += 1;
          }
        }

        const campaignFinished = finishedChamber >= this.totalChambers;

        setTimeout(() => {
          if (campaignFinished) {
            // Full campaign completed: wrap to chamber 1 and show completion banner
            if (this.progress) {
              this.progress.lastChamber = 1;
              this.saveProgress();
            }
            this.completionBannerTimer = 8;
            this.loadChamber(1);
            this.syncChamberToolbar(1);
            if (window.showToast) {
              const lang = window.i18n ? window.i18n.currentLang : 'pl';
              window.showToast(lang === 'en'
                ? '◆ ALL 20 CHAMBERS COMPLETED — CONTINUITY RESTORED ◆'
                : '◆ UKOŃCZONO WSZYSTKIE 20 KOMÓR — CIĄGŁOŚĆ PRZYWRÓCONA ◆');
            }
          } else {
            const nextChamber = finishedChamber + 1;
            if (this.progress) {
              this.progress.lastChamber = nextChamber;
              this.saveProgress();
            }
            this.loadChamber(nextChamber);
            this.syncChamberToolbar(nextChamber);
          }
        }, 800);
      }
    }

    // Completion banner decay
    if (this.completionBannerTimer > 0) {
      this.completionBannerTimer = Math.max(0, this.completionBannerTimer - dt);
    }
  ...
  render() {
    ...
    // HUD Telemetry overlay in top-left (PKG-0090: full 20-chamber progression)
    ctx.fillStyle = 'rgba(5, 8, 12, 0.8)';
    ctx.fillRect(16, 12, 300, 42);
    ctx.strokeStyle = '#243a47';
    ctx.strokeRect(16, 12, 300, 42);

    const chamberLabel = String(this.currentChamber).padStart(2, '0');
    const completedCount = this.progress ? this.progress.completed.length : 0;
    const lang = window.i18n ? window.i18n.currentLang : 'pl';
    ctx.fillStyle = '#75c7c3';
    ctx.font = '10px monospace';
    ctx.textAlign = 'left';
    ctx.fillText(
      `KOMORA: ${chamberLabel}/${this.totalChambers} | ${lang === 'en' ? 'DONE' : 'UKOŃCZONE'}: ${completedCount}/${this.totalChambers} | PROFIL: ${this.currentProfile}`,
      24, 26
    );
    ...
    // Campaign completion banner (PKG-0090)
    if (this.completionBannerTimer > 0) {
      const alpha = Math.min(1, this.completionBannerTimer / 2);
      ctx.globalAlpha = alpha;
      ctx.fillStyle = 'rgba(3, 5, 8, 0.88)';
      ctx.fillRect(60, 130, this.logicalWidth - 120, 100);
      ctx.strokeStyle = '#5da398';
      ctx.lineWidth = 2;
      ctx.strokeRect(60, 130, this.logicalWidth - 120, 100);
      ctx.fillStyle = '#75c7c3';
      ctx.font = 'bold 13px monospace';
      ctx.textAlign = 'center';
      ctx.fillText(
        lang === 'en' ? '◆ ALL 20 CHAMBERS COMPLETED ◆' : '◆ UKOŃCZONO WSZYSTKIE 20 KOMÓR ◆',
        this.logicalWidth / 2, 165
      );
      ctx.fillStyle = '#d39a62';
      ctx.font = '10px monospace';
      ctx.fillText(
        lang === 'en'
          ? 'CONTINUITY RESTORED — THE SEAM HOLDS. CAMPAIGN RESTARTS AT CHAMBER 01.'
          : 'CIĄGŁOŚĆ PRZYWRÓCONA — SZEW TRWA. KAMPANIA WRACA DO KOMORY 01.',
        this.logicalWidth / 2, 188
      );
      ctx.fillStyle = '#6b8291';
      ctx.fillText(
        lang === 'en'
          ? `FINISHED RUNS: ${this.progress ? this.progress.runsFinished : 1}`
          : `UKOŃCZONE PRZEJŚCIA: ${this.progress ? this.progress.runsFinished : 1}`,
        this.logicalWidth / 2, 208
      );
      ctx.globalAlpha = 1.0;
    }
