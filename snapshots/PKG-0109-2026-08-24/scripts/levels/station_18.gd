...
func _draw_state_layer() -> void:
	...
	# The galvanometer records one narrow relation: the needle moves toward the
	# oxide side as the official answer diverges from Lena's memory.
	if is_galvanometer_triggered:
		var needle_offset := lerpf(-12.0, 12.0, clampf(_strain_level, 0.0, 1.0))
		var needle_color := VectorStageStyle.CORRECTION_OXIDE if _strain_level > 0.45 else VectorStageStyle.HUMAN_AMBER
		draw_line(Vector2(450.0, 202.0), Vector2(450.0 + needle_offset, 176.0), needle_color, 2.0)
		draw_circle(Vector2(450.0, 202.0), 3.0, VectorStageStyle.LIGHT_PLANE)

	# The conduit reports pressure at its own seam; it never washes the room in red.
	if is_strain_detected or _strain_level > 0.05:
		var strain_alpha := 0.28 + clampf(_strain_level, 0.0, 1.0) * 0.42
		draw_line(Vector2(160.0, 206.0), Vector2(205.0, 202.0), Color(VectorStageStyle.CORRECTION_OXIDE, strain_alpha), 2.0)

	if is_airlock_unlocked:
		draw_line(Vector2(548.0, 270.0), Vector2(608.0, 270.0), VectorStageStyle.ANCHOR_CYAN, 2.0)
