...
## techniczny tramwaj wykonuje własną pracę przewozową.
## PRZESZKODA — czego wymaga od Leny: obserwacji trzech wersji wypadku bez
## zamiany świadectwa w pościg albo wyzwanie skokowe.
## PRZESZKODA — koszt porażki: nie ma fizycznej korekty; rozbieżność widoków
## pozostaje świadomą ciszą sceny.
...
func _draw_state_layer() -> void:
	...
	var rail_color := VectorStageStyle.ANCHOR_CYAN if is_paradox_inspected else VectorStageStyle.MID_PLANE
	var carriage_y := 152.0 + sin(_tunnel_scroll * 0.012) * 2.0
	draw_line(Vector2(38.0, carriage_y), Vector2(602.0, carriage_y), rail_color, 1.5)
	for window_index in range(3):
		var x := 176.0 + float(window_index) * 112.0
		var version_color := VectorStageStyle.DEEP_PLANE
		if is_paradox_inspected:
			version_color = VectorStageStyle.ANCHOR_CYAN if window_index == 2 else VectorStageStyle.HUMAN_AMBER
		VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
			Vector2(x, 104.0), Vector2(x + 72.0, 104.0), Vector2(x + 62.0, 142.0), Vector2(x + 10.0, 142.0)
		]), version_color, 1.0)
	if is_exit_unlocked:
		draw_line(Vector2(530.0, 224.0), Vector2(604.0, 224.0), VectorStageStyle.HUMAN_AMBER, 2.0)
