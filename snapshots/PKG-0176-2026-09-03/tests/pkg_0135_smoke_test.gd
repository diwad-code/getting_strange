...
func _test_backtrack_05_04_03() -> void:
	...
	if p05:
		...
		for _i in 50:
			...
			p05.move_and_slide()
			await physics_frame
	_expect(not back_requested_s05.is_empty(), "Station 05 triggers previous_level_requested at ReturnZone")
	st05.queue_free()
	await process_frame
	...
	if p04:
		...
		while frames > 0:
			...
			await physics_frame
		# ReturnZone (28 x 220 @ x=18, D-124) fires as soon as the capsule edge
		# touches it, so the walk stops short of the zone centre by design.
		_expect(p04.global_position.x <= 60.0, "Station 04 player walked left to ReturnZone (at x=%.1f)" % p04.global_position.x)

	for _i in 5:
		await physics_frame
	_expect(not back_requested_s04.is_empty(), "Station 04 triggers previous_level_requested at ReturnZone")
	st04.queue_free()
	await process_frame
	...
	for _i in 5:
		await physics_frame
	_expect(not back_requested_s03.is_empty(), "Station 03 triggers previous_level_requested at ReturnZone")
	st03.queue_free()
	await process_frame
	if gsm_node:
