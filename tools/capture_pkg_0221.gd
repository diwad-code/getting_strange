...
## PKG-0221 — capture station_02/15/16 (ladder geometry fixes + Threshold-like
## return, zero paint changes) at 100% x full/notext (normal Windows driver;
## headless hangs on frame_post_draw, fact from PKG-0197).
## Output: 6 PNG 640x360 + frames.tsv under reports/pkg_0221/visual.
## No overwrites of 0187..0220 archives. Inspection: ladders stand on floors,
## protrude to landing/hatch, no visual regression vs archives.

const OUT_DIR := "res://reports/pkg_0221/visual"
const CORE_IDS: Array[String] = ["station_02", "station_15", "station_16"]
...
func _run() -> void:
	...
	for station_id: String in CORE_IDS:
		...
		if packed == null:
			push_error("CAPTURE 0221: cannot load %s" % station_id)
			continue
		var full_image := await _capture_variant(packed, false)
		if full_image != null:
			var full_name := "%s__pkg0221_s100_full.png" % station_id
			full_image.save_png(out_path.path_join(full_name))
			print("CAPTURE 0221 PASS: %s" % full_name)
			index.append("%s\t100\tfull\t%s" % [station_id, full_name])
		else:
			push_error("CAPTURE 0221: no full image %s" % station_id)
		var notext_image := await _capture_variant(packed, true)
		if notext_image != null:
			var notext_name := "%s__pkg0221_s100_notext.png" % station_id
			notext_image.save_png(out_path.path_join(notext_name))
			print("CAPTURE 0221 PASS: %s" % notext_name)
			index.append("%s\t100\tnotext\t%s" % [station_id, notext_name])
		else:
			push_error("CAPTURE 0221: no notext image %s" % station_id)
