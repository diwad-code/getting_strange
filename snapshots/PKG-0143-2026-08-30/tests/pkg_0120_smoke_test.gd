...
func _test_station_14() -> void:
	...
	_expect(station.is_bag_placed, "Torba musi zostać odłożona przy drzwiach")

	station.inspect_kettle()
	_expect(station.is_kettle_inspected, "Przerwany gest przy czajniku musi zostać zauważony")

	station.start_marta_dialogue()
	_expect(station.is_marta_dialogue_active, "Dialog z Martą musi wystartować")
	for i in range(station.marta_dialogue_lines.size()):
		station.advance_marta_dialogue()
	_expect(station.is_marta_dialogue_completed, "Dialog z Martą musi się zakończyć")

	station.apply_threshold_setback()
	_expect(station.threshold_setback_count == 1, "Pośpiech przy progu musi zostać policzony")
	_expect(station.mug_broken, "Setback musi stłuc kubek")
	_expect(not station.is_level_completed, "Setback nie może kończyć sceny")
...
func _test_station_22() -> void:
	print("TEST: Station 22 (Odchylenie w rejestrze)...")
	...
	station.start_offer_dialogue()
	_expect(station.is_offer_active, "Oferta UCP musi się wyświetlić")
	for i in range(station.offer_dialogue_lines.size()):
		station.advance_offer_dialogue()
	_expect(station.is_offer_completed, "Oferta UCP musi zostać odrzucona")

	station.examine_oscilloscope()
	_expect(station.is_oscilloscope_checked, "Zapis oscyloskopu musi zostać zbadany")
	_expect(station.are_modes_named, "Tryby Zakotwiczenie i Uległość muszą zostać nazwane")
...
func _test_station_23() -> void:
	print("TEST: Station 23 (Martwy obwód)...")
	...
	station.connect_circuit()
	station.switch_shunt()
	station.stabilize_overload()
	_expect(station.is_cost_understood, "Koszt mechaniki musi zostać zrozumiany")
