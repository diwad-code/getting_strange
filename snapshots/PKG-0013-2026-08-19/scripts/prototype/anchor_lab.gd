...
func trigger_correction_pulse() -> void:
	...
	_wave_tween.tween_property(self, "_wave_progress", 1.0, 0.40).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
...
func _draw() -> void:
	...
	if goal:
		...
		var goal_alpha := 0.18 if not _goal_reached else 0.40
		draw_rect(Rect2(goal.position + Vector2(-12.0, -32.0), Vector2(24.0, 44.0)), Color(goal_color, goal_alpha), true)
	...
	if _wave_progress >= 0.0:
		...
		# Leading wave line
		draw_line(Vector2(wave_x, 0), Vector2(wave_x, 360), wave_col, 2.5)
		# Trailing institutional consensus field
		draw_rect(Rect2(Vector2(wave_x - 32, 0), Vector2(32, 360)), Color(COLOR_CORRECTION, wave_alpha * 0.12), true)
		draw_rect(Rect2(Vector2(wave_x - 8, 0), Vector2(8, 360)), Color(COLOR_CORRECTION, wave_alpha * 0.22), true)
		
		# Measurement grid markers along the sweep front
		for y in range(20, 360, 40):
			draw_line(Vector2(wave_x - 4, y), Vector2(wave_x + 4, y), wave_col, 1.5)
