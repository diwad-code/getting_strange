...
func _test_minimal_playthrough(state: Node) -> void:
	print("3. Minimal route: movement + threshold + Station 18 commit reaches 43...")
	if state != null:
		state.reset_campaign(true)
		state.campaign_auto_transition_enabled = false
	...
	for station_id in linear:
		...
		if station == null:
			continue
		if station_id == "station_18" and station.has_method("commit_force_home"):
			station.call("commit_force_home")
		elif station_id == "station_18" and station.has_method("_commit_method"):
			station.call("_commit_method", &"force_home")
		...
		await physics_frame
		_expect(bool(station.get("is_level_completed")), "%s threshold must complete without optional readings" % station_id)
		station.queue_free()
		await process_frame
