...
@export var stage_variant: StringName = &""

## Flat-profile rooms named in docs/FRAME_LAYOUT_AUDIT.md §4.
const OVERHEAD_STATIONS := {
	24: true, 31: true, 32: true, 33: true,
	34: true, 35: true, 36: true, 37: true,
}
## Semantic anchor for the station-01 right-edge composition and its evidence gate.
const STATION_01_RIGHT_EDGE_ROLE: StringName = &"airlock_bulkhead"

## Nothing in the overhead run may reach down into the diegetic label band,
## which starts at y=90 across the campaign.
const OVERHEAD_FLOOR := 86.0
...
func _draw() -> void:
	# The apron goes down first: everything the station authors above the play
	# plane keeps drawing over it untouched.
	VectorStageStyle.draw_stage_apron(self, world_size)
	VectorStageStyle.draw_stage_background(self, world_size, stage_seed)
	_draw_act_depth()
	_draw_station_composition()
	_draw_overhead_structure()
	_draw_practical_lighting()
	_draw_stage_frame()


## PKG-0137, closing FRAME_LAYOUT_AUDIT §4.
## The flat-profile rooms carried ~129 px of information-free wall above Lena.
## They now get an overhead service run: one beam, hangers on an irregular
## rhythm, and a single duct for mass. It is scenery — no collider lives here —
## and it stops well above the diegetic label band, so it can never crowd text.
func _draw_overhead_structure() -> void:
	if not OVERHEAD_STATIONS.has(station_number):
		return
	var w := world_size.x
	# Off level on purpose: a perfectly horizontal bar reads as a screen border.
	VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
		Vector2(0.0, 22.0), Vector2(w, 16.0), Vector2(w, 34.0), Vector2(0.0, 40.0),
	]), VectorStageStyle.shade(VectorStageStyle.DEEP_PLANE, 0.18), 0.0)
	const HANGERS := [54.0, 138.0, 196.0, 302.0, 388.0, 452.0, 566.0]
	for i in HANGERS.size():
		var x: float = HANGERS[i]
		var drop: float = minf(
			52.0 + float((station_number + i * 5) % 4) * 8.0,
			OVERHEAD_FLOOR - 4.0
		)
		draw_colored_polygon(PackedVector2Array([
			Vector2(x, 30.0), Vector2(x + 5.0, 30.0),
			Vector2(x + 4.0, drop), Vector2(x + 1.0, drop),
		]), VectorStageStyle.INK)
		draw_line(
			Vector2(x - 3.0, drop), Vector2(x + 8.0, drop),
			VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.2), 1.0
		)
	var duct_x := 168.0 + float(station_number % 5) * 46.0
	draw_colored_polygon(PackedVector2Array([
		Vector2(duct_x, 34.0), Vector2(duct_x + 148.0, 31.0),
		Vector2(duct_x + 144.0, 68.0), Vector2(duct_x + 2.0, 71.0),
	]), VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.44))
	draw_line(
		Vector2(duct_x + 6.0, 52.0), Vector2(duct_x + 140.0, 49.0),
		VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.12), 1.0
	)


func _draw_act_depth() -> void:
	# One broad silhouette establishes the production chapter before local props
	# are added. These are scenic planes only: they never own collision.
	if station_number <= 13:
		draw_colored_polygon(PackedVector2Array([
			Vector2(0.0, 0.0), Vector2(178.0, 0.0), Vector2(146.0, 96.0),
			Vector2(0.0, 128.0),
		]), VectorStageStyle.shade(VectorStageStyle.DEEP_PLANE, 0.34))
	elif station_number <= 30:
		draw_colored_polygon(PackedVector2Array([
			Vector2(418.0, 0.0), Vector2(640.0, 0.0), Vector2(640.0, 172.0),
			Vector2(574.0, 134.0),
		]), VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.42))
	elif station_number <= 41:
		draw_colored_polygon(PackedVector2Array([
			Vector2(0.0, 0.0), Vector2(86.0, 0.0), Vector2(54.0, 292.0),
			Vector2(0.0, 318.0),
		]), VectorStageStyle.INK)
		draw_colored_polygon(PackedVector2Array([
			Vector2(568.0, 0.0), Vector2(640.0, 0.0), Vector2(640.0, 302.0),
			Vector2(600.0, 278.0),
		]), VectorStageStyle.shade(VectorStageStyle.DEEP_PLANE, 0.36))
	elif stage_variant == &"return":
		draw_colored_polygon(PackedVector2Array([
			Vector2(0.0, 0.0), Vector2(250.0, 0.0), Vector2(192.0, 122.0),
			Vector2(0.0, 150.0),
		]), VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.72))
	elif stage_variant == &"reconciliation":
		draw_colored_polygon(PackedVector2Array([
			Vector2(356.0, 0.0), Vector2(640.0, 0.0), Vector2(640.0, 152.0),
			Vector2(526.0, 118.0),
		]), VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.78))
	elif stage_variant == &"testimony":
		draw_colored_polygon(PackedVector2Array([
			Vector2(0.0, 0.0), Vector2(640.0, 0.0), Vector2(590.0, 58.0),
			Vector2(48.0, 58.0),
		]), VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.78))
	else:
		draw_colored_polygon(PackedVector2Array([
			Vector2(0.0, 0.0), Vector2(104.0, 0.0), Vector2(70.0, 286.0),
			Vector2(0.0, 316.0),
		]), VectorStageStyle.INK)


func _draw_practical_lighting() -> void:
	# The previous runtime repeated the same three pendants in every space. One
	# authored practical now identifies each chapter and varies deterministically
	# within it, keeping the route and narrative accents as the brightest planes.
	if station_number <= 13:
		var pendant_x := 92.0 + float((station_number * 137) % 438)
		var pendant_y := 34.0 + float((station_number * 11) % 18)
		var pendant_width := 13.0 + float(station_number % 3) * 3.0
		VectorStageStyle.draw_faceted_lamp(self, Vector2(pendant_x, pendant_y), pendant_width)
		if station_number in [1, 5, 8, 12]:
			_draw_strip_light(Vector2(392.0, 58.0), Vector2(520.0, 50.0), VectorStageStyle.HUMAN_AMBER)
	elif station_number <= 30:
		var strip_x := 54.0 + float((station_number * 83) % 310)
		var strip_y := 38.0 + float(station_number % 4) * 7.0
		var strip_color := VectorStageStyle.CORRECTION_OXIDE if station_number in [14, 20, 22, 25] else VectorStageStyle.LIGHT_PLANE
		_draw_strip_light(Vector2(strip_x, strip_y), Vector2(minf(strip_x + 164.0, 590.0), strip_y - 5.0), strip_color)
	elif station_number <= 41:
		var service_x := 76.0 if station_number % 2 == 0 else 558.0
		var service_color := VectorStageStyle.CORRECTION_OXIDE if station_number in [38, 41] else VectorStageStyle.ANCHOR_CYAN
		_draw_vertical_service_light(service_x, 62.0 + float(station_number % 3) * 15.0, 184.0, service_color)
	elif stage_variant == &"return":
		VectorStageStyle.draw_faceted_lamp(self, Vector2(188.0, 42.0), 20.0)
		_draw_strip_light(Vector2(82.0, 72.0), Vector2(216.0, 64.0), VectorStageStyle.HUMAN_AMBER)
	elif stage_variant == &"reconciliation":
		_draw_vertical_service_light(524.0, 74.0, 158.0, VectorStageStyle.HUMAN_AMBER)
	elif stage_variant == &"testimony":
		_draw_strip_light(Vector2(146.0, 46.0), Vector2(472.0, 46.0), VectorStageStyle.ANCHOR_CYAN)
	else:
		_draw_vertical_service_light(566.0, 66.0, 168.0, VectorStageStyle.ANCHOR_CYAN)


func _draw_strip_light(start: Vector2, end: Vector2, color: Color) -> void:
	var direction := (end - start).normalized()
	var normal := Vector2(-direction.y, direction.x)
	var spill := PackedVector2Array([
		start + normal * 5.0,
		end + normal * 5.0,
		end + normal * 40.0 + direction * 18.0,
		start + normal * 48.0 - direction * 18.0,
	])
	draw_colored_polygon(spill, Color(color, 0.07))
	draw_line(start, end, VectorStageStyle.shade(VectorStageStyle.LIGHT_PLANE, 0.38), 6.0)
	draw_line(start, end, color, 2.0)


func _draw_vertical_service_light(x: float, top: float, height: float, color: Color) -> void:
	draw_colored_polygon(PackedVector2Array([
		Vector2(x - 28.0, top), Vector2(x + 10.0, top),
		Vector2(x + 38.0, top + height), Vector2(x - 18.0, top + height),
	]), Color(color, 0.065))
	draw_rect(Rect2(x - 4.0, top, 8.0, height), VectorStageStyle.INK)
	draw_line(Vector2(x, top + 4.0), Vector2(x, top + height - 4.0), color, 2.0)


func _draw_stage_frame() -> void:
	# A restrained proscenium keeps the 640x360 image from reading as a loose
	# collage. Its asymmetry changes by act and leaves the play plane unobscured.
	var left_width := 10.0
	var right_width := 14.0
	if station_number >= 31:
		left_width = 18.0
		right_width = 10.0
	if station_number >= 42:
		left_width = 12.0
		right_width = 12.0
	# The legs run to the bottom of the framing budget, so the dialogue frame
	# never catches their cut edge against the apron (D-136).
	var leg_bottom := world_size.y + VectorStageStyle.STAGE_APRON
	draw_colored_polygon(PackedVector2Array([
		Vector2(0.0, 0.0), Vector2(left_width + 8.0, 0.0),
		Vector2(left_width, leg_bottom), Vector2(0.0, leg_bottom),
	]), VectorStageStyle.INK)
	draw_colored_polygon(PackedVector2Array([
		Vector2(world_size.x - right_width, 0.0), Vector2(world_size.x, 0.0),
		Vector2(world_size.x, leg_bottom), Vector2(world_size.x - right_width - 6.0, leg_bottom),
	]), VectorStageStyle.INK)
	var header_end := 176.0 if station_number <= 13 else (278.0 if station_number <= 30 else 116.0)
	if station_number >= 42:
		header_end = 212.0
	draw_colored_polygon(PackedVector2Array([
		Vector2(0.0, 0.0), Vector2(header_end, 0.0),
		Vector2(header_end - 18.0, 12.0), Vector2(0.0, 20.0),
	]), VectorStageStyle.INK)
...
func _draw_station_composition() -> void:
	match station_number:
		1:
			...
			# The right edge is the chamber airlock: a bulkhead, inspection window,
			# service panel and sealed seam. It is scenery only; the existing
			# ChamberDoor/AirlockZone remain the sole gameplay geometry.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
				Vector2(480, 68), Vector2(622, 78), Vector2(622, 292), Vector2(486, 292)
			]), VectorStageStyle.DEEP_PLANE, 0.0)
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
				Vector2(500, 94), Vector2(610, 102), Vector2(610, 292), Vector2(504, 292)
			]), VectorStageStyle.MID_PLANE, 1.0)
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
				Vector2(518, 116), Vector2(592, 121), Vector2(592, 160), Vector2(520, 156)
			]), VectorStageStyle.LIGHT_PLANE, 0.0)
			draw_line(Vector2(524, 148), Vector2(586, 152), VectorStageStyle.DEEP_PLANE, 2.0)
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
				Vector2(516, 194), Vector2(596, 199), Vector2(596, 260), Vector2(520, 256)
			]), VectorStageStyle.DEEP_PLANE, 1.0)
			draw_line(Vector2(526, 211), Vector2(584, 214), VectorStageStyle.LIGHT_PLANE, 1.0)
			draw_line(Vector2(526, 226), Vector2(584, 229), VectorStageStyle.LIGHT_PLANE, 1.0)
			draw_line(Vector2(526, 241), Vector2(584, 244), VectorStageStyle.LIGHT_PLANE, 1.0)
			draw_colored_polygon(PackedVector2Array([
				Vector2(558, 171), Vector2(574, 172), Vector2(574, 188), Vector2(558, 187)
			]), VectorStageStyle.HUMAN_AMBER)
			draw_line(Vector2(566, 112), Vector2(566, 278), VectorStageStyle.INK, 5.0)
			draw_colored_polygon(PackedVector2Array([
				Vector2(504, 274), Vector2(608, 278), Vector2(608, 286), Vector2(506, 282)
			]), VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.42))
		...
		5:
			...
			# PKG-0198 (ZERO wycinek 2): dwa prostokąty poza kadrem (x672+/x1010+)
			# z diagnozy 0197 wycięte — martwy kod, nigdy widoczny w 640x360.
			# Zostaje jedno oświetlone przejście jako jedyna jasna bryła.
			var horizon := 186.0
		...
		8:
			# Numer czternaście: elewacja bloku po lewej, ciemny podcień wejścia po prawej,
			# jasna tablica domofonu jako jedyny punkt czytania.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(16, 62), Vector2(452, 48), Vector2(470, 300), Vector2(12, 312)]), VectorStageStyle.MID_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(486, 58), Vector2(624, 72), Vector2(624, 300), Vector2(490, 300)]), VectorStageStyle.INK)
			draw_colored_polygon(PackedVector2Array([Vector2(252, 196), Vector2(310, 192), Vector2(314, 266), Vector2(256, 270)]), VectorStageStyle.LIGHT_PLANE)
			draw_colored_polygon(PackedVector2Array([Vector2(392, 214), Vector2(430, 212), Vector2(432, 258), Vector2(394, 260)]), VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.42))
		9:
			# Sąsiadka z trzeciego: skos biegu schodów w prawo, okno półpiętra jako źródło światła,
			# pusta wnęka po gaśnicy zostaje ciemna.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(18, 56), Vector2(384, 44), Vector2(404, 298), Vector2(14, 310)]), VectorStageStyle.MID_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(404, 300), Vector2(516, 236), Vector2(624, 236), Vector2(624, 302)]), VectorStageStyle.DEEP_PLANE)
			draw_colored_polygon(PackedVector2Array([Vector2(464, 74), Vector2(560, 82), Vector2(556, 186), Vector2(462, 180)]), VectorStageStyle.LIGHT_PLANE)
			draw_colored_polygon(PackedVector2Array([Vector2(140, 228), Vector2(172, 226), Vector2(174, 272), Vector2(142, 274)]), VectorStageStyle.INK)
		10:
			# Klucz: wąski próg. Ciemne skrzydło drzwi 14 po prawej, wąska bursztynowa szpara
			# pod nimi jest jedyną obietnicą wnętrza.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(20, 66), Vector2(430, 52), Vector2(448, 296), Vector2(16, 308)]), VectorStageStyle.MID_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(462, 60), Vector2(596, 74), Vector2(592, 300), Vector2(466, 300)]), VectorStageStyle.INK)
			draw_colored_polygon(PackedVector2Array([Vector2(470, 286), Vector2(588, 286), Vector2(588, 294), Vector2(470, 294)]), VectorStageStyle.HUMAN_AMBER)
			draw_colored_polygon(PackedVector2Array([Vector2(110, 268), Vector2(238, 262), Vector2(244, 302), Vector2(104, 306)]), VectorStageStyle.DEEP_PLANE)
		11:
			# Dwie osoby na zdjęciu: ciepła domowa wyspa w środku kadru, komoda przecina przejście,
			# a głąb mieszkania zostaje niedopowiedziana.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(14, 70), Vector2(468, 52), Vector2(492, 296), Vector2(12, 312)]), VectorStageStyle.MID_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(120, 254), Vector2(360, 240), Vector2(372, 300), Vector2(112, 306)]), VectorStageStyle.HUMAN_AMBER)
			draw_colored_polygon(PackedVector2Array([Vector2(508, 62), Vector2(626, 76), Vector2(626, 302), Vector2(512, 296)]), VectorStageStyle.INK)
			draw_colored_polygon(PackedVector2Array([Vector2(206, 202), Vector2(258, 198), Vector2(262, 244), Vector2(210, 248)]), VectorStageStyle.LIGHT_PLANE)
		12:
			# Wiadomość głosowa: ciepły stolik z telefonem po lewej, zimne światło ulicy
			# wpada uchylonym balkonem po prawej i rozcina pokój.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(16, 64), Vector2(474, 50), Vector2(496, 298), Vector2(14, 310)]), VectorStageStyle.MID_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(186, 246), Vector2(392, 236), Vector2(402, 298), Vector2(178, 304)]), VectorStageStyle.HUMAN_AMBER)
			draw_colored_polygon(PackedVector2Array([Vector2(512, 60), Vector2(624, 70), Vector2(624, 300), Vector2(516, 300)]), VectorStageStyle.DEEP_PLANE)
			draw_colored_polygon(PackedVector2Array([Vector2(538, 96), Vector2(600, 100), Vector2(598, 268), Vector2(540, 266)]), VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.30))
		13:
			# Dwie ważne wersje: blat biurka na skos, dwa jasne pola dokumentów obok siebie,
			# reszta pokoju schodzi w cień.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(18, 60), Vector2(452, 46), Vector2(474, 290), Vector2(16, 306)]), VectorStageStyle.MID_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(120, 240), Vector2(410, 226), Vector2(424, 286), Vector2(112, 296)]), VectorStageStyle.LIGHT_PLANE)
			draw_colored_polygon(PackedVector2Array([Vector2(504, 66), Vector2(624, 80), Vector2(620, 296), Vector2(508, 288)]), VectorStageStyle.INK)
			draw_colored_polygon(PackedVector2Array([Vector2(268, 156), Vector2(302, 154), Vector2(304, 202), Vector2(270, 204)]), VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.68))
		14:
		...
		38:
			...
			draw_colored_polygon(PackedVector2Array([Vector2(272, 230), Vector2(326, 224), Vector2(334, 268), Vector2(264, 274)]), VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.24))
			...
			draw_line(Vector2(326, 218), Vector2(392, 214), VectorStageStyle.HUMAN_AMBER, 1.5)
		39:
		...
		41:
			# Komora wyboru: three upright bays share one institutional stage, while
			# the open lower band leaves the real route as the highest-contrast plane.
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(0, 42), Vector2(624, 48), Vector2(624, 168), Vector2(0, 160)]), VectorStageStyle.MID_PLANE, 0.0)
			VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([Vector2(0, 176), Vector2(624, 184), Vector2(624, 278), Vector2(0, 278)]), VectorStageStyle.DEEP_PLANE, 0.0)
			draw_colored_polygon(PackedVector2Array([Vector2(58, 72), Vector2(168, 66), Vector2(174, 218), Vector2(54, 224)]), VectorStageStyle.LIGHT_PLANE)
			draw_colored_polygon(PackedVector2Array([Vector2(196, 58), Vector2(266, 58), Vector2(270, 266), Vector2(194, 266)]), VectorStageStyle.INK)
			draw_colored_polygon(PackedVector2Array([Vector2(306, 64), Vector2(376, 66), Vector2(378, 266), Vector2(304, 266)]), VectorStageStyle.INK)
			draw_colored_polygon(PackedVector2Array([Vector2(416, 58), Vector2(486, 60), Vector2(490, 266), Vector2(414, 266)]), VectorStageStyle.INK)
			draw_line(Vector2(220, 76), Vector2(220, 198), VectorStageStyle.ANCHOR_CYAN, 1.4)
			draw_line(Vector2(330, 76), Vector2(330, 198), VectorStageStyle.HUMAN_AMBER, 1.4)
			draw_line(Vector2(440, 76), Vector2(440, 198), VectorStageStyle.CORRECTION_OXIDE, 1.4)
		42:
			_draw_finale_42()
		43:
			_draw_finale_43()
...
func _draw_finale_42() -> void:
	# The three endings share one reference chamber but use different material
	# axes. The route remains open below y=276 for the station-owned play pass.
	match stage_variant:
		&"return":
			# Powrót: a measured horizontal return frame and two-cup witness bay.
			...
			draw_line(Vector2(76.0, 86.0), Vector2(414.0, 80.0), VectorStageStyle.LIGHT_PLANE, 1.0)
			draw_line(Vector2(76.0, 98.0), Vector2(414.0, 92.0), VectorStageStyle.shade(VectorStageStyle.LIGHT_PLANE, 0.48), 1.0)
			VectorStageStyle.draw_facet_polygon(
				self,
				PackedVector2Array([
					Vector2(184.0, 204.0),
				...
				VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.24),
				0.0,
			)
			draw_line(Vector2(430.0, 224.0), Vector2(592.0, 218.0), VectorStageStyle.ANCHOR_CYAN, 1.2)
		&"reconciliation":
			# Uzgodnienie: a domestic threshold interrupts the institutional plane.
			VectorStageStyle.draw_facet_polygon(
				self,
				PackedVector2Array([
					Vector2(0.0, 62.0),
			...
			VectorStageStyle.draw_facet_polygon(
				...
				VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.44),
				1.0,
			)
			draw_line(Vector2(294.0, 78.0), Vector2(294.0, 244.0), VectorStageStyle.LIGHT_PLANE, 1.4)
			draw_line(Vector2(524.0, 82.0), Vector2(524.0, 238.0), VectorStageStyle.shade(VectorStageStyle.CORRECTION_OXIDE, 0.48), 1.0)
			draw_line(Vector2(530.0, 224.0), Vector2(606.0, 224.0), VectorStageStyle.ANCHOR_CYAN, 1.2)
		&"testimony":
			# Świadectwo: two public tracks share the same civic frame and remain
			# visibly concurrent rather than collapsing into a single solution.
			VectorStageStyle.draw_facet_polygon(
				self,
				PackedVector2Array([
					Vector2(0.0, 48.0),
			...
			for track_index in range(2):
				var track_y := 214.0 + float(track_index) * 28.0
				draw_line(Vector2(48.0, track_y), Vector2(582.0, track_y + 18.0), VectorStageStyle.LIGHT_PLANE, 1.5)
				draw_line(Vector2(48.0, track_y + 7.0), Vector2(582.0, track_y + 25.0), VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.34), 1.0)
				draw_line(Vector2(116.0 + float(track_index) * 210.0, 78.0), Vector2(116.0 + float(track_index) * 210.0, 188.0), VectorStageStyle.LIGHT_PLANE, 1.0)
			draw_colored_polygon(
				PackedVector2Array([Vector2(468.0, 92.0), Vector2(602.0, 98.0), Vector2(602.0, 166.0), Vector2(468.0, 160.0)]),
				VectorStageStyle.INK,
			)
			return
		_:
			# Deterministic fallback for a future audit: neutral reference chamber.
			VectorStageStyle.draw_facet_polygon(
				self,
				PackedVector2Array([Vector2(24.0, 58.0), Vector2(616.0, 58.0), Vector2(616.0, 260.0), Vector2(24.0, 260.0)]),
				VectorStageStyle.MID_PLANE,
				0.0,
			)


func _draw_finale_43() -> void:
	# Epilogue: ordinary civic surfaces carry the ending; no branch receives a
	# brighter or darker full-screen treatment.
	VectorStageStyle.draw_facet_polygon(
		self,
		PackedVector2Array([
			Vector2(0.0, 54.0),
	...
	draw_colored_polygon(
		PackedVector2Array([Vector2(42.0, 86.0), Vector2(192.0, 76.0), Vector2(188.0, 196.0), Vector2(36.0, 202.0)]),
		VectorStageStyle.MID_PLANE,
	)
	draw_colored_polygon(
		PackedVector2Array([Vector2(246.0, 72.0), Vector2(408.0, 70.0), Vector2(412.0, 194.0), Vector2(250.0, 198.0)]),
		VectorStageStyle.MID_PLANE,
	)
	draw_colored_polygon(
		PackedVector2Array([Vector2(462.0, 84.0), Vector2(604.0, 90.0), Vector2(606.0, 188.0), Vector2(466.0, 194.0)]),
		VectorStageStyle.MID_PLANE,
	)
	draw_line(Vector2(58.0, 118.0), Vector2(174.0, 112.0), VectorStageStyle.LIGHT_PLANE, 1.0)
	draw_line(Vector2(272.0, 106.0), Vector2(382.0, 106.0), VectorStageStyle.HUMAN_AMBER, 1.0)
	draw_line(Vector2(486.0, 122.0), Vector2(584.0, 126.0), VectorStageStyle.ANCHOR_CYAN, 1.0)
	draw_line(Vector2(220.0, 214.0), Vector2(420.0, 210.0), VectorStageStyle.CORRECTION_OXIDE, 1.0)
