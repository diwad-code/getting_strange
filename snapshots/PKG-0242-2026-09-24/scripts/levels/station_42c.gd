...
## CR-C (PKG-0195): rozmowa wg DIALOGUE_SCRIPT §13 — kubek i pusta półka,
## pytanie Równi o odpowiedzialność zamiast komentarza o dwóch domach.
## Przeciek obcej pamięci Jakuba dociera jako echo / ekran czytnika (JAKUB (ECHO)),
## nie jako fizyczna obecność Jakuba przy imadle w salonie (PKG-0237 D5 / PKG-0238 E3).
## Dostarcza lokalny prezenter CRT (wzór CR-A/CR-B) z `creative_scene_lines.gd`.
## PKG-0238 (E3): rezerwa ta nie gra na trasie kampanii (gdzie kwestie dostarcza
...
const DIALOGUE_LINES: Array[Dictionary] = [
	{"speaker": "MARTA DOMOWA", "text": "Znam ten kubek."},
	{"speaker": "LENA", "text": "Nie masz go."},
	{"speaker": "LENA", "text": "Obie patrzymy na pustą półkę."},
	{"speaker": "JAKUB (ECHO)", "text": "Przy imadle zobaczyłem prosektorium. Odkładam narzędzie."},
]
...
## PKG-0233 (D5): maly koszt z 16 w wyplacie finalu ("marta_memory" /
## "sample_second" / "" = jawna luka). Czytany z decyzji, nie nowy fakt.
var payoff_cost := ""
...
func _ready() -> void:
	...
	payoff_cost = _read_small_cost()
	_restore_chain_from_decisions()
...
func _setup_guidance() -> void:
	...
	# PKG-0225 (K1/K3/K5): nośniki finałowe — kurtka na haku (cudze mydło),
	# hełm z garnka przy imadle (konkret Jakuba z jakub_questions) oraz koszula
	# ściągnięta w dół (blizna ciałem, granica z 12 trzyma). Konkrety bez tezy
	# (D-214); wzór K4: rejestracja + ślad w _draw, bez triggera.
	_register_beat(&"s42c_jacket", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Kurtka na haku pachnie cudzym mydłem.", "The jacket on the hook smells of foreign soap.", &"", "")
	_register_beat(&"s42c_helmet", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Przy imadle hełm z garnka. Ten sam, o który pytałam.", "A pot helmet by the vise. The same one I asked about.", &"", "")
	_register_beat(&"s42c_shirt", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Przy imadle koszula ściągnięta w dół. Boku nie pokazuje.", "A shirt pulled down by the vise. He does not show his side.", &"", "")
	_register_beat(&"s42c_leak_hypothesis", GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT, &"interpretation", &"fallible", "Co zostanie połączone na stałe? Pamięć przecieka między światami. Obie odpowiadamy przed własnym domem.", "What will be permanently connected? Memory leaks between worlds. Both of us answer to our own homes.", &"mutual_memory_leak_uncontrolled", "read_memory_leak")
	...
	_register_beat(&"s42c_both_seen", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Obie odpowiadamy przed własnym życiem. Most trzyma okno.", "Both of us answer to our own lives. The bridge holds the window.", &"", "")
...
## PKG-0232 (D-244): fakty żyją w decyzjach (raz), prezentacja gra na każdej
## świeżej instancji. Flaga lokalna + fakt w decyzjach = rozliczone;
## flaga pusta + fakt w decyzjach = odtworzenie bez zapisu (zero _record).
func _decided(fact: StringName) -> bool:
	var value: Variant = _read_decision(fact)
...
func execute_mutual_passage() -> bool:
	if is_passage_opened and _decided(FACT_PASSAGE_OPENED):
		return false
	...
	if not _decided(FACT_PASSAGE_OPENED):
...
func read_memory_leak() -> bool:
	if is_memory_leak_read and _decided(FACT_MEMORY_LEAK):
		return false
	if not _has_mutual_passage():
		_record_feedback(&"method_mutual_passage_required")
		return false
	# PKG-0232 (D-244, D3): stan dopiero po wykonaniu.
	if not is_passage_opened and not _decided(FACT_PASSAGE_OPENED):
		_record_feedback(&"passage_execution_required")
		if guidance_service:
			guidance_service.trigger_beat(&"s42c_read_plan")
		return false
	is_memory_leak_read = true
	is_chamber_c_witnessed = true
	if not _decided(FACT_MEMORY_LEAK):
...
func read_household_consequence() -> bool:
	if is_household_read and _decided(FACT_HOUSEHOLD):
		return false
	if not _has_mutual_passage():
		_record_feedback(&"method_mutual_passage_required")
		return false
	# PKG-0232 (D-244, D3): skutek dopiero po odczycie stanu.
	if not is_memory_leak_read and not _decided(FACT_MEMORY_LEAK):
		_record_feedback(&"leak_state_required")
		if guidance_service:
			guidance_service.trigger_beat(&"s42c_read_plan")
		return false
	marta_truth_state = _read_marta_truth()
	jakub_consent_state = _read_jakub_consent()
	payoff_cost = _read_small_cost()
	# PKG-0230 (P0-1, S-02): pusta zgoda jest luka, nie wartoscia domyslna —
	# final nie wypelnia jej SCOPE_GRANTED. Nazwana luka zamiast fabrykacji.
	...
	household_consequence = {
		...
		# bramka 0195 czyta tylko klucze marta/jakub — zero churnu).
		"cost": payoff_cost,
	}
	is_household_read = true
	if not _decided(FACT_HOUSEHOLD):
...
func forward_block_line() -> String:
	if is_level_completed or not _has_mutual_passage():
		return ""
	if not (is_passage_opened or _decided(FACT_PASSAGE_OPENED)):
		return "Zanim wyjdę, otworzę okno dla obu sygnatur."
	if not (is_memory_leak_read or _decided(FACT_MEMORY_LEAK)):
		return "Zanim wyjdę, odczytam, co przeciekło między nami."
	if not (is_household_read or _decided(FACT_HOUSEHOLD)):
		return "Zanim wyjdę, zobaczę, co zostało przy stole."
	return ""
...
func _trigger_level_completion() -> void:
	if is_level_completed:
		return
	# PKG-0232 (D-244, D3): próg 43 dopiero po pełnym łańcuchu
	# (wykonanie → stan → skutek), lokalnie lub w decyzjach (powrót).
	# Wyjście wizualnie otwarte (D-227), ale completion bez łańcucha nie istnieje.
	if not ((is_passage_opened or _decided(FACT_PASSAGE_OPENED)) \
			and (is_memory_leak_read or _decided(FACT_MEMORY_LEAK)) \
			and (is_household_read or _decided(FACT_HOUSEHOLD))):
		_record_feedback(&"finale_sequence_incomplete")
		if guidance_service:
			guidance_service.trigger_beat(&"s42c_read_plan")
		return
...
## PKG-0232 (D-244, pakiet A): rozdzielenie writerów od nawigacji — świeża
...
func _restore_chain_from_decisions() -> void:
	is_passage_opened = _decided(FACT_PASSAGE_OPENED)
	is_tram_inspected = is_passage_opened
	is_memory_leak_read = _decided(FACT_MEMORY_LEAK)
	is_chamber_c_witnessed = is_memory_leak_read
...
func _has_mutual_passage() -> bool:
	return NarrativeRules.committed(_narrative_decisions(), "mutual_passage")
...
func _draw_dawn_apartment_c() -> void:
	...
	# PKG-0225 (K1): kurtka na haku między progiem a stołem; zapach cudzego
	# mydła niesie beat s42c_jacket. Prymitywy, zero nowych assetów.
	draw_line(Vector2(400.0, 210.0), Vector2(400.0, 226.0), VectorStageStyle.MID_PLANE, 2.0)
	draw_arc(Vector2(400.0, 230.0), 4.0, 0.0, PI, 8, VectorStageStyle.MID_PLANE, 1.5)
	draw_colored_polygon(
		PackedVector2Array([Vector2(392.0, 234.0), Vector2(408.0, 234.0), Vector2(406.0, 262.0), Vector2(394.0, 262.0)]),
		VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.55),
	)
...
func _draw_mutual_passage() -> void:
	...
	# PKG-0225 (K3): kącik imadła Jakuba między przejściem a progiem —
	# stół warsztatowy, imadło, hełm z garnka (beat s42c_helmet).
	# PKG-0225 (K5): koszula ściągnięta w dół przy imadle (beat s42c_shirt).
	# Prymitywy, zero nowych assetów.
	draw_rect(Rect2(228.0, 252.0, 40.0, 20.0), VectorStageStyle.shade(VectorStageStyle.DEEP_PLANE, 0.1))
	draw_rect(Rect2(240.0, 240.0, 16.0, 12.0), VectorStageStyle.MID_PLANE, false, 1.5)
	draw_arc(Vector2(262.0, 248.0), 8.0, PI, TAU, 12, VectorStageStyle.LIGHT_PLANE, 1.5)
	draw_line(Vector2(252.0, 248.0), Vector2(272.0, 248.0), VectorStageStyle.MID_PLANE, 1.5)
	draw_colored_polygon(
		PackedVector2Array([Vector2(230.0, 238.0), Vector2(244.0, 238.0), Vector2(243.0, 252.0), Vector2(231.0, 252.0)]),
		VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.5),
	)
	draw_line(Vector2(230.0, 250.0), Vector2(243.0, 250.0), VectorStageStyle.LIGHT_PLANE, 1.0)
...
func _draw_memory_leak_threshold() -> void:
	...
	# PKG-0187: two unreadable reflections inside frosted reader panes. They are
	# not NPC bodies in the room, so no circle-head / line-body shorthand leaks
	# back into the active route (CAST_AND_NPC_BIBLE §1 exception).
	draw_colored_polygon(PackedVector2Array([
		Vector2(309.0, 270.0), Vector2(325.0, 270.0), Vector2(324.0, 232.0),
		Vector2(321.0, 218.0), Vector2(323.0, 210.0), Vector2(317.0, 204.0),
		Vector2(311.0, 210.0), Vector2(313.0, 218.0), Vector2(310.0, 232.0),
	]), Color(VectorStageStyle.HUMAN_AMBER, 0.32))
	draw_colored_polygon(PackedVector2Array([
		Vector2(339.0, 270.0), Vector2(355.0, 270.0), Vector2(354.0, 232.0),
		Vector2(351.0, 218.0), Vector2(353.0, 210.0), Vector2(347.0, 204.0),
		Vector2(341.0, 210.0), Vector2(343.0, 218.0), Vector2(340.0, 232.0),
	]), Color(VectorStageStyle.ANCHOR_CYAN, 0.32))
	draw_line(Vector2(308.0, 236.0), Vector2(326.0, 236.0), Color(leak_color, 0.28), 1.0)
	draw_line(Vector2(338.0, 236.0), Vector2(356.0, 236.0), Color(leak_color, 0.28), 1.0)
...
func _draw_household_table() -> void:
	...
	# PKG-0226 (N6-reszta): pierścień prawdy NA BLACIE stołu (490,262) —
	# zapisany marta_truth_state, nie nowy fakt.
	_draw_truth_ring(Vector2(490.0, 262.0))
	# PKG-0233 (D5): zgoda Jakuba i maly koszt z 16 jako ksztalty na blacie.
	_draw_payoff_marks()


func _draw_truth_ring(center: Vector2) -> void:
	# PKG-0226 (N6-reszta): r8/w2.5 — próg widoczności kompozytora (cienki
	# łuk r5/w1.5 ginie w próbkowaniu 2x2; kadr (570,262) odpada, bo stoi
	# na geometrii drzwi — fakt z inspekcji kadrów).
	var ring_color := VectorStageStyle.ANCHOR_CYAN if is_household_read else VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.5)
	match marta_truth_state:
		"full":
			draw_arc(center, 8.0, 0.0, TAU, 20, ring_color, 2.5)
		"partial":
			draw_arc(center, 8.0, -PI * 0.5, PI * 0.5, 16, ring_color, 2.5)
		_:
			draw_arc(center, 8.0, 0.4, TAU - 0.4, 20, ring_color, 2.5)


## PKG-0233 (D5): dwie kategorie wyplaty, ktorych 0226 nie rysowal — zgoda
## (zatrzask: granted = podwojny slupek, limited = pojedynczy, refused = X,
## brak = dwie kropki) i maly koszt z 16 (trojkat = pamiec Marty, kwadrat =
## sekunda probki, brak = jawna nieobecnosc). Ksztalty, nie sam kolor
## (VISUAL_DESIGN §2); wspolrzedne na blacie jak pierscien (r8/w2.5).
func _draw_payoff_marks() -> void:
	var mark_color := VectorStageStyle.ANCHOR_CYAN if is_household_read else VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.5)
	match jakub_consent_state:
		"granted":
			draw_line(Vector2(462.0, 268.0), Vector2(462.0, 280.0), mark_color, 2.0)
			draw_line(Vector2(470.0, 268.0), Vector2(470.0, 280.0), mark_color, 2.0)
		"limited":
			draw_line(Vector2(466.0, 268.0), Vector2(466.0, 280.0), mark_color, 2.0)
		"refused":
			draw_line(Vector2(460.0, 268.0), Vector2(472.0, 280.0), mark_color, 2.0)
			draw_line(Vector2(472.0, 268.0), Vector2(460.0, 280.0), mark_color, 2.0)
		_:
			draw_circle(Vector2(463.0, 274.0), 1.5, mark_color)
			draw_circle(Vector2(469.0, 274.0), 1.5, mark_color)
	match payoff_cost:
		"marta_memory":
			draw_colored_polygon(
				PackedVector2Array([Vector2(540.0, 278.0), Vector2(546.0, 268.0), Vector2(552.0, 278.0)]),
				mark_color,
			)
		"sample_second":
			draw_rect(Rect2(540.0, 268.0, 12.0, 12.0), mark_color, false, 2.0)
		_:
			pass


## PKG-0233 (D5): odczyt malego kosztu z 16; tylko znane identyfikatory
## wyboru przechodza, reszta (w tym brak) to jawna luka "".
func _read_small_cost() -> String:
	var state := get_node_or_null("/root/GameStateManager")
	if state == null:
		return ""
	var cost := String(state.decisions.get(&"p9.mechanics.small_cost.choice", ""))
	if cost == "marta_memory" or cost == "sample_second":
		return cost
	return ""
...
func _draw_exit() -> void:
	...
	# PKG-0198 (ZERO wycinek 2): cień kontaktowy w prawo, 0.48, zgodnie
	# z lampą finału. Finał: cisza z jednym źródłem (dron rigu); brak
	# dodatkowego pozycjonowanego humu — decyzja jawna.
	draw_colored_polygon(PackedVector2Array([
		Vector2(120.0, 298.0), Vector2(564.0, 294.0),
		Vector2(572.0, 302.0), Vector2(128.0, 306.0),
...
func _on_narrative_dialogue_finished(id: String) -> void:
	if id == "household_consequence" and is_household_read:
		_record(&"p9.finale.mutual_passage.home_marta_told", true)
