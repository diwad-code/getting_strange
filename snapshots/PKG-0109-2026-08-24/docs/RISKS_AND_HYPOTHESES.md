...
| H-005 | Rówień Vector-Stage (własne płaszczyzny wielokątne i rytm poz) jest wykonalny w zakładanym zakresie | KOSZTOWA | TECHNICAL | PKG-0109 wykonał świeży pomiar 10 kadrów świata (w tym 9 wymaganych reprezentatywnych i Station 02 z `DiscontinuousShadow`), dwa kadry świata z CRT, cykl proceduralny protagonistki oraz inventory 45 zasobów odpowiadających 43 przestrzeniom. Średni `render_cpu_ms` kadrów świata wyniósł 0.654–1.016 ms, średni `render_gpu_ms` 0.720–1.606 ms; nadal brak jawnego budżetu liczbowego i drugiego profilu sprzętowego | porównać bezpośredni koszt kadru i animacji z jawnym budżetem produkcyjnym; **rozstrzygalne bez testerów** |
