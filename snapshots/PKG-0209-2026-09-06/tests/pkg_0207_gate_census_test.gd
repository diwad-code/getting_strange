...
##
## PKG-0208 (D-223): kontrolowana aktualizacja liczb 101 -> 102 / 103 -> 104 /
## 102 -> 103 razem z nowym testem (jawna regula ewolucji z D-222, wzor
## D-216/D-218: stara asercja nazwana i zastapiona kontrola rownowazna,
## prog nie obnizony). Asercje handoffu `contains` przechodza bez zmian.
...
const EXPECTED_INVOKE_LINES := 104
const EXPECTED_SCRIPT_ARGS := 103
const EXPECTED_TEST_REFS := 102
const EXPECTED_DISK_TESTS := 102
...
func _finish() -> void:
	if _failures.is_empty():
		print("PKG-0207 GATE CENSUS PASS: 104 invokes / 103 scripts / 102 tests, 640x360, 60 Hz, 10 actions, handoff coherent.")
		quit(0)
	else:
