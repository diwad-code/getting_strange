...
## PRZESZKODA — dlaczego to tu jest: rysunek studni istnieje w dwóch wersjach
## papieru, bo korekta zachowała skażenie wody, ale wymazała osobę, która je wykryła.
## PRZESZKODA — czego wymaga od Leny: zakotwiczenia konkretnego śladu przed
## oddaniem świadectwa albo świadomego pozostawienia go procedurze UCP.
## PRZESZKODA — koszt porażki: korekta odsyła Lenę do wejścia, a podpis Igi
## znika z papieru na stałe, choć publiczny raport o wodzie pozostaje.
...
@onready var well_drawing_anchor: AnchorableObject = get_node_or_null("Geometry/SzymonWellDrawing") as AnchorableObject
...
var is_drawing_anchored: bool = false
var drawing_correction_count: int = 0
var drawing_detail_faded: bool = false
var last_drawing_correction_target: AnchorableObject.RealityState = AnchorableObject.RealityState.STATE_A
...
func _physics_process(_delta: float) -> void:
	if well_drawing_anchor and player:
		well_drawing_anchor.update_player_distance(player.global_position)


func _setup_drawing_anchor() -> void:
	if well_drawing_anchor == null:
		return
	well_drawing_anchor.anchor_state_changed.connect(_on_drawing_anchor_changed)
	well_drawing_anchor.apply_reality_shift(AnchorableObject.RealityState.STATE_A, false)


func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed(&"trigger_correction") and not event.is_echo():
		run_drawing_correction_pass()
	...
	if dialogue_active:
		...
		get_viewport().set_input_as_handled()
		return
	if is_drawing_inspected and well_drawing_anchor and well_drawing_anchor.is_player_in_range:
		well_drawing_anchor.toggle_anchor()
		get_viewport().set_input_as_handled()
...
func choose_drawing_disposition(choice: int) -> void:
	...
	if well_drawing_anchor:
		if choice == DrawingChoice.ANCHOR_DRAWING:
			well_drawing_anchor.set_anchored(true)
		elif choice == DrawingChoice.SUBMIT_TO_UCP:
			well_drawing_anchor.set_anchored(false)
			well_drawing_anchor.apply_reality_shift(AnchorableObject.RealityState.STATE_B, false)
		else:
			well_drawing_anchor.set_anchored(false)
			well_drawing_anchor.apply_reality_shift(AnchorableObject.RealityState.STATE_A, false)
	var state := get_node_or_null("/root/GameStateManager")
	if state and state.has_method("record_decision"):
		state.record_decision(&"station_20_drawing_disposition", choice)
	drawing_disposition_chosen.emit(choice)
	_unlock_exit()


func _on_drawing_anchor_changed(anchored: bool) -> void:
	is_drawing_anchored = anchored
	if anchored:
		var state := get_node_or_null("/root/GameStateManager")
		if state and state.has_method("record_decision"):
			state.record_decision(&"station_20_anchored_well_drawing", true)
	queue_redraw()


func run_drawing_correction_pass() -> void:
	if well_drawing_anchor == null:
		return
	last_drawing_correction_target = (
		AnchorableObject.RealityState.STATE_B
		if well_drawing_anchor.current_reality == AnchorableObject.RealityState.STATE_A
		else AnchorableObject.RealityState.STATE_A
	)
	well_drawing_anchor.apply_reality_shift(last_drawing_correction_target, false)
	if not well_drawing_anchor.is_anchored:
		_apply_drawing_correction()
	queue_redraw()


func _apply_drawing_correction() -> void:
	drawing_correction_count += 1
	drawing_detail_faded = true
	var state := get_node_or_null("/root/GameStateManager")
	if state and state.has_method("record_decision"):
		state.record_decision(&"station_20_drawing_corrected", drawing_correction_count)
	var drawing_prop := props.get_node_or_null("WellDrawing") as MemoryResonancePoint
	if drawing_prop:
		drawing_prop.prop_subtitle = "Kredkowy szkic — podpis Igi wyblakł"
	if well_drawing_anchor:
		well_drawing_anchor.set_anchored(false)
		well_drawing_anchor.apply_reality_shift(AnchorableObject.RealityState.STATE_A, false)
	if player:
...
func _draw_state_layer() -> void:
	...
	if is_door_shifted:
		...
		draw_line(Vector2(shift_x - 14.0, 206.0), Vector2(shift_x - 14.0, 270.0), VectorStageStyle.CORRECTION_OXIDE, 1.5)
		draw_line(Vector2(shift_x, 206.0), Vector2(shift_x, 270.0), VectorStageStyle.ANCHOR_CYAN, 1.0)
	if drawing_detail_faded:
		draw_line(Vector2(334.0, 236.0), Vector2(350.0, 236.0), VectorStageStyle.shade(VectorStageStyle.CORRECTION_OXIDE, 0.40), 2.0)
	if is_drawing_anchored:
		draw_line(Vector2(306.0, 222.0), Vector2(354.0, 222.0), VectorStageStyle.ANCHOR_CYAN, 1.0)
	if is_exit_unlocked:
...
func _draw_dialogue_hud(line: Dictionary) -> void:
	...
	if font:
		draw_string(font, Vector2(72.0, 304.0), speaker, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, border_color)
		draw_string(font, Vector2(72.0, 326.0), text, HORIZONTAL_ALIGNMENT_LEFT, 500, 10, Color("dcebe4"))
...
func _draw_legacy_environment() -> void:
	...
	if dialogue_active and dialogue_index >= 0 and dialogue_index < dialogue_lines.size():
		_draw_legacy_dialogue_hud(dialogue_lines[dialogue_index])


func _draw_legacy_dialogue_hud(line: Dictionary) -> void:
	var speaker: String = line.get("speaker", "")
	var text: String = line.get("text", "")
	var is_witness: bool = line.get("is_witness", false)
	var is_lena: bool = line.get("is_lena", false)
