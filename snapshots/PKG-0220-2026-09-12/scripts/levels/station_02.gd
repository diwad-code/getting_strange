...
func compare_detour_time() -> bool:
	...
	# CR-D (PKG-0196 §6): czas podaje raz znak robót i poprzedni odczyt;
	# tutaj gest telefonu zamiast drugiej deklaracji dwunastu minut.
	_present([
		{"speaker": "LENA", "text": "Marta poczeka dłużej, niż obiecałam. Idę nasypem."},
	])
	queue_redraw()
	return true
...
func _draw_outdoor_detour() -> void:
	...
	# PKG-0179: Service narrowing constriction (width 48 px, clearance 105 px above floor)
	draw_rect(Rect2(250.0, 58.0, 48.0, 133.0), VectorStageStyle.shade(VectorStageStyle.DEEP_PLANE, 0.40))
	draw_rect(Rect2(250.0, 58.0, 48.0, 133.0), VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.60), false, 2.0)
	draw_line(Vector2(250.0, 191.0), Vector2(298.0, 191.0), VectorStageStyle.HUMAN_AMBER, 2.0)
	draw_colored_polygon(PackedVector2Array([
	...
	# Landing is world architecture. The climbable ladder is drawn only by LadderZone (D-190).
	draw_rect(Rect2(418.0, 232.0, 198.0, 24.0), VectorStageStyle.MID_PLANE)
	# PKG-0198 (ZERO wycinek 2): cień kontaktowy lądowiska w prawo, 0.48,
	# zgodnie z lampą roboczą gantry (196,104) i konwencją 01/06/08.
	draw_colored_polygon(PackedVector2Array([
		Vector2(410.0, 254.0), Vector2(622.0, 250.0),
		Vector2(630.0, 258.0), Vector2(418.0, 262.0),
	...
	draw_line(Vector2(418.0, 232.0), Vector2(616.0, 232.0), route_color, 2.0)
