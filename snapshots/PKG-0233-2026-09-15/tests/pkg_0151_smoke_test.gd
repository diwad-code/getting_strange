...
func _test_selector_routes(state: Node) -> void:
	...
	for branch in ["A", "B", "C"]:
		...
		_expect(state.get_next_campaign_station(&"station_18") == finale_id, "station_18 must route to %s for branch %s" % [String(finale_id), branch])
		_expect(state.get_next_campaign_station(finale_id) == &"station_43", "%s must route to station_43" % String(finale_id))
		_expect(state.get_previous_campaign_station(finale_id) == &"station_18", "%s previous must be station_18" % String(finale_id))
		...
		_expect(selectable.size() == 20, "Selectable route must expose 20 narrative addresses for branch %s" % branch)
		_expect(selectable[18] == finale_id, "Selectable route index 18 must swap in %s for branch %s" % [String(finale_id), branch])
		_expect(selectable[19] == &"station_43", "Selectable route must always end at station_43")
