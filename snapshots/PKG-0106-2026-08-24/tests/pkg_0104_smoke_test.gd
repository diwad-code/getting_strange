...
func _run() -> void:
	...
	if _failures.is_empty():
		print("PKG-0104 SMOKE PASS: Act IIIb Vector-Stage route, R6/R1 mechanisms and chain boundary 25")
		quit(0)
	else:
...
func _check_campaign_boundary(state: Node) -> void:
	...
	_expect(not state.has_reached_station(&"station_26"), "PKG-0104 must not raise the delivered campaign boundary")
