...
func _physics_process(delta: float) -> void:
	...
	if now_on_floor and is_on_wall() and not is_zero_approx(horizontal_input):
	...
	if visual_rig:
		visual_rig.set_mechanical_state(velocity, now_on_floor, false, is_sprinting)
...
func _emit_run_dust() -> void:
	var particles := get_node_or_null("RunDust") as CPUParticles2D
	if particles:
		particles.emitting = is_sprinting and absf(velocity.x) > movement_profile.move_speed * 0.85
...
func try_curb_step(direction: float) -> bool:
	if is_zero_approx(direction) or is_climbing:
		return false
