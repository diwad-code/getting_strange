...
## PKG-0165 normal-driver evidence for Station 17.
## Three textless M2 state frames (neutral / ledger read / offer rejected) and one
## true-grayscale M3 structure frame after the limited consent scope. This is
## structural evidence only; it does not claim human comprehension, emotion,
## comfort or PRODUCT GO.

const PACKAGE_ID := "PKG-0165"
const LOGICAL_SIZE := Vector2i(640, 360)
const OUTPUT_ROOT := "res://reports/pkg_0165"
const REPORT_PATH := "res://reports/pkg_0165/visual_evidence_report.txt"
const WAIT_FRAMES := 20

const CASES: Array[Dictionary] = [
	{"id": "neutral", "mono": false},
	{"id": "ledger_read", "mono": false},
	{"id": "offer_rejected", "mono": false},
...
func _capture_one(capture: Dictionary, state: Node) -> void:
	...
	if packed == null:
		_failures.append("cannot load station_17")
		return
	var station := packed.instantiate() as CanvasItem
	if station == null:
		_failures.append("station_17 root is not CanvasItem")
		return
	root.add_child(station)
	for _frame in range(WAIT_FRAMES):
		await physics_frame
	if not _apply_state(station, String(capture["id"])):
		_failures.append("cannot apply station_17/%s" % String(capture["id"]))
	else:
...
func _save_frame(image: Image, capture: Dictionary, hidden_items: int) -> void:
	...
	var relative_path := OUTPUT_ROOT.path_join("station_17_%s.png" % String(capture["id"]))
...
func _apply_state(station: CanvasItem, state_id: String) -> bool:
	match state_id:
		"neutral":
			return true
		"ledger_read":
			return bool(station.call("read_cost_ledger"))
		"offer_rejected":
			return bool(station.call("read_cost_ledger")) \
				and bool(station.call("reject_adaptation_offer"))
		"structure_mono":
			return bool(station.call("read_cost_ledger")) \
				and bool(station.call("reject_adaptation_offer")) \
				and bool(station.call("record_jakub_consent_limited"))
	return false
...
func _write_report() -> void:
	...
	var lines := PackedStringArray([
		"PKG-0165 VISUAL EVIDENCE REPORT",
		"Renderer: %s" % RenderingServer.get_video_adapter_name(),
		"Viewport: 640x360; normal Windows display driver; script was not headless.",
		"Three textless M2 frames of Station 17: neutral, cost ledger read, adaptation offer rejected.",
		"One M3 true-grayscale structure frame after the limited consent scope was recorded.",
