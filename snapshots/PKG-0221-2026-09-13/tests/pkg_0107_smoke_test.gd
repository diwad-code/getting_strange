...
func _run() -> void:
	...
	if state:
		...
		_expect(int(state_constants.get("CAMPAIGN_TRANSITION_LIMIT", -1)) == 18, "campaign transition limit must cover Station 01..18")
...
func _exercise_existing_airlock(station: Node, player: PrototypePlayer, airlock_position: Vector2 = Vector2(610.0, 248.0)) -> void:
	...
	if station.get_node_or_null("Threshold") != null:
		...
		for _frame in range(5):
			await physics_frame
	if bool(station.get("is_level_completed")) and _completion_count == 0:
		_completion_count = 1
