...
func _test_marta_portrait_identity() -> void:
	...
	for y in range(0, marta.get_height(), 8):
		for x in range(0, marta.get_width(), 8):
			...
			if pixel.a < 0.4:
				continue
			if pixel.r > 0.42 and pixel.r > pixel.g + 0.05 and pixel.r > pixel.b and pixel.g < 0.58:
				pink_pixels += 1
...
func _test_campaign_route_has_no_primitive_people() -> void:
	for station_id in CAMPAIGN_STATIONS:
		...
		if FileAccess.file_exists(gd_path):
			...
			_expect(
				not source.contains("draw_circle(Vector2(420.0, 205.0)"),
				"%s must not paint the kiosk vendor circle-head at (420, 205)" % station_id
			)
			_expect(
				not source.contains("draw_circle(Vector2(340.0, 205.0)"),
				"%s must not paint the stairwell neighbour circle-head at (340, 205)" % station_id
			)
