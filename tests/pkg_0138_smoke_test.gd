...
func _test_softlock_resilience() -> void:
	...
	if p13:
		...
		if gsm_run and not gsm_run.decisions.has(&"p9.threshold_obstacle.marta_threshold.independent_questions_ready"):
			gsm_run.record_decision(&"p9.threshold_obstacle.marta_threshold.independent_questions_ready", true)
...
func _test_bidirectional_and_backtrack() -> void:
	...
	for s_id in stations_to_test:
		...
		if player:
			...
			for _i in 12:
				await physics_frame
			# PKG-0221 (D-234): dojscie ustawia zasieg; powrot idzie interactem.
			_expect(previous_requested.is_empty(), "%s ReturnZone overlap alone must not emit" % s_id)
			var ret_zone := _find(st, func(n): return String(n.name) == "ReturnZone") as ReturnZone
			if ret_zone != null:
				ret_zone.trigger_return(player)
			_expect(not previous_requested.is_empty(), "%s ReturnZone must emit previous_level_requested via interact" % s_id)
...
func _seed_sample_facts(gsm: Node) -> void:
	...
	gsm.record_decision(&"p9.threshold_obstacle.foreign_daily_life.neighbour_account", "twelve_lower_fourteen_home")
	gsm.record_decision(&"p9.threshold_obstacle.marta_threshold.independent_questions_ready", true)
	...
	## Station 16's donor entry is explicit; the package must not depend on
	## the generic prop walk having completed Station 15 first.
	gsm.record_decision(&"p7.work_history_and_record.own_record_requested", true)
	...
	## Station 18 (P9 BUNDLE-25) wymaga donor faktów Station 17; nie fabrykuje
	## wyboru metody z samego wejścia. Stacje 19–21 pozostają dawcą S07 i
	## potrzebują jawnego mostu publicznego, którego Station 18 już nie tworzy.
	gsm.record_decision(&"p7.work_history_and_record.trace", "cost_ledger_and_consent_scope_recorded")
	gsm.record_decision(&"p9.consent_and_cost.cost_ledger_read", true)
	gsm.record_decision(&"p9.consent_and_cost.adaptation_offer", "rejected")
	gsm.record_decision(&"p9.consent_and_cost.jakub_consent_scope", "limited")
	gsm.record_decision(&"p7.three_place_proofs.public_trial_result", "two_systems_and_nine_years")
	gsm.record_decision(&"jakub_public_history_verified", true)
	gsm.record_decision(&"recognition_evidence_public", true)
	## Station 42A (P9 PHASE-06) wymaga donor method_committed = force_home.
	## Station 18 w tej samej próbce może nadpisać metodę; seedujemy ponownie
	## bezpośrednio przed otwarciem 42A w pętli playthrough.
	...
	for s_id in SAMPLE_STATIONS:
		...
		if player:
			...
			if st.get_node_or_null("Threshold") != null:
				preload("res://scripts/environment/threshold_binder.gd").complete_from_test(st, player)
				await process_frame
			elif airlock:
...
func _perform_station_actions(st: Node2D, player: CharacterBody2D) -> void:
	...
	if id.contains("01"):
		if st.has_method(&"repeat_line_four_measurement"):
			st.call(&"repeat_line_four_measurement")
		if st.has_method(&"secure_raw_sample"):
			st.call(&"secure_raw_sample")
		if st.has_method(&"read_marta_message"):
			st.call(&"read_marta_message")
	elif id.contains("07"):
