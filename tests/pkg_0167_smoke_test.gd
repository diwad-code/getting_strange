...
func _test_incomplete_attempt_keeps_exit(state: Node) -> void:
	...
	# pełnym łańcuchu (wykonanie → stan → skutek). Sam rygiel nie domyka.
	_expect(not station.is_level_completed, "Niepełna próba (sam rygiel) nie domyka progu 43")
	_expect(String(station.get("last_feedback")) == "finale_sequence_incomplete", "Próg bez łańcucha nazywa lukę")
	_expect(station.read_sealed_other_lena(), "Stan po wykonaniu przechodzi")
	_expect(station.read_household_consequence(), "Skutek po stanie przechodzi")
	_ThresholdBinder.complete_from_test(station, player)
	await physics_frame
	await physics_frame
	_expect(station.is_level_completed, "Pełny łańcuch 42A domyka się w AirlockZone")
	await _close_station(station)
