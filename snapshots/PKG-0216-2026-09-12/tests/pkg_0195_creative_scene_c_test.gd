...
func run() -> void:
	...
	await run_b(state)
	var joined_b := "\n".join(delivered)
	for required in ["Jej adres wrócił", "Wygaszam domową sygnaturę", "po jej powrocie", "Co wiedziałaś przed testem?", "Próbę zrobiłam sama", "twój oddech", "wiacie z linii 03", "brak w sieci", "bez szczegółu próby", "swoim zakresie", "Eksport kosztów"]:
	...
	ordered(joined_b, "Co wiedziałaś", "wiacie z linii 03", "42B recovery scene must precede the arrived perspective cut")
