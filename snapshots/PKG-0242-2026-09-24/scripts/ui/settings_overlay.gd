...
## PKG-0141 (D-151). Jeden przelacznik dostepnosci dla calego ruchu
## peryferyjnego kampanii. Stoi w tej samej kolumnie co skala tekstu, bo
## nalezy do tej samej rodziny decyzji: jak gra ma byc czytana, nie jak ma byc
## grana.
...
func _build_interface() -> void:
	...
	_master_label = _add_label("MasterVolumeLabel", "", Vector2(18.0, 62.0), Vector2(180.0, 16.0), 9, VectorStageStyle.LIGHT_PLANE)
	_master_volume_slider = _add_slider("MasterVolumeSlider", Vector2(18.0, 78.0), 0.0, 1.0, 0.01)
	_master_volume_value = _add_label("MasterVolumeValue", "", Vector2(204.0, 75.0), Vector2(65.0, 20.0), 9, VectorStageStyle.ANCHOR_CYAN)

	_text_speed_label = _add_label("TextSpeedLabel", "", Vector2(18.0, 102.0), Vector2(205.0, 16.0), 9, VectorStageStyle.LIGHT_PLANE)
	_text_speed_slider = _add_slider("TextSpeedSlider", Vector2(18.0, 118.0), 10.0, 90.0, 1.0)
	_text_speed_value = _add_label("TextSpeedValue", "", Vector2(204.0, 115.0), Vector2(65.0, 20.0), 9, VectorStageStyle.ANCHOR_CYAN)

	_text_scale_label = _add_label("TextScaleLabel", "", Vector2(18.0, 142.0), Vector2(205.0, 16.0), 9, VectorStageStyle.LIGHT_PLANE)
	_text_scale_slider = _add_slider("TextScaleSlider", Vector2(18.0, 158.0), 0.85, 1.15, 0.05)
	_text_scale_value = _add_label("TextScaleValue", "", Vector2(204.0, 155.0), Vector2(65.0, 20.0), 9, VectorStageStyle.ANCHOR_CYAN)

	_fullscreen_check = CheckButton.new()
	_fullscreen_check.name = "FullscreenCheckButton"
	_fullscreen_check.position = Vector2(14.0, 182.0)
	_fullscreen_check.size = Vector2(260.0, 24.0)
	...
	# PKG-0141 (D-151). Przelacznik ograniczonego ruchu.
	_reduced_motion_check = CheckButton.new()
	_reduced_motion_check.name = "ReducedMotionCheckButton"
	_reduced_motion_check.position = Vector2(14.0, 207.0)
	_reduced_motion_check.size = Vector2(260.0, 24.0)
	_reduced_motion_check.focus_mode = Control.FOCUS_ALL
	_mark_scalable(_reduced_motion_check, 9)
	_reduced_motion_check.toggled.connect(_on_reduced_motion_toggled)
	_style_check_button(_reduced_motion_check)
	add_child(_reduced_motion_check)

	_remap_button = _add_button("RemapButton", "", Vector2(18.0, 234.0), Vector2(256.0, 24.0), _open_remap)
	_settings_hint = _add_label("SettingsHint", "", Vector2(18.0, 261.0), Vector2(256.0, 22.0), 8, VectorStageStyle.shade(VectorStageStyle.LIGHT_PLANE, 0.20))
	_settings_hint.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_settings_back_button = _add_button("SettingsBackButton", "", Vector2(18.0, 285.0), Vector2(256.0, 24.0), close_panel)
...
func _refresh_localized_text() -> void:
	...
	_reduced_motion_check.text = _tr("SETTINGS_REDUCED_MOTION")
	_reduced_motion_check.tooltip_text = _tr("SETTINGS_REDUCED_MOTION_HINT")
...
func _sync_controls() -> void:
	...
	_reduced_motion_check.set_pressed_no_signal(bool(_game_state.reduced_motion))
	_refresh_remap_rows()


func _update_focus_chain() -> void:
	var controls: Array[Control] = [_locale_option, _master_volume_slider, _text_speed_slider, _text_scale_slider, _fullscreen_check, _reduced_motion_check, _remap_button, _settings_back_button]
...
func _on_reduced_motion_toggled(enabled: bool) -> void:
	if _game_state:
		_game_state.set_reduced_motion(enabled)
