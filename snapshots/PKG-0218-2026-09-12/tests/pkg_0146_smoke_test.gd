...
func _test_early_save_migration(state: Node) -> void:
	...
	# PKG-0191: `marta_relationship_disclosed` graduated from P7-only dialogue
	# contamination to a permanent CAMPAIGN_MAP canonical fact with a single
	# audited writer (`station_08.gd`'s `speak_with_neighbour()`). The
	# migration table no longer erases this name, so it now survives a
	# reload like any other current-state decision.
	_expect(state.decisions.get(&"marta_relationship_disclosed", false) == true, "PKG-0191: marta_relationship_disclosed is canonical and must survive migration, not be erased as P7 dialogue")
	for spec in SEQUENCES:
		var sequence_id := spec["id"] as StringName
...
func _test_s04_foreign_daily_life(state: Node) -> void:
	...
	if station == null:
		return
	# PKG-0192: station_09's real ask_neighbour_without_leading() writes the
	# now-out-of-scope p7.foreign_daily_life.neighbour_account (station_09.gd
	# is not part of this package's retirement); station_10's own gate reads
	# the renamed p9.threshold_obstacle.* key, so this isolated S04 scenario
	# seeds it directly, exactly like the other diagnostic waves already do.
	...
	_expect(state.decisions.get(&"p9.threshold_obstacle.foreign_daily_life.trace", "") == "foreign_address_and_photograph", "S04 musi utrwalić obcy adres i fotografię")
...
func _test_s05_marta_threshold(state: Node) -> void:
	...
	_expect(state.decisions.get(&"p9.threshold_obstacle.marta_threshold.safe_trial_feedback", "") == "balcony_open", "Błędna próba S05 musi dać fakt bez usuwania wiadomości")
