...
var _pass_progress := 0.0
var pass_progress: float:
	get:
	...
	set(value):
		...
		if _pass_progress == clamped:
			return
		_pass_progress = clamped
		queue_redraw()

## PKG-0179: Sensory interference factor (0.0 to 1.0) based on Lena's proximity.
## Splits the CRT beam into two phases with micro-jitter (1.5–3.0 px offset).
var _interference_factor := 0.0
@export var interference_factor: float = 0.0:
	get:
		return _interference_factor
	set(value):
		var clamped := clampf(value, 0.0, 1.0)
		if _interference_factor == clamped:
			return
		_interference_factor = clamped
		queue_redraw()
...
func _draw() -> void:
	...
	if points.size() >= 2:
		...
		if interference_factor > 0.01:
			var split_offset := 1.5 + interference_factor * 1.5
			var shadow_points := PackedVector2Array()
			var shadow_color := Color(trace_color.r, trace_color.g, trace_color.b, trace_color.a * 0.48 * interference_factor)
			for p in points:
				shadow_points.append(Vector2(p.x, p.y + split_offset))
			draw_polyline(shadow_points, shadow_color, 1.0)
