...
func _test_mutual_test_save_migration(state: Node) -> void:
	...
	_expect(state.decisions.get(&"world_recognized", false) == true, "Migracja musi zachować wcześniejsze fakty")
	...
	_expect(
		not state.decisions.has(&"mechanic_cost_observed") and not state.decisions.has(&"marta_boundary_accepted"),
		"Legacy koszt i zgoda nie mogą nadać stanu wycinka P7"
	)
	_expect(
