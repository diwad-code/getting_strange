...
func _test_station_01_traversal() -> void:
	...
	if file != null:
		...
		_expect(content.contains("OperatorLadder"), "OperatorLadder must exist in Station 01")
		_expect(content.contains("ConsoleLadder"), "ConsoleLadder must exist in Station 01")
		_expect(content.contains("one_way_collision = true"), "one_way_collision must be configured for platforms in Station 01")
