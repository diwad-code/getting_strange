...
Stan na: 2026-08-26 po PKG-0133 (chód, śluza 01, interakcja bez pętli)
...
**P6: Human Scale & Playability — OTWARTA (PKG-0132 i PKG-0133 zamknięte).**

Decyzje D-121..D-127. Runtime po PKG-0133:

1. Lena 4.0 sprite 87 px; kapsuła 72 × 16.
2. Chód jest defaultem. `sprint` (Shift / LB) to jedyny bieg (D-127).
3. Stacja 01: po procedurze collider drzwi jest `disabled`, skrzydło +140 px.
4. Interakcja: po sukcesie cyjanowy haczyk, zero pulsującego retikula; one-shot.
5. ReturnZone 02–43, próg 18 px, 0 blockerów. Zero nowych `.exe` (D-125).

Kanon skali: `docs/WORLD_SCALE.md`.
Kanon traweru 1.1 + D-127: `docs/TRAVERSAL_AND_OBSTACLE_DESIGN.md`.
Kanon Leny 4.0: `docs/LENA_CHARACTER_AND_ANIMATION.md`.

Nastepny pakiet: **PKG-0134** (capture okiem, one_way 09/11, chód 05→04→03).
...
- `tests/pkg_0133_smoke_test.gd`: **PASS** (walk@96, run tylko ze sprintem, drzwi 01 disabled, one-shot).
- `tests/pkg_0132_smoke_test.gd`: **PASS** (regresja).
- `tests/traversal_lint_test.gd`: **PASS**.
- `tools/geometry_audit.gd`: 45/45, 0 blockerów.
- Pełne `tools/verify.ps1` po zamknięciu pakietu.

## Ostatnia swieza weryfikacja

- Data: 2026-08-26 po PKG-0133
- Wynik `pwsh -NoProfile -File "tools/verify.ps1"`: PASS (docs + smoke + 0095–0124 w pierwszym przebiegu; 0125–0133 po korekcie asercji chodu D-127).
...
- Capture Station 01 Lena+krzesło+drzwi na **normalnym sterowniku Windows** (H-027; ADR-003).
- Sterowany chód 05→04→03 na ekranie.
- One_way meble 09/11 nadal da się nadskoczyć; nie blokują audytu 18 px.
- Flux 2 Pro idle jako ewentualna wymiana tożsamości.
- Reduced-motion i nazwa wezla kamery — odroczone.
- Testy na Steam Deck / AMD / NVIDIA.

## Nastepny pakiet

- **PKG-0134**: capture okiem, one_way 09/11, weryfikacja 05→04→03. Prompt: `docs/NEXT_SESSION_PROMPT.md`.
