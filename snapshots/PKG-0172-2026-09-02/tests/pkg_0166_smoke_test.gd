...
func _test_safe_rejections(state: Node) -> void:
	...
	_expect(not station.commit_force_home(), "Zatwierdzenie przed zestawieniem musi być bezpieczne")
	_expect(state.decisions.get(FEEDBACK_FACT, "") == "forecast_and_consent_inventory_required", "Wczesne zatwierdzenie musi dać informację o brakach")
	_expect(not state.decisions.has(MARTA_FACT), "Wczesna prawda nie może zapisać stanu Marty")
	_expect(not state.decisions.has(METHOD_FACT), "Wczesne zatwierdzenie nie może zapisać metody")
	...
	_expect(not station.is_exit_unlocked, "Samo zestawienie prognoz nie może otworzyć wyjścia")
	_expect(not station.commit_close_equal(), "Zatwierdzenie bez stanu Marty musi pozostać informacyjne")
	_expect(state.decisions.get(FEEDBACK_FACT, "") == "marta_truth_required", "Brak stanu Marty musi zostać nazwany")
	await _close_station(station)
...
func _test_refused_and_withheld_path(state: Node) -> void:
	...
	_expect(station.commit_mutual_passage(), "Odmowa Jakuba i wstrzymanie Marty nie mogą zablokować zatwierdzenia")
	_expect(state.decisions.get(CANONICAL_METHOD_FACT, "") == METHOD_MUTUAL, "Kanoniczna metoda przy odmowie to mutual_passage")
	_expect(station.is_exit_unlocked, "Odmowa i wstrzymanie nie mogą softlockować wyjścia")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player != null:
		player.global_position = Vector2(606.0, 238.0)
		await physics_frame
	...
	_expect(decisions_text.contains(METHOD_MUTUAL), "Zapis musi być JSON-safe dla zatwierdzonej metody")
	await _close_station(station)
