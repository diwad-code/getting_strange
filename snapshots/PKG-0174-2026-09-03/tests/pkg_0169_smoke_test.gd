...
func _test_scene_contract(state: Node) -> void:
	...
	_expect(not station.is_exit_unlocked, "Wejście bez mutual_passage nie może otworzyć drogi do 43")
...
func _test_donor_entry_requirement(state: Node) -> void:
	...
	_expect(not station.is_exit_unlocked, "Brak mutual_passage nie może odblokować wyjścia")
	await _close_station(station)
...
func _test_wrong_method_rejection(state: Node) -> void:
	...
	_expect(not station.is_exit_unlocked, "Obca metoda nie może otworzyć 42C do 43")
	await _close_station(station)
...
func _test_incomplete_attempt_keeps_exit(state: Node) -> void:
	...
	if player != null:
		...
		await physics_frame
		await physics_frame
	_expect(station.is_level_completed, "Niepełna próba w wybranym 42C musi dać się domknąć w AirlockZone")
	await _close_station(station)
