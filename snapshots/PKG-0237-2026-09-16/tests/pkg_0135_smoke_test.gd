...
func _test_backtrack_05_04_03() -> void:
	...
	var back_requested_s05: Array = []
	st05.connect(&"previous_level_requested", func(): back_requested_s05.append(true))
	if rz05:
		rz05.connect(&"return_requested", func(): back_requested_s05.append(true))
	...
	if p05:
		...
		for _i in 50:
			if not back_requested_s05.is_empty():
				break
			p05.velocity = Vector2(-160.0, 0.0)
			p05.move_and_slide()
			await physics_frame
		# PKG-0221 (D-234): sam overlap nie wraca — dojscie ustawia zasieg,
		# powrot idzie czasownikiem interact (trigger_return).
		if rz05 != null and back_requested_s05.is_empty():
			rz05.trigger_return(p05)
	_expect(not back_requested_s05.is_empty(), "Station 05 triggers previous_level_requested at ReturnZone via interact")
	st05.queue_free()
	await process_frame
	...
	var back_requested_s04: Array = []
	st04.connect(&"previous_level_requested", func(): back_requested_s04.append(true))
	if rz04:
		rz04.connect(&"return_requested", func(): back_requested_s04.append(true))
	...
	if p04:
		...
		while frames > 0:
			...
			if absf(dx) <= 6.0 or not back_requested_s04.is_empty():
				break
			...
			await physics_frame
		# ReturnZone (28 x 220 @ x=18, D-124) ustawia zasieg, gdy krawedz kapsuly
		# dotknie strefy; sam powrot idzie interactem (PKG-0221/D-234).
		_expect(p04.global_position.x <= 60.0, "Station 04 player walked left to ReturnZone (at x=%.1f)" % p04.global_position.x)

	for _i in 5:
		await physics_frame
	# PKG-0221 (D-234): dojscie ustawia zasieg, powrot idzie interactem.
	if rz04 != null and back_requested_s04.is_empty():
		rz04.trigger_return(p04)
	_expect(not back_requested_s04.is_empty(), "Station 04 triggers previous_level_requested at ReturnZone via interact")
	st04.queue_free()
	await process_frame
	...
	var back_requested_s03: Array = []
	st03.connect(&"previous_level_requested", func(): back_requested_s03.append(true))
	if rz03:
		rz03.connect(&"return_requested", func(): back_requested_s03.append(true))
	...
	if p03:
		...
		while frames > 0:
			...
			if absf(dx) <= 6.0 or not back_requested_s03.is_empty():
				break
		...
		# ReturnZone (28 x 220 @ x=18, D-124) ustawia zasieg, gdy krawedz kapsuly
		# dotknie strefy; sam powrot idzie interactem (PKG-0221/D-234).
		_expect(p03.global_position.x <= 60.0, "Station 03 player walked left to ReturnZone (at x=%.1f)" % p03.global_position.x)

	for _i in 5:
		await physics_frame
	# PKG-0221 (D-234): dojscie ustawia zasieg, powrot idzie interactem.
	if rz03 != null and back_requested_s03.is_empty():
		rz03.trigger_return(p03)
	_expect(not back_requested_s03.is_empty(), "Station 03 triggers previous_level_requested at ReturnZone via interact")
	st03.queue_free()
	await process_frame
	if gsm_node:
