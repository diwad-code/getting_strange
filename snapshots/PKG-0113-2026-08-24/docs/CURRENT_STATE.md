...
-   Nie ma repozytorium, galezi, commita ani historii. Nie uruchamiamy `git`. Ostatni zamkniety pakiet: `PKG-0113`, 2026-08-24
...
-   ostatnie: `snapshots/PKG-0113-2026-08-24`
...
- Domknięcie pełnej pętli gry (Menu Główne, ekran pauzy, system punktów
  ...
  PKG-0113 jest to najwyższy P0: menu główne i ciąg 26..43 jeszcze nie istnieją.
...
### Stan wydawniczy po PKG-0113

**Projekt nie jest gotowy do publicznego wydania.** Wszystkie sceny 01..43
istnieją i pełny smoke rzeczywiście je wywołuje, ale:

- `run/main_scene` nadal wskazuje `scenes/prototype/movement_lab.tscn`;
- nie istnieje produkcyjny ekran tytułowy ani Nowa gra/Kontynuuj;
- `CAMPAIGN_TRANSITION_LIMIT = 25`, więc normalny ciąg nie dochodzi do
  Station 26..41, wyboru 42A/B/C i epilogu 43;
- brak `export_presets.cfg`, buildów Windows/Linux i testu czystej instalacji;
- brak kompletnego ekranu ustawień, pełnego pada, remapu i rzeczywistej
  integracji PL/EN.
...
- Nie ma produkcyjnego menu głównego, pełnego ciągu kampanii 26..43,
  `export_presets.cfg`, buildów Windows/Linux ani testu czystej instalacji.
- Dialogi runtime istnieją, ale nie ma kompletnej integracji PL/EN, ustawień
  tempa/skali tekstu, historii dialogu ani pełnego zestawu portretów obsady.
...
`PKG-0114: R0 — produkcyjny shell, pełna topologia kampanii i test end-to-end`

Nowa sesja zaczyna od świeżego `tools/verify.ps1` i wykonuje aktualny
`docs/NEXT_SESSION_PROMPT.md`. Priorytetem jest właściwy punkt wejścia, menu
główne i bezpieczne przejście 01..41 → 42A/B/C → 43 z Nowa gra/Kontynuuj oraz
testem realnego ciągu. Nie zaczyna od eksportu, marketingu, nowych przeszkód
ani kolejnego ogólnego passu grafiki. Każda zmiana limitu/topologii i schematu
zapisu wymaga migracji, testu i decyzji; Godot-only, brak Git i D-099 pozostają
twarde.
