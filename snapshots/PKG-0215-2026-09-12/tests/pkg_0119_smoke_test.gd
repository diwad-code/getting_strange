...
func _test_station_08() -> void:
	...
	await process_frame
	_expect(
		station.get_node_or_null("ApartmentDoor14") is AnimatableBody2D
		or station.get_node_or_null("BuildingEntranceDoor") is AnimatableBody2D
		or station.get_node_or_null("Geometry/BuildingEntranceDoor") is AnimatableBody2D,
		"Station 08 musi mieć drzwi wejściowe jako ciało fizyczne"
	)
	...
	_expect(not station.get("is_level_completed"), "Dwa źródła nie wystarczą do zakończenia sceny")
...
func _test_station_10() -> void:
	...
	_expect(not station.get("is_level_completed"), "Sam klucz nie może zakończyć sceny odruchowo")
	var guidance := station.get_node_or_null("NarrativeGuidanceService") as NarrativeGuidanceService
	if guidance:
		_expect(
...
func _test_station_13() -> void:
	...
	if state:
		state.reset_campaign(true)
		state.record_decision(&"p9.threshold_obstacle.marta_threshold.independent_questions_ready", true)
