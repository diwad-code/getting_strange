...
func _draw_state_layer() -> void:
	...
	var checklist_col := VectorStageStyle.shade(VectorStageStyle.CORRECTION_OXIDE, 0.72)
	if is_procedure_completed:
		checklist_col = VectorStageStyle.ANCHOR_CYAN
	draw_line(Vector2(42.0, 224.0), Vector2(178.0, 224.0), checklist_col, 2.0)
	var checklist := [circuit_alpha_on, circuit_beta_on, circuit_gamma_on, vacuum_checked]
	for idx in range(4):
		var complete: bool = bool(checklist[idx])
		var col := VectorStageStyle.ANCHOR_CYAN if complete else VectorStageStyle.MID_PLANE
		draw_rect(Rect2(54.0 + float(idx) * 28.0, 214.0, 14.0, 6.0), col)
	var door_col := VectorStageStyle.ANCHOR_CYAN if is_procedure_completed else VectorStageStyle.CORRECTION_OXIDE
