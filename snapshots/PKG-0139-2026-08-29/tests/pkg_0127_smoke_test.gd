...
func _test_bidirectional_graph_and_continuity() -> void:
	...
	_expect(gsm.get_next_campaign_station(&"station_41") == &"station_42a", "station_41 with Op A must lead to station_42a")
	_expect(gsm.get_next_campaign_station(&"station_42a") == &"station_43", "station_42a must lead to station_43")
	_expect(gsm.get_previous_campaign_station(&"station_42a") == &"station_41", "station_42a previous must be station_41")
	...
	_expect(gsm.get_next_campaign_station(&"station_41") == &"station_42b", "station_41 with Op B must lead to station_42b")
	...
	_expect(gsm.get_next_campaign_station(&"station_41") == &"station_42c", "station_41 with Op C must lead to station_42c")
	_expect(gsm.get_previous_campaign_station(&"station_43") == &"station_42c", "station_43 with Op C must backtrack to station_42c")

	# 4. Save and Restore
	gsm.mark_station_reached(&"station_18")
	gsm.collect_clue(&"ucp_record_18")
	gsm.record_decision(&"test_decision", "verified")
	gsm.set_checkpoint(&"station_18", Vector2(120.0, 280.0))
	_expect(gsm.save_campaign() == true, "save_campaign must succeed")

	gsm.reload_campaign_from_disk()
	_expect(gsm.has_reached_station(&"station_18"), "Reached station_18 must persist across save/reload")
	...
	_expect(gsm.last_checkpoint_station == &"station_18", "Checkpoint station must be station_18")
	_expect(gsm.last_checkpoint_position == Vector2(120.0, 280.0), "Checkpoint position must be (120, 280)")
