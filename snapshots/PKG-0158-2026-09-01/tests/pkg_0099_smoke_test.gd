...
func _check_station_14_anchor() -> void:
	...
	if gsm:
		...
		gsm.record_decision(&"p7.marta_threshold.recall_requested", true)
	var station := packed.instantiate() as Station14
	root.add_child(station)
	await process_frame
	_expect(station.place_bag_at_door(), "bag must be placed at door")
	_expect(station.is_bag_placed, "bag must be placed at door")
	_expect(station.disclose_arrival_time(), "arrival time must be disclosed")
	_expect(station.compare_field_equipment(), "field equipment must be compared")
	_expect(station.verify_key_position(), "key position must be verified")
	_expect(station.ask_independent_day_description(), "independent day description must commit")
	_expect(station.get("is_exit_unlocked") == true, "threshold must open after the comparison")
	var gsm_state := root.get_node_or_null("GameStateManager")
	if gsm_state:
		_expect(
			gsm_state.decisions.get(&"p7.marta_threshold.trace", "") == "relationship_difference_explicit",
			"the relationship difference must be written into campaign decisions"
		)
	station.queue_free()
	await process_frame
...
func _check_station_12_obstacle() -> void:
	...
	if gsm:
		gsm.reset_campaign(true)
		gsm.record_decision(&"p7.foreign_daily_life.trace", "foreign_address_and_photograph")
	var station := packed.instantiate()
	root.add_child(station)
	await process_frame
	...
	if leaf is AnimatableBody2D:
		...
		if gsm_state2:
			_expect(
				gsm_state2.decisions.get(&"p7.marta_threshold.safe_trial_feedback", "") == "balcony_open",
