...
func _run() -> void:
	...
	if state:
		...
		_expect(
			...
			"this package must preserve the delivered campaign transition limit"
		)
		_expect(
	...
	if _failures.is_empty():
		print("PKG-0106 SMOKE PASS: Station 41 Vector-Stage choice chamber, conscious silence and chain boundary 25")
		quit(0)
	else:
...
func _check_campaign_boundary(state: Node) -> void:
	...
	_expect(not state.has_reached_station(&"station_26"), "PKG-0106 must not raise the delivered campaign boundary")
