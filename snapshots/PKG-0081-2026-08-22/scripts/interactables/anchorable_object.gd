...
func _draw() -> void:
	...
	var is_vertical_barrier := (_current_size.y > _current_size.x * 1.5)
	...
	if not is_anchored:
		...
		if is_vertical_barrier:
			# Ghost outline for vertical barrier slats
			for gy in range(int(ghost_rect.position.y) + 8, int(ghost_rect.end.y), 12):
				draw_line(Vector2(ghost_rect.position.x, float(gy)), Vector2(ghost_rect.end.x, float(gy)), Color(ghost_color, 0.4), 1.0)

	# 2. Main Platform / Barrier Body
	var is_solid := state_a_solid if current_reality == RealityState.STATE_A else state_b_solid
	var fill_color := COLOR_PLATFORM if is_solid else Color(COLOR_PLATFORM.r, COLOR_PLATFORM.g, COLOR_PLATFORM.b, 0.25)
	draw_rect(body_rect, fill_color, true)

	# 3. Structural Details: specialized for vertical gates vs horizontal platforms
	if is_vertical_barrier:
		# Security Gate: vertical guide rails + horizontal reinforcement slats
		var rail_color := Color("a8b2ac", 0.7 if is_solid else 0.3)
		draw_line(Vector2(-half.x, -half.y), Vector2(-half.x, half.y), rail_color, 1.5)
		draw_line(Vector2(half.x, -half.y), Vector2(half.x, half.y), rail_color, 1.5)
		
		# Horizontal heavy steel interlocking slats
		var slat_color := Color(COLOR_EDGE, 0.8 if is_solid else 0.3)
		for sy in range(int(-half.y) + 8, int(half.y), 10):
			draw_line(Vector2(-half.x + 2.0, float(sy)), Vector2(half.x - 2.0, float(sy)), slat_color, 1.5)
			# Slat rivet
			draw_circle(Vector2(0.0, float(sy)), 1.0, Color("a8b2ac", 0.5 if is_solid else 0.2))

		# Bottom security shoe / locking tooth
		var shoe_color := COLOR_ANCHOR_CYAN if is_anchored else (COLOR_CORRECTION_CINNABAR if is_solid else Color(COLOR_EDGE, 0.4))
		draw_line(Vector2(-half.x, half.y), Vector2(half.x, half.y), shoe_color, 2.5)
		
		# Top optical interlock diode
		var diode_col := COLOR_ANCHOR_CYAN if is_anchored else (COLOR_CORRECTION_CINNABAR if is_solid else Color("a8b2ac", 0.4))
		draw_circle(Vector2(0.0, -half.y + 4.0), 2.0, diode_col)
	else:
		# Horizontal Platform: top walkable surface + edge trim
		...
		# Structural trusses on underside
		if _current_size.x > 60.0:
			var truss_col := Color(COLOR_EDGE, 0.35)
			draw_line(Vector2(-half.x + 10.0, half.y), Vector2(0.0, -half.y + 4.0), truss_col, 1.0)
			draw_line(Vector2(0.0, -half.y + 4.0), Vector2(half.x - 10.0, half.y), truss_col, 1.0)
