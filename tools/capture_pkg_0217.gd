...
## PKG-0217 — capture station_13 / station_18 after the synthesis + forecast +
## device-voice edit (normal Windows driver; headless hangs on
## frame_post_draw, fact from PKG-0197). Output: 12 PNG 640x360
## (2 stations x 3 scales x full/notext) + frames.tsv
## under reports/pkg_0217/visual. No overwrites of 0187..0216 archives.
## Scale via set_text_scale(..., false) without persist, back to 1.0 after.

const OUT_DIR := "res://reports/pkg_0217/visual"
const STATION_IDS: Array[String] = ["station_13", "station_18"]
...
func _run() -> void:
	...
	if state == null:
		push_error("CAPTURE 0217: no GameStateManager autoload")
		quit(1)
		return
	...
	for station_id: String in STATION_IDS:
		...
		if packed == null:
			push_error("CAPTURE 0217: cannot load %s" % station_id)
			continue
		for sci: int in range(SCALE_LABELS.size()):
			...
			if full_image != null:
				var full_name := "%s__pkg0217_s%d_full.png" % [station_id, label]
				full_image.save_png(out_path.path_join(full_name))
				print("CAPTURE 0217 PASS: %s" % full_name)
				index.append("%s\t%d\tfull\t%s" % [station_id, label, full_name])
			else:
				push_error("CAPTURE 0217: no full image %s s%d" % [station_id, label])
			var notext_image := await _capture_variant(packed, true)
			if notext_image != null:
				var notext_name := "%s__pkg0217_s%d_notext.png" % [station_id, label]
				notext_image.save_png(out_path.path_join(notext_name))
				print("CAPTURE 0217 PASS: %s" % notext_name)
				index.append("%s\t%d\tnotext\t%s" % [station_id, label, notext_name])
			else:
				push_error("CAPTURE 0217: no notext image %s s%d" % [station_id, label])
