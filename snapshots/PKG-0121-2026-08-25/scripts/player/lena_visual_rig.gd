...
func _draw() -> void:
	...
	match state:
		State.IDLE:
			...
			head_offset.y = -breath * 0.6
			hip_offset.y = -breath * 0.2
			# Relaxed stance with subtle weight shift to back leg
			leg_left_angle = -0.5 * f
			leg_right_angle = 1.0 * f
		State.START:
			lean = f * 3.5
			hip_offset.y = 1.0
			leg_left_angle = -1.5 * f
			leg_right_angle = 3.0 * f
		State.WALK:
			var cycle := _walk_cycle
			var stride := sin(cycle)
			leg_left_angle = stride * 5.2
			leg_right_angle = -stride * 5.2
			hip_offset.y = absf(sin(cycle * 2.0)) * -1.4
			lean = f * 1.8
		State.RUN:
			var cycle := _walk_cycle
			var stride := sin(cycle)
			leg_left_angle = stride * 8.5
			leg_right_angle = -stride * 8.5
			hip_offset.y = absf(sin(cycle * 2.0)) * -2.4
			lean = f * 4.5
		State.STOP:
			lean = -f * 2.8
			hip_offset.y = 1.2
		...
		State.JUMP_RISE:
			hip_offset.y = -2.5
			head_offset.y = -1.0
			leg_left_angle = 2.5 * f
			leg_right_angle = 3.5 * f
			lean = f * 1.2
		State.JUMP_FALL:
			hip_offset.y = -1.0
			leg_left_angle = -2.5 * f
			...
			lean = f * 0.5
		State.LAND:
			hip_offset.y = _land_compression * 3.5
			head_offset.y = _land_compression * 1.5
			lean = f * 0.8
	...
	# Boots (heavy sole, defined heel & toe)
	var boot_l_poly := PackedVector2Array([
		Vector2(boot_left.x - 3.2 * f, boot_left.y - 3.0),
		Vector2(boot_left.x + 3.2 * f, boot_left.y - 3.0),
		Vector2(boot_left.x + 4.2 * f, boot_left.y + 0.8),
	...
	var boot_r_poly := PackedVector2Array([
		Vector2(boot_right.x - 3.2 * f, boot_right.y - 3.0),
		Vector2(boot_right.x + 3.2 * f, boot_right.y - 3.0),
		Vector2(boot_right.x + 4.2 * f, boot_right.y + 0.8),
	...
	# If performing seam gesture, left hand touches the sleeve seam
	if state == State.SEAM_GESTURE:
		hand_reach_x = seam_x + 1.0 * f
		hand_y = torso_y_top + 6.0
