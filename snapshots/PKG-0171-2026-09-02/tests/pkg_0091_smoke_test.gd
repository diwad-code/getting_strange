...
func _run() -> void:
	...
	if game_state:
		...
		var selectable_stations: Array[StringName] = game_state.get_selectable_stations(true)
		_expect(selectable_stations.size() == 20, "level selection does not expose all 20 stations (%d)" % selectable_stations.size())
	...
	for path in STATION_PATHS:
		...
		var is_rebuilt_opening: bool = path in [
			"res://scenes/levels/station_01.tscn",
			"res://scenes/levels/station_02.tscn",
			"res://scenes/levels/station_03.tscn",
			"res://scenes/levels/station_04.tscn",
		]
		if is_rebuilt_opening:
			_expect(station.get_node_or_null("VectorStageEnvironment") == null, "P9 opening must not retain shared VectorStageEnvironment: %s" % path)
		else:
			_expect(station.get_node_or_null("VectorStageEnvironment") is VectorStageEnvironment, "vector-stage environment is missing: %s" % path)
		_expect(station.get_node_or_null("Player") is PrototypePlayer, "player presentation is missing: %s" % path)
