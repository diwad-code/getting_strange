...
-   Nie ma repozytorium, galezi, commita ani historii. Nie uruchamiamy `git`. Ostatni zamkniety pakiet: `PKG-0099`, 2026-08-24
...
-   ostatnie: `snapshots/PKG-0099-2026-08-24`
...
## Ustalenie audytu PKG-0099: gra nie ma jeszcze przestrzeni grywalnej

To najważniejszy otwarty problem projektu i ma priorytet nad oprawą.

- **38 z 43 przestrzeni ma dokładnie cztery collidery**: `FloorMain`, `Ceiling`,
  `WallLeft`, `WallRight`. Wejście z lewej, marsz w prawo, dotknięcie rekwizytów
  `Area2D`, wyjście. Wyjątki z dodatkową geometrią: Station 01, 02, 03, 04, 11, 12.
- **Centralna mechanika nie jest w kampanii.** Zakotwiczenie/Uległość działa
  wyłącznie w `scenes/prototype/anchor_lab.tscn`. Klasy `AnchorableObject`
  i `MovableAnchorableProp` istnieją i działają, ale żadna z 43 przestrzeni ich nie używa.
- **Pętla 30-sekundowa z `PRODUCT_BRIEF.md` nie jest zaimplementowana nigdzie.**
- Obowiązuje `docs/TRAVERSAL_AND_OBSTACLE_DESIGN.md` (D-099): przeszkody wyłącznie
  diegetyczne, zakaz platformingu zręcznościowego egzekwowany testem
  `tests/traversal_lint_test.gd` w `tools/verify.ps1`.
- Plan naprawy: pionowy plaster grywalności na Station 11..15 (PKG-0099 w handoffie),
  dopiero potem rozszerzenie wzorca.
...
- **Dług techniczny (D-096): Station 01..20 mają zasłonięty Rówień Vector-Stage.**
  Warstwa `VectorStageEnvironment` jest tam obecna, ale skrypty stacji malują nad nią
  własne, nieprzezroczyste tło pełnoekranowe, więc konwersja Aktów I/II/IIb jest
  faktycznie niewidoczna w grze. Smoke testy PKG-0094/0095/0096 tego nie wykrywają,
  bo sprawdzają obecność węzła, a nie widoczność kadru. Wzorzec naprawy jest gotowy
  w Station 21..25 (`_draw_state_layer()`); przeniesienie go na 01..20 jest zakresem PKG-0099.
...
`PKG-0100: Rozszerzenie plastra grywalności na Akt I (Station 06..10) i naprawa widoczności kadru w Station 01..10`

Cel: kontynuacja wyłącznie gry Godot (D-091..D-099):
1. przenieść wzorzec D-096 (`_draw_state_layer()` + `draw_play_plane()`) na
   Station 01..10, bez zmiany colliderów i geometrii;
2. dodać po jednej diegetycznej przeszkodzie z katalogu R do Station 06..10,
   każdą z nagłówkiem testu trzech pytań i modelem porażki przez korektę;
3. rozszerzyć bramkę o kontrolę Aktu I i odnowić rendery Aktu I;
4. zredukować rysunek `AnchorableObject` do 2–4 płaszczyzn Rówień Vector-Stage;
5. uruchomić pełną weryfikację, udokumentować oraz zamrozić `PKG-0100`.
...
plik i `NEXT_SESSION_PROMPT.md`, a następnie przechodzi do realizacji PKG-0100 w silniku Godot 4.7.
