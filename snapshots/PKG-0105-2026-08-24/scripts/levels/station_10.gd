...
## PRZESZKODA — dlaczego to tu jest: Próg techniczny jest zamknięty, dopóki telefon nie potwierdzi tożsamości lokalnej wersji Jakuba.
## PRZESZKODA — czego wymaga od Leny: wysłuchania potwierdzenia przy telefonie i przejścia przez otwarty próg bez wymuszania wersji.
## PRZESZKODA — koszt porażki: korekta odsyła do biurka, a jeden szczegół rozmowy zostaje wyciszony.
...
@onready var identity_gate: AnimatableBody2D = $IdentityGate
...
func _process(delta: float) -> void:
	...
	if identity_gate:
		var target_y := 110.0 if is_airlock_unlocked else 252.0
		identity_gate.position.y = lerpf(identity_gate.position.y, target_y, delta * 7.0)
		queue_redraw()


func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed(&"trigger_correction"):
		_apply_identity_gate_correction()
...
func _check_unlock_conditions() -> void:
	...
	if is_jakub_dialogue_completed and not is_airlock_unlocked:
		...
		if identity_gate:
			identity_gate.position.y = 110.0
		if _door_player:
			_door_player.play()
...
func _on_airlock_entered(body: Node2D) -> void:
	if (body is PrototypePlayer or body.name == "Player") and is_airlock_unlocked and not is_level_completed:
		is_level_completed = true
		level_completed.emit()


func _apply_identity_gate_correction() -> void:
	...
	if state:
		state.record_decision(&"station_10_identity_gate_corrected", gate_correction_count)
	if player:
...
func _draw_state_layer() -> void:
	...
	var gate_col := VectorStageStyle.CORRECTION_OXIDE if not is_airlock_unlocked else VectorStageStyle.ANCHOR_CYAN
	var gate_y := identity_gate.position.y if identity_gate else 252.0
	var gate_rect := Rect2(512.0, gate_y - 58.0, 16.0, 116.0)
	if not is_airlock_unlocked:
		draw_rect(gate_rect, VectorStageStyle.MID_PLANE)
		draw_rect(gate_rect, gate_col, false, 2.0)
	else:
		draw_line(Vector2(512.0, gate_y - 58.0), Vector2(512.0, gate_y + 58.0), gate_col, 2.0)
		draw_line(Vector2(528.0, gate_y - 58.0), Vector2(528.0, gate_y + 58.0), gate_col, 2.0)
	draw_line(Vector2(536.0, 232.0), Vector2(596.0, 228.0), gate_col, 3.0)
	if gate_detail_faded:
		draw_rect(Rect2(432.0, 150.0, 8.0, 38.0), VectorStageStyle.shade(VectorStageStyle.CORRECTION_OXIDE, 0.38))
