...
- Ostatni zamkniety pakiet: `PKG-0007`, 2026-08-15
...
- Zamrozenia: `snapshots/PKG-NNNN-DATA/` przez `tools/snapshot.ps1` (D-017);
  ostatnie: `snapshots/PKG-0007-2026-08-15`
...
- Biblia dialogowa odroznia glosy i zawiera 18 scen niosacych glowne zwroty
  emocjonalne, w tym D-16, D-17 i D-18 dla przestrzeni 24, 35 i 41. W pliku
...
- **H-010a: MEASURED po `PKG-0007`.** Zwrot o skorygowanej galezi Leny ma trzy
  zapowiedzi spelniajace wszystkie cztery warunki pomiaru (sceny 01, 13, 23)
  ...
  trwale bez dowodu.
- **H-011a: nadal REFUTED.** Swiadectwo mialo piec strukturalnych sygnalow
  przewagi. `PKG-0007` zdjal jeden posrednio (D-022 usuwa smierc Wierzbickiej,
  ktora byla szostym kandydatem) i przyjal D-015, ktory zdejmie warunkowosc.
  Pozostale cztery czekaja na `N0.2-C`.
...
- cztery pozostale sygnaly przewagi finalu C - `N0.2-C`;
- scalenie scen 18 i 36, przeniesienie sceny 19, przenumerowanie - `N0.2-C`;
...
Zadnej z siedmiu sprzecznosci naprawionych w `PKG-0007` nie wykryla i nie
wykrylaby ich ponownie. D-014 jest przyjete i czeka na implementacje.
...
- ponowny pomiar H-011a po `N0.2-C`. H-010a zmierzone w `PKG-0007`.
...
- Final Swiadectwa nadal ma cztery zmierzone sygnaly przewagi strukturalnej
  (H-011a). Slepy test nie odbedzie sie (D-012), wiec jedynym dostepnym
  narzedziem jest usuniecie sygnalow w `N0.2-C` i ponowny pomiar.
...
`N0.2-C: Symetria finalow i struktura aktu II`

Cel: zdjac cztery pozostale sygnaly przewagi strukturalnej finalu Swiadectwo
i przebudowac srodek gry. Zakres: wdrozenie D-015 (Swiadectwo zawsze dostepne,
braki obnizaja jakosc wyniku), przeniesienie kosztu finalu C na Slad, usuniecie
dwoch replik z D-14, domkniecie relacji Jakub-Wierzbicka takze w finalach A i B,
wyrownanie rejestru trzech epilogow sceny 45, scalenie scen 18 i 36,
przeniesienie sceny 19 do aktu I i przenumerowanie przestrzeni. Pakiet konczy
sie ponownym pomiarem H-011a.

Przenumerowanie wykonuje sie raz. Dotyka `FULL_STORY.md`, `CONTINUITY_TRACKER.md`,
`DIALOGUE_SCRIPT.md` i `VISUAL_DESIGN.md`; kazde odwolanie do numeru sceny musi
zostac przeliczone w tym samym pakiecie.
