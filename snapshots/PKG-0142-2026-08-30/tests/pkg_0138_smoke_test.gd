...
const SAMPLE_STATIONS := [
	...
	"station_16", "station_21", "station_24", "station_30", "station_31",
	"station_37", "station_38", "station_41", "station_42a", "station_43"
]
...
func _test_sample_playthrough() -> void:
	for s_id in SAMPLE_STATIONS:
		...
		_expect(not completed.is_empty(), "%s must complete level via narrative player verbs" % s_id)
...
func _perform_station_actions(st: Node2D, player: CharacterBody2D) -> void:
	...
	if id.contains("01"):
		...
		if st.has_method("start_offer_dialogue"):
			st.call("start_offer_dialogue")
		await _advance_any_active_dialogue(st)
		if st.has_method("examine_oscilloscope"):
			st.call("examine_oscilloscope")
	elif id.contains("23"):
		if st.has_method("connect_circuit"):
			st.call("connect_circuit")
		if st.has_method("switch_shunt"):
			st.call("switch_shunt")
		if st.has_method("stabilize_overload"):
			st.call("stabilize_overload")
	elif id.contains("24"):
		if st.has_method("inspect_cctv"):
			st.call("inspect_cctv")
		if st.has_method("inspect_gauge"):
			st.call("inspect_gauge")
		if st.has_method("inspect_terminal"):
			st.call("inspect_terminal")
		if st.has_method("set_disposition"):
			st.call("set_disposition", 0)
		await _advance_any_active_dialogue(st)
	elif id.contains("25"):
		if st.has_method("inspect_cart"):
			st.call("inspect_cart")
		if st.has_method("inspect_chart"):
			st.call("inspect_chart")
		if st.has_method("inspect_sensor"):
			st.call("inspect_sensor")
		if st.has_method("confront_jakub"):
			st.call("confront_jakub")
		await _advance_any_active_dialogue(st)
	elif id.contains("26"):
