...
func _test_station_01_layout() -> void:
	...
	_expect(station.get_node_or_null("VectorStageEnvironment") == null, "station_01 musi odciąć wspólny VectorStageEnvironment")
	var station_script := FileAccess.get_file_as_string("res://scripts/levels/station_01.gd")
	_expect(station_script.contains("_draw()"), "station_01 musi mieć własne autorstwo obrazu")
	_expect(station_script.contains("repeat_line_four_measurement"), "station_01 musi mieć pojedynczy pomiar Linii 4")
	_expect(station_script.contains("read_marta_message"), "station_01 musi ujawnić koszt wobec Marty")
