...
func _test_procedural_audio() -> void:
	...
	var land_sfx := ProceduralAudio.create_land_sound(false)
	_expect(land_sfx != null, "land sound synthesis failed")
	_expect(land_sfx.data.size() > 0, "land sound buffer empty")

	var airlock_sfx := ProceduralAudio.create_airlock_seal_sound()
	_expect(airlock_sfx != null, "airlock seal sound synthesis failed")
	_expect(airlock_sfx.data.size() > 0, "airlock seal sound buffer empty")

	var memory_sfx := ProceduralAudio.create_memory_resonance_sound()
	_expect(memory_sfx != null, "memory resonance sound synthesis failed")
	_expect(memory_sfx.data.size() > 0, "memory resonance sound buffer empty")

	var switch_sfx := ProceduralAudio.create_switch_toggle_sound()
	_expect(switch_sfx != null, "switch toggle sound synthesis failed")
	_expect(switch_sfx.data.size() > 0, "switch toggle sound buffer empty")
	...
	var cam_click_sfx := ProceduralAudio.create_camera_click_sound()
	_expect(cam_click_sfx != null, "camera click sound synthesis failed")
	_expect(cam_click_sfx.data.size() > 0, "camera click sound buffer empty")

	var turnstile_sfx := ProceduralAudio.create_turnstile_unlatch_sound()
	_expect(turnstile_sfx != null, "turnstile unlatch sound synthesis failed")
	_expect(turnstile_sfx.data.size() > 0, "turnstile unlatch sound buffer empty")
