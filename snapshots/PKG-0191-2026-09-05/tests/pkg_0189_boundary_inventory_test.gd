...
## alignment this inventory audited. The F-0184-010 MRP static contract
## (below) is still untouched and still asserted unchanged.
##
...
func _run() -> void:
	...
	if _failures.is_empty():
		print("PKG-0189 INVENTORY PASS: MRP contract unchanged; Station 10-13 canonical facts recorded post-PKG-0191")
		quit(0)
	else:
...
func _check_station_hybrid_inventory() -> void:
	...
	for namespace_name in LEGACY_P7_NAMESPACES:
		_expect(combined.contains(namespace_name), "legacy P7 namespace unexpectedly absent (PKG-0191 retains callable P7 surface; PKG-0191's own scope is only F-0184-012 canonical facts): %s" % namespace_name)
