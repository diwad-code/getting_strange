...
func _test_marta_visual_identity() -> void:
	...
	for renderer in ["_draw_marta_interaction", "_draw_marta_observation_dialogue", "_draw_marta_witness_station", "_draw_epilogue_marta_doorstep"]:
	...
	_expect(resonance_source.count("draw_arc(") >= 3, "Marta's active close and final silhouettes must render a septum arc")
	...
	var portrait := Image.load_from_file("res://assets/characters/portraits/marta.png")
	var pink_pixels := 0
