...
func _ready() -> void:
	collision_layer = PhysicsLayers.THRESHOLD.x
	collision_mask = PhysicsLayers.THRESHOLD.y
	monitoring = true
	monitorable = true
	z_index = 4
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# could not be used. A reading point in reach that still has something
	# to do owns the press; one more step into the doorway leaves.
...
var _station_host_cached: Node = null


func _get_station_host() -> Node:
	if _station_host_cached != null and is_instance_valid(_station_host_cached):
		return _station_host_cached
	_station_host_cached = get_parent()
	return _station_host_cached
...
func _find_player() -> Node2D:
	...
	if host == null:
		return null
	return host.get_node_or_null("Player") as Node2D
...
func _disable_blocking_body() -> void:
	...
	if body == null:
		var host := _get_station_host()
		if host != null:
			body = host.get_node_or_null(blocking_body_path) as Node2D
	if body == null:
		return
...
func _draw_ready_notches(rect: Rect2) -> void:
	# Non-colour affordance: two 2 px mechanical notches remain legible in mono
	# and supplement (rather than replace) the cyan ready-state edge.
	var x := rect.position.x - 5.0
	var center_y := rect.position.y + rect.size.y * 0.5
	draw_line(Vector2(x, center_y - 7.0), Vector2(x + 4.0, center_y - 7.0), VectorStageStyle.LIGHT_PLANE, 2.0)
	draw_line(Vector2(x, center_y + 7.0), Vector2(x + 4.0, center_y + 7.0), VectorStageStyle.LIGHT_PLANE, 2.0)
