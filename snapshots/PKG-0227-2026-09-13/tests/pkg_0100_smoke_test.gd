...
const REQUIRED_SHELLS := {
	...
	"res://scenes/levels/station_02.tscn": ["WallLeft", "WallRight"],
	"res://scenes/levels/station_03.tscn": ["WallLeft", "WallRight"],
	"res://scenes/levels/station_04.tscn": ["CarriageRearWall", "CarriageFrontFrame"],
	"res://scenes/levels/station_05.tscn": ["WallLeft", "WallRight"],
	"res://scenes/levels/station_06.tscn": ["WallLeft", "WallRight"],
	"res://scenes/levels/station_07.tscn": ["WallLeft", "WallRight"],
...
func _check_stations(state: Node) -> void:
	for expected_number in range(1, 11):
		...
		if expected_number <= 8:
			_expect(stage == null, "P9 opening must not retain Vector-Stage environment: %s" % path)
		else:
...
func _check_state_pass_source(path: String) -> void:
	...
	if station_number <= 8:
		_expect(draw_body.find("VectorStageStyle.draw_stage_apron") >= 0, "P9 opening state pass must paint its own framed world: %s" % script_path)
	else:
	...
	match expected_number:
		...
		10:
			...
			if apartment_door and station_10:
				...
				_expect(not bool(station_10.get("is_level_completed")), "Station 10 key trial alone must not complete the station")
