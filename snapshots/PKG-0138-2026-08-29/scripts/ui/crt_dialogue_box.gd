...
func _play_speech_blip() -> void:
	if _audio.playing:
		return
	var is_lena := _speaker_label.text == "LENA"
	var cache_key: StringName = &"blip_lena" if is_lena else &"blip_other"
	_audio.stream = ProceduralAudio.get_cached_sound(cache_key, func() -> AudioStreamWAV:
		return ProceduralAudio.create_dialogue_blip_sound(is_lena)
	)
