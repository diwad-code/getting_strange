...
func _expected_family(station_id: String) -> int:
	...
	# PKG-0236 (B3, D-248): 13 wychodzi wlazem serwisowym za klatka, wiec
	# HATCH jak 14/15. Kontrolowana aktualizacja po realnym FAIL-u na
	# rodzinie DOOR (wzor D-216); 09/10 zostaja DOOR.
	if station_id == "station_13" or station_id == "station_14" or station_id == "station_15":
		return FAMILY_HATCH
	return FAMILY_DOOR
