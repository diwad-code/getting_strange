...
## Station 11 — P9, dowod instytucjonalny: lada UCP (S-13, PKG-0230).
## Karta sluzbowa, fizyczny rejestr 186 dni i okienko minimalnego raportu;
## Lena przechodzi kontrole mimo obcego numeru i kopiuje tylko potrzebny
## zakres (identity_card → record_186_days → minimal_report).
## Warstwa dawcy P7 (komoda, prywatne slady) jest nieaktywna w trasie, ale
## callable dla testow — patrz werby nizej.

## PRZESZKODA — dlaczego to tu jest: Lada wydaje rejestr tylko po kontroli biometrycznej, a nocny dyzur przyjmuje po godzinach (P2-4).
## PRZESZKODA — czego wymaga od Leny: okazania karty, odczytania rejestru i poproszenia o waski wyciag zamiast calej historii.
## PRZESZKODA — koszt porażki: zadanie o calosc zamiast zakresu zostawia luke w GapLedger, nie blokade.

## PKG-0192 (D-205 §4) — jawna decyzja dla `HallwaySideboard`: ZOSTAJE jako
## wezel w drzewie, nie jest przeszkoda trasy (PKG-0234 A2). Callable
## `push_sideboard()` zostaje dla testow; kampania otwiera prog wylacznie
## `request_minimal_report()`. Metody
...
## PKG-0233 (B3, S-09): warunkowe pokwitowanie opieki nad czytnikiem.
## UCP zostaje racjonalna instytucja (pokwitowanie minimalnego zakresu),
## a 12/13 pamietaja ten status z decyzji. Jeden writer (terminalny werb 11).
const FACT_CUSTODY := &"p9.mystery.institution.reader_custody_receipt"
const CUSTODY_VALUE := "conditional_custody_minimal_scope"
...
var is_minimal_report_requested := false
...
func _setup_guidance() -> void:
	...
	_register_beat(&"s11_photo_contact", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Czytnik przy ladzie czeka na kartę.", "The reader at the counter awaits a card.", &"", "")
	_register_beat(&"s11_lost_relationship_hypothesis", GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT, &"interpretation", &"fallible", "Może pomylili kartę. Porównam ją z historią wejść.", "Perhaps they mixed up the card. I will compare the entry history.", &"lost_relationship", "read_186_day_record")
	_register_beat(&"s11_private_material", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Pokażę kartę. Potem sprawdzę rejestr i poproszę o wyciąg.", "I will show my card, check the record and request an extract.", &"", "")
	# PKG-0230 (P2-4, S-12): jedno zdanie na wejscie — nocny dyzur, wzor
	# warsztatu Jakuba (12: koniec zmiany). Wypowiadane przy pierwszym werbie.
	_register_beat(&"s11_night_desk", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Nocny dyżur: okienko czynne do rana. Zmiana kontrolna przyjmuje po godzinach.", "Night duty: the window stays open till morning. The control shift receives after hours.", &"", "")
	# PKG-0233 (B3, S-09): proceduralne rozstrzygniecie opieki nad czytnikiem —
	# warunkowe pokwitowanie zamiast milczacego wyniesienia.
	_register_beat(&"s11_reader_receipt", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Pokwitowanie: czytnik w opiece warunkowej, zakres minimalny.", "Receipt issued: the reader stays in conditional custody, minimal scope.", &"", "")
	_register_beat(&"s11_system_hint", GuidanceBeat.Tier.L4_RESCUE_HINT, &"system_hint", &"system", "WSKAZÓWKA: Karta, rejestr 186 dni, wyciąg z raportu.", "HINT: Card, 186-day record, report extract.", &"", "")
...
func _physics_process(_delta: float) -> void:
	# PKG-0234 (A2): auto-push zdjety z trasy. Callable push_sideboard zostaje
	# dla testow; is_passage_clear nie otwiera progu kampanii.
...
func _draw() -> void:
	...
	# PKG-0233 (B3, S-09): pokwitowanie opieki warunkowej na ladzie — widoczny
	# slad procedury (warunkowe wydanie czytnika), nie nowy wezel ani tekst.
	if _decision_bool(P9_REPORT):
		draw_rect(Rect2(196.0, 240.0, 34.0, 10.0), Color("d9e2de"))
		draw_line(Vector2(200.0, 245.0), Vector2(226.0, 245.0), Color("314149"), 1.0)
	draw_rect(Rect2(0.0, 304.0, 640.0, 56.0), Color("48585f"))
	draw_line(Vector2(0.0, 304.0), Vector2(640.0, 304.0), Color("dce3df"), 2.0)
	# PKG-0198 (ZERO wycinek 2): cień kontaktowy lady w prawo, 0.48, zgodnie
	# z górnym światłem instytucji i konwencją 01/06/08. [MONO-Q] komoda/lada
	# czyta się bryłą i działaniem (przejście przez ladę), nigdy samym kolorem.
	draw_colored_polygon(PackedVector2Array([
		Vector2(164.0, 300.0), Vector2(464.0, 296.0),
		Vector2(472.0, 304.0), Vector2(172.0, 308.0),
...
func present_identity_card() -> bool:
	...
	if guidance_service:
		guidance_service.trigger_beat(&"s11_night_desk")
	queue_redraw()
	return true
...
func request_minimal_report() -> bool:
	...
	# PKG-0233 (B3, S-09): wyciag w minimalnym zakresie = warunkowe
	# pokwitowanie. Ten sam akt, zero nowych werbow i par dialogowych.
	_record(FACT_CUSTODY, CUSTODY_VALUE)
	is_minimal_report_requested = true
	if guidance_service:
		guidance_service.trigger_beat(&"s11_reader_receipt")
	_unlock_exit()
	queue_redraw()
	return true
