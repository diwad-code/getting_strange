...
func _test_door_clearance() -> void:
	...
	if gsm and not gsm.decisions.has(&"p7.sample_and_promise.sample_preserved"):
		gsm.record_decision(&"p7.sample_and_promise.sample_preserved", true)
	if station.has_method("observe_service_detour"):
		station.call("observe_service_detour")
	if station.has_method("compare_route_duration"):
		station.call("compare_route_duration")
	if station.has_method("take_safe_bypass"):
		station.call("take_safe_bypass")
