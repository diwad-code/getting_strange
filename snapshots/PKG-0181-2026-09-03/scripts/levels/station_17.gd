...
func _commit_consent(scope: StringName) -> bool:
	...
	var dialogue_box := get_node_or_null("CRTDialogueBox") as CRTDialogueBox
	if dialogue_box != null:
		if scope == SCOPE_REFUSED:
			dialogue_box.present([{"speaker": "JAKUB", "text": "Przyszłaś po odczyt, a teraz każesz mi podpisać protokół in blanco. Nie ze mną, Lena."}])
		elif scope == SCOPE_GRANTED:
			dialogue_box.present([
				{"speaker": "JAKUB", "text": "Dziesięć sekund. Mój nadajnik, moja ręka na wyłączniku."},
				{"speaker": "LENA", "text": "Dziesięć."}
			])
