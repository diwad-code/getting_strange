...
func _test_station_18() -> void:
	...
	root.add_child(station)
	await physics_frame
	_expect(not station.commit_force_home(), "station 18: commit without inventory must stay safe")
	...
	_expect(station.commit_close_equal(), "station 18: one method must be committed")
