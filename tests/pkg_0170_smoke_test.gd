...
func _test_return_zone(state: Object) -> void:
	...
	_expect(zone != null, "ReturnZone exists")
	if player != null and zone != null:
		...
		for _i in 8:
			await physics_frame
		_expect(not signal_fired[0], "Return zone overlap alone must not emit")
		zone.trigger_return(player)
		_expect(signal_fired[0], "Return zone emits previous_level_requested via interact")
