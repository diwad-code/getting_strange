...
const P7_EARLY_SEQUENCE_MIGRATIONS := [
	...
	},
	{
		...
		"legacy_keys": [&"station_12_playback_muffled", &"station_13_papers_scattered", &"station_14_mug_broken", &"marta_relationship_disclosed"],
	},
	{
	...
	},
	{
		...
		"legacy_keys": [
			...
			&"recognition_evidence_carried", &"world_recognized",
			&"local_lena_search_committed", &"local_lena_search_started",
		],
	},
	{
	...
	},
	{
		...
		"legacy_keys": [
			...
			&"s36_line4_ledger_revealed", &"home_echo_verified", &"ucp_cost_ledger_found",
		],
	},
	{
...
func _restore_campaign(data: Dictionary) -> bool:
	...
	if saved_gaps is Dictionary:
		open_gaps = (saved_gaps as Dictionary).duplicate(true)
...
func _refresh_station_buttons() -> void:
	...
	if is_instance_valid(_status_label):
		_status_label.text = LocalizationManager.tr_key("PAUSE_STATUS") % [get_selectable_stations().size(), "ON" if test_mode_enabled else "OFF"]
	var test_button := _pause_layer.get_node_or_null("PanelContainer/VBoxContainer/HBoxContainer/TestModeButton") as Button
	if test_button:
		test_button.text = LocalizationManager.tr_key("PAUSE_TEST_MODE") % ("ON" if test_mode_enabled else "OFF")
	_refresh_pause_localization()
