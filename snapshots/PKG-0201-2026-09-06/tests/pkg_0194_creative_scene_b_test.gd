...
func run() -> void:
	...
	var joined_a := "\n".join(delivered)
	for required in ["fala korekty co siedem sekund", "Utrzymanie i uległość", "Kontakt przy pierwszym odczycie", "Wyjście na czas: sam czytnik", "Ktoś przerwał ją z zewnątrz", "Dwa identyczne echa", "poprawiła tylko mój celowy błąd", "Bez jej zgody nie powtarzaj", "kurtka na kaloryferze", "Twoja kurtka była", "zgłosiłam zaginięcie", "nie podmiana", "Ktoś utrzymuje zapis", "wygodnym zastępstwem", "przed końcem zmiany", "Dziesięć.", "Wiedziała czy zapytała", "Pomogę ją wyciągnąć", "Moja prośba, mój ruch"]:
	...
	ordered(joined_a, "Pomogę ją wyciągnąć", "Moja prośba, mój ruch", "truth must precede the commit")
