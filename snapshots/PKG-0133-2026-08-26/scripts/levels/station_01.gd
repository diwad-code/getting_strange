...
func _setup_guidance() -> void:
	...
	beat_pack.text_pl = "Zapis zapisany. Zbieram sprzęt i wychodzę."
...
func _unlock_exit_door() -> void:
	...
	if chamber_door:
		var shape := chamber_door.get_node_or_null("CollisionShape2D") as CollisionShape2D
		if shape:
			shape.disabled = true
		var tween := create_tween()
		tween.tween_property(chamber_door, "position:y", chamber_door.position.y - 140.0, 1.0).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
