...
## PKG-0168 normal-driver evidence for Station 42B.
## Three textless M2 state frames (neutral / flow closed / local Lena recovered)
...
const PACKAGE_ID := "PKG-0168"
const LOGICAL_SIZE := Vector2i(640, 360)
const OUTPUT_ROOT := "res://reports/pkg_0168"
const REPORT_PATH := "res://reports/pkg_0168/visual_evidence_report.txt"
const WAIT_FRAMES := 20

const CASES: Array[Dictionary] = [
	{"id": "neutral", "mono": false},
	{"id": "flow_closed", "mono": false},
	{"id": "local_recovered", "mono": false},
...
func _capture_one(capture: Dictionary, state: Node) -> void:
	...
	if packed == null:
		_failures.append("cannot load station_42b")
		return
	var station := packed.instantiate() as CanvasItem
	if station == null:
		_failures.append("station_42b root is not CanvasItem")
		return
	root.add_child(station)
	for _frame in range(WAIT_FRAMES):
		await physics_frame
	if not _apply_state(station, String(capture["id"])):
		_failures.append("cannot apply station_42b/%s" % String(capture["id"]))
	else:
...
func _save_frame(image: Image, capture: Dictionary, hidden_items: int) -> void:
	...
	var relative_path := OUTPUT_ROOT.path_join("station_42b_%s.png" % String(capture["id"]))
...
func _apply_state(station: CanvasItem, state_id: String) -> bool:
	match state_id:
		"neutral":
			return true
		"flow_closed":
			return bool(station.call("execute_close_flow"))
		"local_recovered":
			return bool(station.call("execute_close_flow")) \
				and bool(station.call("read_local_lena_recovered"))
		"structure_mono":
			return bool(station.call("execute_close_flow")) \
				and bool(station.call("read_local_lena_recovered")) \
