...
func _test_station_24() -> void:
	print("TEST: Station 24 (Nie jesteś jej zastępstwem / Granice Marty)...")
	...
	inst.inspect_cctv()
	inst.inspect_gauge()
	inst.inspect_terminal()
	inst.set_disposition(Station24.DispositionChoice.APPARENT_COOPERATION)

	_expect(inst.is_cctv_inspected, "Station 24: CCTV musi być sprawdzone")
	_expect(inst.is_gauge_inspected, "Station 24: Wskaźnik musi być sprawdzony")
	_expect(inst.is_terminal_inspected, "Station 24: Terminal musi być sprawdzony")
	_expect(inst.is_disposition_made, "Station 24: Dyspozycja musi być ustalona")

	# Advance dialogue to end
	while inst.dialogue_active:
		inst.advance_dialogue()

	_expect(inst.is_exit_unlocked, "Station 24: Śluza tranzytowa musi być odblokowana")
...
func _test_station_25() -> void:
	print("TEST: Station 25 (Węzeł pod Linią 4 / Jakub)...")
	...
	_expect(inst.is_cart_inspected, "Station 25: Wózek musi być sprawdzony")
	_expect(inst.is_chart_inspected, "Station 25: Schemat musi być sprawdzony")
	_expect(inst.is_sensor_inspected, "Station 25: Czujnik gestu musi być sprawdzony")
	_expect(inst.is_jakub_confronted, "Station 25: Jakub musi być skonfrontowany")

	# Advance dialogue to end
	while inst.dialogue_active:
		inst.advance_dialogue()

	_expect(inst.is_exit_unlocked, "Station 25: Wyjście do śluzy 26 musi być odblokowane")
