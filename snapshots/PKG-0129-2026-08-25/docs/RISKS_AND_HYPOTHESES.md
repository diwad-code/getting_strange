...
| H-005 | Pixel-Stage jest wydajny i powtarzalny w 43 scenach | KOSZTOWA | TECHNICAL | WorldPixelCompositor w Layer 5 przetestowany dla wszystkich 43 scen w `pkg_0123_smoke_test.gd` (60 Hz baseline) |
...
| H-017 | Kobieca sylwetka i animacja wspinaczki Leny 3.0 wzmacniają fizyczną wiarygodność bez arcade'owego odczucia | MIERZALNA + ODBIORCZA | TECHNICAL | LenaVisualRig 3.0 z 14 stanami (w tym climb), proporcje 1:6.7 i wysokość ~66 px zweryfikowane w `pkg_0125_smoke_test.gd`; estetyczny odbiór bez dowodu |
| H-018 | Swobodne cofanie dwukierunkowe wspiera poczucie ciągłości przestrzennej | MIERZALNA + ODBIORCZA | TECHNICAL | `get_previous_campaign_station` i `target_spawn_side` przetestowane w GameStateManager; płynność eksploracji bez dowodu |
...
| H-025 | Globalny audyt drożności geometrii i diegetyczne drabiny/windy eliminują wszelkie zablokowania gracza bez naruszania kanonu D-099 | MIERZALNA + ODBIORCZA | TECHNICAL | automatyczny audyt 45 scen w `tools/geometry_audit.gd` i certyfikacja w `pkg_0129_smoke_test.gd`; subiektywny rytm eksploracji bez dowodu |
...
| R-025 | Nadmiar proceduralnego audio lub cząsteczek obciąży mikser lub wątek renderera | niskie | wysoki | Buforowanie 16-bit PCM w GDScript z mix_rate 44100 Hz, CPUParticles2D zamiast GPU i brak ciągłego przeładowywania streamów |
