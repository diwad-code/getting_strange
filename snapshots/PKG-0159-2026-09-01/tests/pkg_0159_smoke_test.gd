...
func _start_new_game_to_station_01() -> Node:
	...
	for _frame in range(240):
		...
		if current_scene is Station01:
			break
	_expect(current_scene is Station01, "ui_accept on Nowa gra must route to Station 01")
...
func _exit_current_station_to(station: Node, expected_station: StringName) -> Node:
	...
	for _frame in range(480):
		await physics_frame
		if current_scene != station:
			break
	Input.action_release(&"move_right")
	for _frame in range(300):
		await process_frame
		if current_scene != null and String(current_scene.name).to_lower() == String(expected_station).replace("_", ""):
			break
