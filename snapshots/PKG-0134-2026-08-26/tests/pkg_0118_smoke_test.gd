...
func _test_station_01_slice() -> void:
	...
	if sensor and bag:
		...
		_expect(station.get("sensor_checked") == true, "Pierwszy odczyt musi być zarejestrowany")
		# Reading 2 (clean repeat)
		sensor.trigger_interaction()
		_expect(station.get("second_reading_done") == true, "Drugi odczyt musi być wykonany")
		# Pack gear
		bag.trigger_interaction()
		_expect(station.get("gear_packed") == true, "Sprzęt musi być spakowany")
...
func _test_station_07_slice() -> void:
	...
	# Test shopkeeper interaction & water purchase
	if station.has_method("start_shopkeeper_dialogue"):
		station.call(&"start_shopkeeper_dialogue")
		_expect(station.get("shopkeeper_dialogue_active") == true, "Dialog ze sprzedawcą musi być aktywny")
		for i in range(10):
			if not station.get("shopkeeper_dialogue_active"):
				break
			station.call(&"advance_shopkeeper_dialogue")
		_expect(station.get("is_water_purchased") == true, "Woda musi być zakupiona po dialogu")
