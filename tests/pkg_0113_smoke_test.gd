...
func _run() -> void:
	...
	if state:
		...
		_expect(int(constants.get("CAMPAIGN_TRANSITION_LIMIT", -1)) == 18, "campaign transition limit must cover Station 01..18")
...
func _check_pause_surface(state: Node) -> void:
	...
	if grid:
		...
		_expect(grid.get_child_count() == 20, "pause grid must expose the established 20 selectable campaign entries")
