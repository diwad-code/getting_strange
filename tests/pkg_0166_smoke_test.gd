...
func _test_refused_and_withheld_path(state: Node) -> void:
	...
	if player != null:
		...
		await physics_frame
		await physics_frame
	# PKG-0232 (D-244, D1, kontrolowana aktualizacja): scena z odmową NIE
	# commituje progu w przód (brak metody = brak finału); droga powrotu do
	# 17 zostaje otwarta (is_exit_unlocked wyżej), finał się nie wybiera.
	_expect(not station.is_level_completed, "Scena z odmowa nie commituje progu bez metody")
	_expect(String(state.call("get_selected_finale_id")) == "", "Odmowa nie wybiera finalu")
	_expect(not state.decisions.has(&"campaign_finale"), "Odmowa nie zapisuje finalu")
