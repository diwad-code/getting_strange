...
func _test_mutual_test_save_migration(state: Node) -> void:
	...
	# PKG-0194 (D-211): `mechanic_cost_observed` graduated from P7-only
	# mutual_test contamination to a permanent P9 fact with a single audited
	# writer (`station_16.gd`'s `_commit_cost()`) and a Station 17 donor
	# requirement. Same pattern as PKG-0191's `marta_boundary_accepted` below:
	# it must survive this migration like any other current-state decision.
	# The genuinely-legacy P7 slice markers around it are still erased.
	_expect(state.decisions.get(&"mechanic_cost_observed", false) == true, "PKG-0194: mechanic_cost_observed is canonical and must survive migration, not be erased as P7 mutual_test state")
