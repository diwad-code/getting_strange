...
## RYTM (P2-2, PKG-0230): to jest scena z jednym punktem i dlugim oddechem —
## dziennik zamiast trzech rekwizytow. Swiadome odstepstwo, nie brak tresci.
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed(&"interact") and not event.is_echo():
		# PKG-0215 (D-228): priority MRP > Anchor > Threshold. A ready
		# Threshold consumes entry itself; the anchor must not steal it
		# (nor record a miss at the door).
		var threshold := get_node_or_null("Threshold")
		if threshold != null and threshold.has_method("is_ready_for_entry") and bool(threshold.call("is_ready_for_entry")):
			return
...
func _setup_guidance() -> void:
	...
	# PKG-0230 (P1-1): zdanie wyjscia — 14 → 15: most nauczyl trzymania;
	# wyciag z 11 mowi, gdzie szukac echa.
	_register_beat(&"s14_exit_to_loop", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Most trzyma obie wersje. Z mostu do pętli — wyciąg mówi, gdzie szukać echa.", "The bridge holds both versions. From the bridge to the loop — the extract says where to seek the echo.", &"", &"")
	_register_beat(&"s14_system_hint", GuidanceBeat.Tier.L4_RESCUE_HINT, &"system_hint", &"system", "POMOC: Chwyć obejmę przed impulsem rozdzielnicy. Jeśli puścisz, most przejdzie na rezerwę i odetnie zasilanie.", "HELP: Grip the collar before the switchgear pulse. If you let go, the bridge shifts to backup and cuts power.", &"", &"")
	_register_beat(&"s14_method_named", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Zakotwiczenie i uległość. Dwa ruchy jednej metody — sekcja pokazała różnicę.", "Anchoring and yielding. Two motions of one method — the section showed the difference.", &"", &"")
...
func _apply_correction_pulse() -> void:
	...
	if bridge.is_anchored:
		...
		if not now_live and was_live and not _decision_bool(FACT_COST):
			...
			# PKG-0222 (M7): puszczony most = stan kosztu + linia L2 + zapis.
			# Zanik wizualny niesie istniejacy cien sekcji w _draw().
			if guidance_service != null:
				guidance_service.trigger_beat(&"s14_cost_hypothesis", true)
...
func _try_name_behaviors() -> void:
	...
	if guidance_service != null:
		...
		guidance_service.trigger_beat(&"s14_exit_to_loop")
...
func _draw() -> void:
	...
	# 0.48, zgodnie z lampą stanowiska (296..344,56..62) i konwencją 01/06/08.
	draw_colored_polygon(PackedVector2Array([
		Vector2(202.0, 294.0), Vector2(478.0, 292.0),
		Vector2(486.0, 300.0), Vector2(210.0, 302.0),
