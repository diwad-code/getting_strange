...
## PRZESZKODA — dlaczego to tu jest: Kanał burzowy odprowadza osad sedacyjny
## do wód gruntowych, bo instalacja filtracyjna wykonuje zapisany zrzut.
## PRZESZKODA — czego wymaga od Leny: Obejrzenia jazu, nurtu, drabinki i kurka
## oraz połączenia fizycznego skażenia z decyzjami UCP.
## PRZESZKODA — koszt porażki: Nie ma próby korekty; stawką sceny jest
...
func _draw_state_layer() -> void:
	...
	var weir_color := VectorStageStyle.HUMAN_AMBER if is_weir_inspected else VectorStageStyle.LIGHT_PLANE
	draw_line(Vector2(116.0, 92.0), Vector2(116.0, 244.0), weir_color, 2.0)
	draw_line(Vector2(126.0, 92.0), Vector2(126.0, 244.0), VectorStageStyle.MID_PLANE, 1.0)
	var current_color := VectorStageStyle.CORRECTION_OXIDE if is_current_inspected else VectorStageStyle.MID_PLANE
	draw_line(Vector2(198.0, 218.0), Vector2(294.0, 218.0), current_color, 2.0)
	var ladder_color := VectorStageStyle.ANCHOR_CYAN if is_ladder_inspected else VectorStageStyle.LIGHT_PLANE
	for rung in range(5):
		var rung_y := 154.0 + float(rung) * 20.0
		draw_line(Vector2(330.0, rung_y), Vector2(360.0, rung_y), ladder_color, 1.2)
	var tap_color := VectorStageStyle.CORRECTION_OXIDE if is_tap_inspected else VectorStageStyle.LIGHT_PLANE
	draw_colored_polygon(PackedVector2Array([Vector2(426.0, 188.0), Vector2(458.0, 184.0), Vector2(466.0, 236.0), Vector2(432.0, 240.0)]), tap_color)
	if is_exit_unlocked:
