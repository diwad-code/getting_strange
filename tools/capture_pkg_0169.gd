...
## PKG-0169 normal-driver evidence for Station 42C.
## Three textless M2 state frames (neutral / passage opened / memory leak accepted)
...
const PACKAGE_ID := "PKG-0169"
const LOGICAL_SIZE := Vector2i(640, 360)
const OUTPUT_ROOT := "res://reports/pkg_0169"
const REPORT_PATH := "res://reports/pkg_0169/visual_evidence_report.txt"
const WAIT_FRAMES := 20

const CASES: Array[Dictionary] = [
	{"id": "neutral", "mono": false},
	{"id": "passage_opened", "mono": false},
	{"id": "leak_accepted", "mono": false},
...
func _capture_one(capture: Dictionary, state: Node) -> void:
	...
	if packed == null:
		_failures.append("cannot load station_42c")
		return
	var station := packed.instantiate() as CanvasItem
	if station == null:
		_failures.append("station_42c root is not CanvasItem")
		return
	root.add_child(station)
	for _frame in range(WAIT_FRAMES):
		await physics_frame
	if not _apply_state(station, String(capture["id"])):
		_failures.append("cannot apply station_42c/%s" % String(capture["id"]))
	else:
...
func _save_frame(image: Image, capture: Dictionary, hidden_items: int) -> void:
	...
	var relative_path := OUTPUT_ROOT.path_join("station_42c_%s.png" % String(capture["id"]))
...
func _apply_state(station: CanvasItem, state_id: String) -> bool:
	match state_id:
		"neutral":
			return true
		"passage_opened":
			return bool(station.call("execute_mutual_passage"))
		"leak_accepted":
			return bool(station.call("execute_mutual_passage")) \
				and bool(station.call("read_memory_leak"))
		"structure_mono":
			return bool(station.call("execute_mutual_passage")) \
				and bool(station.call("read_memory_leak")) \
