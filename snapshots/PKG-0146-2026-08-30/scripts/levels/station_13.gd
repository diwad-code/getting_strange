...
func _on_prop_resonance_triggered(id: String, prop_type: int, prop: MemoryResonancePoint) -> void:
	match id:
		"drawer_handle":
			open_drawer()
		"bag_certificate":
			observe_field_certificate()
		"drawer_contract":
			observe_tenancy_contract()
		"seal_magnifier":
			verify_document_independence()
		"reader_offline_log":
			compare_document_versions()
		"phone_to_marta":
			request_independent_description()
...
func _is_resolved(id: String) -> bool:
	match id:
		"drawer_handle": return is_drawer_open
		"bag_certificate": return is_certificate_observed
		"drawer_contract": return is_contract_observed
		"seal_magnifier": return are_seals_verified
		"reader_offline_log": return are_documents_compared
		"phone_to_marta": return is_meeting_requested
	return false
...
func _draw() -> void:
	...
	var cert_color := VectorStageStyle.LIGHT_PLANE if is_certificate_observed else VectorStageStyle.HUMAN_AMBER
	var contract_color := VectorStageStyle.ANCHOR_CYAN if is_contract_observed else VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.5)
	var seal_color := VectorStageStyle.ANCHOR_CYAN if are_seals_verified else VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.35)
	var result_color := VectorStageStyle.CORRECTION_OXIDE if are_documents_compared else VectorStageStyle.HUMAN_AMBER
	draw_rect(Rect2(Vector2(168.0, 244.0), Vector2(30.0, 20.0)), cert_color, false, 1.0)
	draw_rect(Rect2(Vector2(316.0, 244.0), Vector2(30.0, 20.0)), contract_color, false, 1.0)
	draw_circle(Vector2(190.0, 236.0), 3.0, seal_color)
	draw_circle(Vector2(338.0, 236.0), 3.0, seal_color)
	draw_line(Vector2(420.0, 250.0), Vector2(448.0, 250.0), result_color, 3.0)
	var exit_color := VectorStageStyle.ANCHOR_CYAN if is_exit_unlocked else VectorStageStyle.HUMAN_AMBER
	draw_line(Vector2(574.0, 180.0), Vector2(574.0, 298.0), exit_color, 2.0)
