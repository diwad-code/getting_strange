...
func _test_18_guards(state: Node) -> void:
	for method in [&"force_home", &"close_equal_recover_local", &"mutual_passage"]:
		...
		await _close(station)
	# granted: pełny łańcuch czasownikami przechodzi i domyka próg.
	state.reset_campaign(true)
	var full := await _open(state, 18)
	if full == null:
		return
	_seed_18_entry(state, "granted")
	_expect(bool(full.call("compare_forecast_consent_dependencies")), "zestawienie wychodzi")
	_expect(bool(full.call("disclose_marta_truth_partial")), "prawda wychodzi")
...
func _test_42a_order(state: Node) -> void:
	state.reset_campaign(true)
	_seed_42_entry(state, "force_home", "partial", "granted")
...
func _test_42b_order(state: Node) -> void:
	state.reset_campaign(true)
	_seed_42_entry(state, "close_equal_recover_local", "partial", "limited")
	...
	_expect(String(station.get("last_feedback")) == "flow_closure_required", "42B: luka nazywa wykonanie")
	...
	_expect(not bool(station.call("read_household_consequence")), "42B: skutek przed stanem nadal odrzucony")
...
func _test_42c_order(state: Node) -> void:
	state.reset_campaign(true)
	_seed_42_entry(state, "mutual_passage", "partial", "granted")
...
func _test_forward_back_forward(state: Node) -> void:
	...
	if live18 != null:
		_expect(bool(live18.call("compare_forecast_consent_dependencies")), "18: zestawienie (macierz)")
		_expect(bool(live18.call("disclose_marta_truth_partial")), "18: prawda (macierz)")
