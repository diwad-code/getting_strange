...
func _on_prop_resonance_triggered(id: String, prop_type: int) -> void:
	...
	match prop_type:
		MemoryResonancePoint.PropType.BUS_SPEAKER:
			trigger_speaker_announcement()
		MemoryResonancePoint.PropType.ELDERLY_PASSENGER:
			if not is_passenger_dialogue_completed:
				if not passenger_dialogue_active:
					start_passenger_dialogue()
				else:
					advance_passenger_dialogue()
		MemoryResonancePoint.PropType.GOLD_RING:
			inspect_gold_ring()
		MemoryResonancePoint.PropType.BUS_ROUTE_MAP:
...
func _draw() -> void:
	# 1. Outer night sky and scrolling street elements behind windows
	_draw_window_backgrounds()
	
	# 2. Bus interior structural walls, roof, and floor
	_draw_bus_structure()
	
	# 3. Overhead handrails, fluorescent light fixtures & hanging straps
	_draw_overhead_fixtures()
	
	# 4. Bus seats & passenger area
	_draw_bus_seats()
	
	# 5. Driver cabin partition and route placard
	_draw_driver_partition()
	
	# 6. Exit doors at the rear (animated pneumatic folding)
	_draw_exit_doors()
	
	# 7. In-world dialogue and institutional PA banners
