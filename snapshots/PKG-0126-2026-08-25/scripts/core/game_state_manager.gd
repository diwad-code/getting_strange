...
func _apply_loaded_settings(should_load: bool, data: Dictionary) -> bool:
	...
	for action in parsed_bindings:
		...
		InputMap.action_add_event(action, (parsed_bindings[action] as InputEvent).duplicate())
	LocalizationManager.set_locale(locale_code)
	return true
...
func remap_action(action: StringName, event: InputEvent) -> Dictionary:
	...
	InputMap.action_erase_events(action)
	InputMap.action_add_event(action, normalized.duplicate())
...
func _apply_spawn_side_deferred(node: Node, side: StringName) -> void:
	...
	if side == &"right":
		...
		if player != null and player.is_inside_tree():
			var right_spawn_x := 540.0
			var spawn_pos := Vector2(right_spawn_x, player.global_position.y)
			player.reset_to(spawn_pos)
