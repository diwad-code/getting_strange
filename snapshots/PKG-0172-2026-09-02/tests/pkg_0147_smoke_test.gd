...
func _test_s07_three_place_proofs(state: Node) -> void:
	...
	_expect(not _call_bool(station, &"commit_force_home", "Station 18 musi wystawiać zatwierdzenie metody"), "Zatwierdzenie S07/P9 bez zestawienia musi być odrzucone bezpiecznie")
	_expect(state.decisions.get(&"p9.method_commitment.safe_trial_feedback", "") == "forecast_and_consent_inventory_required", "Błędna próba na 18 musi dać informację")
	_expect(not state.decisions.has(&"method_committed"), "Błędna próba nie może nadać method_committed")
