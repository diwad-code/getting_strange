...
## (wzór CR-A/CR-B) z `creative_scene_lines.gd`.
...
func _setup_guidance() -> void:
	...
	# PKG-0225 (K1): kurtka-kosztomierz — domowa Marta odwróciła kurtkę na lewą
	# stronę i zawiesiła ją na oparciu pustego krzesła. Konkret bez tezy (D-214);
	# wzór K4: rejestracja beatu + ślad w _draw, bez nowego triggera.
	_register_beat(&"s42a_jacket", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Na oparciu kurtka odwrócona na lewą stronę. Marta nic nie mówi.", "A jacket turned inside-out over the chair back. Marta says nothing.", &"", "")
...
## świeżej instancji. Reguła: flaga lokalna + fakt w decyzjach = rozliczone
## (powtórka milczy); flaga pusta + fakt w decyzjach = odtworzenie bez zapisu
## (sygnały/beaty dla winiet i prezentera, zero _record).
...
func execute_forced_return() -> bool:
	if is_return_executed and _decided(FACT_RETURN):
		return false
...
func read_sealed_other_lena() -> bool:
	if is_other_lena_sealed and _decided(FACT_SEALED):
		return false
	if not _has_force_home():
		_record_feedback(&"method_force_home_required")
		return false
	# PKG-0232 (D-244, D3): stan dopiero po wykonaniu (wykonanie → odczyt
	# stanu → odczyt skutku → próg 43). Bramka 0215 nie obejmuje 42.
	if not is_return_executed and not _decided(FACT_RETURN):
		_record_feedback(&"return_execution_required")
		if guidance_service:
			guidance_service.trigger_beat(&"s42a_read_plan")
		return false
	is_other_lena_sealed = true
	is_chamber_a_witnessed = true
	if not _decided(FACT_SEALED):
...
func read_household_consequence() -> bool:
	...
	if not is_other_lena_sealed and not _decided(FACT_SEALED):
		_record_feedback(&"sealed_state_required")
		if guidance_service:
			guidance_service.trigger_beat(&"s42a_read_plan")
		return false
	marta_truth_state = _read_marta_truth()
	...
	# finau nie wypelnia jej SCOPE_LIMITED. Nazwana luka zamiast fabrikacji.
...
func forward_block_line() -> String:
	if is_level_completed or not _has_force_home():
		return ""
	if not (is_return_executed or _decided(FACT_RETURN)):
		return "Zanim wyjdę, wykonam powrót przy czytniku."
	if not (is_other_lena_sealed or _decided(FACT_SEALED)):
		return "Zanim wyjdę, sprawdzę, co zostało po drugiej stronie."
...
func _trigger_level_completion() -> void:
	...
	if not ((is_return_executed or _decided(FACT_RETURN)) \
			and (is_other_lena_sealed or _decided(FACT_SEALED)) \
...
func _draw_household_table() -> void:
	...
	# PKG-0225 (K1): kurtka odwrócona na lewą stronę na oparciu pustego krzesła
	# (beat s42a_jacket). Jaśniejsza podszewka na wierzchu = odwrócenie; rysunek
	# z prymitywów, zero nowych assetów.
	draw_colored_polygon(
		PackedVector2Array([Vector2(510.0, 262.0), Vector2(534.0, 262.0), Vector2(532.0, 244.0), Vector2(512.0, 244.0)]),
		VectorStageStyle.shade(VectorStageStyle.LIGHT_PLANE, 0.55),
	)
	draw_line(Vector2(522.0, 262.0), Vector2(522.0, 244.0), VectorStageStyle.shade(VectorStageStyle.MID_PLANE, 0.5), 1.0)
	...
	# uzupelnienie pierscienia prawdy (truth ma juz linie w 0226).
	_draw_payoff_marks()
