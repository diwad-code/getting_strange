...
func _run() -> void:
	...
	if state:
		...
		_expect(state.get_selectable_stations(true).size() == 20, "test selection must expose all 20 spaces")
		...
		_expect(state.get_selectable_stations().size() == 20, "enabled test mode must expose all spaces")
		...
		_expect(state.get_next_campaign_station(&"station_20") == &"station_21", "station 20 must lead to station 21 in the production route")
	...
	for path in ACT_I_PATHS:
		...
		_expect(
			...
			"P9 world pixel compositor missing: %s" % path
		)
	...
	if _failures.is_empty():
		print("PKG-0094 SMOKE PASS: campaign save, menu, P9 early-route compositor and cues")
		quit(0)
	else:
