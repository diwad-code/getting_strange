...
func _test_early_save_migration(state: Node) -> void:
	...
	_expect(state.decisions.get(&"world_recognized", false) == true, "Migracja S01–S05 musi zachować fakty spoza zakresu")
...
func _test_s01_sample_and_promise(state: Node) -> void:
	...
	if station == null:
		return
	_test_guidance(station, &"station_01")
	_expect(not _call_bool(station, &"repeat_measurement", "Station 01 musi wystawiać próbę powtórnego pomiaru"), "Próba S01 bez źródeł musi być bezpiecznie odrzucona")
	_expect(state.decisions.get(&"p7.sample_and_promise.safe_trial_feedback", "") == "mount_or_raw_record_missing", "Błędna próba S01 musi dać informację")
	_expect(_call_bool(station, &"observe_measurement_gap", "Station 01 musi obserwować lukę"), "S01 musi otworzyć rozbieżność")
	_expect(_call_bool(station, &"inspect_sensor_mount", "Station 01 musi sprawdzać mocowanie"), "S01 musi sprawdzić mocowanie")
	_expect(_call_bool(station, &"record_raw_measurement", "Station 01 musi zachować surowy odczyt"), "S01 musi mieć drugi kontekst dowodu")
	_expect(_call_bool(station, &"repeat_measurement", "Station 01 musi wykonać próbę"), "S01 musi wykonać próbę po źródłach")
	_expect(_call_bool(station, &"preserve_raw_sample", "Station 01 musi wystawiać zobowiązanie ochrony próbki"), "S01 musi zachować próbkę po próbie")
	await _close_station(station)
	station = await _open_station(&"station_02")
	if station == null:
		return
	_expect(_call_bool(station, &"observe_service_detour", "Station 02 musi obserwować obejście"), "S01 musi dawać drugi kontekst trasy")
	_expect(_call_bool(station, &"compare_route_duration", "Station 02 musi porównywać czas przejścia"), "S01 musi testować czas względem surowego zapisu")
	_expect(_call_bool(station, &"take_safe_bypass", "Station 02 musi wystawiać bezpieczne obejście"), "S01 musi zakończyć techniczny test bez checklisty")
	await _close_station(station)
	station = await _open_station(&"station_03")
	if station == null:
		return
	_expect(_call_bool(station, &"observe_departure_time", "Station 03 musi obserwować czas odjazdu"), "S01 musi ustanowić niezależny czas dojazdu")
	_expect(_call_bool(station, &"send_measurement_time_notice", "Station 03 musi wystawiać jawną wiadomość do Marty"), "S01 musi odrzucić ukrywanie opóźnienia przez działanie")
...
func _test_s02_return_under_control(state: Node) -> void:
	...
	if station == null:
		return
	_expect(not _call_bool(station, &"repeat_reader_without_route_change", "Station 04 musi wystawiać próbę czytnika"), "S02 bez obserwacji musi dać bezpieczny fakt")
	_expect(state.decisions.get(&"p7.return_under_control.safe_trial_feedback", "") == "reader_or_clock_missing", "Błędna próba S02 musi dać informację")
	_expect(_call_bool(station, &"observe_reader_repeat", "Station 04 musi obserwować powtórkę czytnika"), "S02 musi obserwować powtórkę")
	_expect(_call_bool(station, &"observe_wagon_clock", "Station 04 musi obserwować niezależny zegar"), "S02 musi obserwować ruch wagonu")
	_expect(_call_bool(station, &"repeat_reader_without_route_change", "Station 04 musi wykonać próbę powtórki"), "S02 musi porównać dwa źródła")
	await _close_station(station)
...
func _test_s05_marta_threshold(state: Node) -> void:
	...
	if station == null:
		return
	_expect(_call_bool(station, &"place_bag_at_door", "Station 14 musi odłożyć torbę przy drzwiach"), "S05 musi przygotować spotkanie przy progu")
	_expect(_call_bool(station, &"disclose_arrival_time", "Station 14 musi ujawniać godzinę"), "S05 musi podać pierwszy szczegół")
	_expect(_call_bool(station, &"compare_field_equipment", "Station 14 musi porównywać sprzęt"), "S05 musi podać drugi szczegół")
	_expect(_call_bool(station, &"verify_key_position", "Station 14 musi sprawdzać położenie klucza"), "S05 musi podać trzeci szczegół")
	_expect(_call_bool(station, &"ask_independent_day_description", "Station 14 musi żądać niezależnego opisu dnia"), "S05 musi rozstrzygnąć pamięć bez sugerowania odpowiedzi")
	_expect(_call_bool(station, &"respect_marta_threshold", "Station 14 musi wystawiać szacunek progu Marty"), "S05 musi utrwalić odmienne relacje bez nagrody za dopasowanie")
	_expect(state.decisions.get(&"p7.marta_threshold.trace", "") == "relationship_difference_explicit", "S05 musi zostawić jawną różnicę relacji")
	_expect(state.decisions.get(&"marta_relationship_disclosed", false) == true and state.decisions.get(&"conflicting_documents_found", false) == true, "S05 musi utrwalić kanoniczne fakty po wykonanych testach")
	await _close_station(station)
