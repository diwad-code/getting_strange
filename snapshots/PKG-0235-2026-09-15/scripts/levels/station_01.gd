...
## PKG-0233 (B2, S-08): materialne zrodlo zaswiadczenia m. 12. Dokument
## jedzie w torbie od poczatku (niezaleznie od wyboru probki); 07 wyjmuje
## ten sam dokument, 13 kladzie go na stole. Jeden writer, zero nowych stacji.
const FACT_CERTIFICATE := &"p9.opening.field_certificate_packed"
const CERTIFICATE_VALUE := "sadowa_7_m12"
...
## PKG-0233 (B2, S-08): pakowanie sprzetu zawsze zabiera ten sam dokument.
## Jedyny writer zaswiadczenia; obie drogi otwieraja ten sam rekwizyt w 07.
func _pack_field_certificate() -> void:
	_record(FACT_CERTIFICATE, CERTIFICATE_VALUE)
	if guidance_service:
		guidance_service.trigger_beat(&"s01_field_certificate")
	queue_redraw()
...
func _setup_guidance() -> void:
	...
	# PKG-0233 (B2, S-08): dokument w torbie od poczatku — zrodlo
	# zaswiadczenia m. 12 dla 07 i stolu w 13. Tekst pinowany (0179) powyzej
	# nietkniety; to jest osobny beat pakowania.
	_register_beat(&"s01_field_certificate", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Do torby: zaświadczenie z adresem Sadowa 7 m. 12.", "Into the bag: the certificate with the Sadowa 7, flat 12 address.", &"", "")
...
func _draw() -> void:
	...
	# PKG-0233 (B2, S-08): zaswiadczenie m. 12 w torbie od poczatku — ten sam
	# dokument, ktory 07 porowna z tabliczka, a 13 poloży na stole. Rysunek
	# warunkowy (po spakowaniu sprzetu), zero nowych wezlow.
	if not opening_choice.is_empty():
		draw_rect(Rect2(350.0, 266.0, 50.0, 8.0), VectorStageStyle.LIGHT_PLANE)
		draw_line(Vector2(354.0, 270.0), Vector2(396.0, 270.0), VectorStageStyle.MID_PLANE, 1.0)
