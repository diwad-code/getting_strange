...
func _test_station_01() -> void:
	...
	_expect(station.observe_measurement_gap(), "station_01: gap observation must register")
	_expect(station.inspect_sensor_mount(), "station_01: mount check must register after gap")
	_expect(station.record_raw_measurement(), "station_01: raw record must register after gap")
	_expect(station.repeat_measurement(), "station_01: repeat must run after sources")
	_expect(station.preserve_raw_sample(), "station_01: sample preservation must commit")
	_expect(station.get("is_exit_unlocked") == true, "station_01: exit must unlock after commitment")
...
func _test_station_02() -> void:
	...
	_expect(station.get_node_or_null("ChamberDoor") != null, "station_02: chamber door missing")
	_expect(station.get_node_or_null("AirlockZone") != null, "station_02: airlock zone missing")
	_expect(station.observe_service_detour(), "station_02: detour observation must register")
	_expect(station.compare_route_duration(), "station_02: route time comparison must register")
	_expect(station.take_safe_bypass(), "station_02: safe bypass must commit")
...
func _test_station_03() -> void:
	...
	_expect(station.get_node_or_null("SecurityDoor") != null, "station_03: security door missing")
	_expect(station.get_node_or_null("ReturnZone") != null, "station_03: return zone missing")
	_expect(station.observe_departure_time(), "station_03: departure time must register")
	_expect(station.send_measurement_time_notice(), "station_03: time notice must commit")
...
func _test_station_04() -> void:
	...
	_expect(station.get_node_or_null("TurnstileBarrier") != null, "station_04: turnstile barrier missing")
	_expect(station.observe_reader_repeat(), "station_04: reader repeat must register")
	_expect(station.observe_wagon_clock(), "station_04: wagon clock must register")
	_expect(station.repeat_reader_without_route_change(), "station_04: repeat trial must commit")
...
func _test_station_06() -> void:
	...
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(585.0, 238.0)
		await physics_frame
		await physics_frame
...
func _test_station_07() -> void:
	...
	_expect(station.ask_shopkeeper_recent_visit(), "station_07: shopkeeper question must register")
	_expect(station.inspect_sale_ledger(), "station_07: sale ledger must register")
	_expect(station.commit_ordinary_explanation(), "station_07: ordinary explanation must commit")
	_expect(station.get("is_exit_unlocked") == true, "station_07: exit must unlock after commitment")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(530.0, 238.0)
		await physics_frame
		await physics_frame
...
func _test_station_08() -> void:
	...
	_expect(station.get_node_or_null("Geometry/BuildingEntranceDoor") != null, "station_08: entrance door missing")
	_expect(station.read_certificate(), "station_08: certificate must register")
	_expect(station.read_directory(), "station_08: directory must register")
	_expect(station.test_intercom_recognition(), "station_08: intercom test must commit")
	_expect(station.get("is_exit_unlocked") == true, "station_08: exit must unlock after intercom test")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(600.0, 238.0)
		await physics_frame
		await physics_frame
...
func _test_station_11() -> void:
	...
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(604.0, 238.0)
		await physics_frame
		await physics_frame
...
func _test_station_12() -> void:
	...
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(590.0, 238.0)
		await physics_frame
		await physics_frame
...
func _test_station_14() -> void:
	...
	_expect(station.place_bag_at_door(), "station_14: bag must be placed")
	_expect(station.disclose_arrival_time(), "station_14: arrival time must register")
	_expect(station.compare_field_equipment(), "station_14: field equipment must compare")
	_expect(station.verify_key_position(), "station_14: key position must verify")
	_expect(station.ask_independent_day_description(), "station_14: independent day description must commit")
	_expect(station.get("is_exit_unlocked") == true, "station_14: exit must unlock after threshold")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(590.0, 238.0)
		await physics_frame
...
func _test_station_15() -> void:
	...
	_expect(not station.request_own_work_record(), "station 15: request without two details must stay safe")
	_expect(station.observe_expedition_notes(), "station 15: field notes must register")
	_expect(station.compare_rain_detail(), "station 15: rain detail must compare")
	_expect(station.compare_fence_detail(), "station 15: fence detail must compare")
	_expect(station.observe_secured_phone(), "station 15: secured phone must be observed")
	_expect(station.request_own_work_record(), "station 15: own work record must be requested")
	_expect(station.get("is_exit_unlocked") == true, "station 15: exit must unlock after the request")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
...
func _test_station_16() -> void:
	...
	root.add_child(station)
	await physics_frame
	_expect(not station.compare_field_biometrics_and_audit(), "station 16: trial without three reads must stay safe")
	_expect(station.scan_biometric_profile(), "station 16: biometric profile must match")
	_expect(station.compare_card_number(), "station 16: card number must compare")
	_expect(station.read_activity_history(), "station 16: activity history must be read")
	_expect(station.compare_field_biometrics_and_audit(), "station 16: institution trial must resolve")
...
func _test_station_17() -> void:
	...
	root.add_child(station)
	await physics_frame
	_expect(not station.copy_report_header(), "station 17: copying without scope must stay safe")
	_expect(station.read_incident_report(), "station 17: incident report must be read")
	_expect(station.compare_signature_with_card(), "station 17: signature must compare")
	_expect(station.listen_intercom_warning(), "station 17: intercom warning must register")
	_expect(station.release_service_route(), "station 17: service route must release")
	_expect(station.copy_report_header(), "station 17: report header must be copied")
	_expect(station.get("is_exit_unlocked") == true, "station 17: exit must unlock after the scope commitment")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(590.0, 240.0)
		for f in range(5):
...
func _test_station_18() -> void:
	...
	root.add_child(station)
	await physics_frame
	_expect(not station.compare_independent_registries(), "station 18: trial without registries must stay safe")
	_expect(station.search_municipal_death_registry(), "station 18: municipal registry must be read")
	_expect(station.search_hospital_registry(), "station 18: hospital registry must be read")
	_expect(station.verify_employment_card(), "station 18: employment card must verify")
	_expect(station.read_disaster_case_file(), "station 18: disaster case file must be read")
	_expect(station.compare_independent_registries(), "station 18: public trial must resolve")
	_expect(station.get("is_exit_unlocked") == true, "station 18: exit must unlock after the public trial")
	var player := station.get_node_or_null("Player") as PrototypePlayer
	if player:
		player.global_position = Vector2(590.0, 240.0)
		for f in range(5):
