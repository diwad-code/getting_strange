...
func _audit_tscn_file(file_path: String, fname: String) -> Dictionary:
	...
	for line in lines:
		...
		if trimmed.begins_with("[node "):
			...
			elif trimmed.begins_with("disabled = true"):
	...
	for b in solid_bodies:
		...
		var is_blocking: bool = step_up > 18.0 and not bool(b["one_way"]) and not is_door
	...
	for obs in obstacles:
		var status: String = "OK (step <= 18px)"
		if obs["blocking"]:
			status = ">>> BLOCKING BARRIER (>18px, solid, no ladder) <<<"
		elif obs["one_way"]:
