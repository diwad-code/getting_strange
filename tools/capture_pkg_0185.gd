...
## PKG-0185 cast/portrait/NPC visual audit capture.
## Normal Windows display driver. Does not overwrite reports/pkg_0184/.
...
const REPORT_DIR := "res://reports/pkg_0185"
...
func _run() -> void:
	...
	if _failures.is_empty():
		print("PKG-0185 CAST AUDIT CAPTURE PASS: %d frames on %s." % [_rows.size() - 1, DisplayServer.get_name()])
		quit(0)
	else:
		for failure in _failures:
			push_error("PKG-0185 CAPTURE: " + failure)
		quit(1)
...
func _save_frame(surface: String, mode: String) -> void:
	...
	print("PKG-0185 CAPTURE: %s" % relative_path.trim_prefix("res://"))
