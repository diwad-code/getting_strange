...
## H-005: repeatable technical cost audit for the existing Godot runtime.
##
...
var _output_root: String = OUTPUT_ROOT
var _package_name: String = "PKG-0109"
...
func _read_runtime_options() -> void:
	# The default remains PKG-0109. User arguments only separate later audit
	# evidence into its own directory and never alter the game project.
	for raw_arg in OS.get_cmdline_user_args():
		var arg := String(raw_arg)
		if arg.begins_with("--audit-output-root="):
			var candidate_root := arg.trim_prefix("--audit-output-root=")
			if candidate_root.begins_with("res://"):
				_output_root = candidate_root
		elif arg.begins_with("--audit-package="):
			var candidate_package := arg.trim_prefix("--audit-package=")
			if not candidate_package.is_empty():
				_package_name = candidate_package
...
func _run() -> void:
	...
	print("%s AUDIT PASS: %d scene resources inventoried, %d frame rows, %d animation rows" % [
		_package_name,
...
func _write_json(file_name: String, value: Variant) -> void:
	var output_dir := ProjectSettings.globalize_path(_output_root)
...
func _write_table(file_name: String, headers: Array, rows: Array) -> void:
	...
	var file := FileAccess.open(output_dir.path_join(file_name), FileAccess.WRITE)
	if file == null:
		push_error(_package_name + ": cannot write " + file_name)
		return
