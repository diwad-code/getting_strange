...
## PRZESZKODA — dlaczego to tu jest: Lustro ma dwa stałe mocowania, a wersja korytarza wybiera jedno z nich.
## PRZESZKODA — czego wymaga od Leny: zakotwiczenia właściwego mocowania przed przejściem i zaakceptowania kosztu dla zapisu.
## PRZESZKODA — koszt porażki: korekta odsyła do umywalki, a ślad inskrypcji traci jedną warstwę czytelności.
...
@onready var observed_mirror: AnchorableObject = $Geometry/ObservedMirror
...
var mirror_anchor_cost_paid: bool = false
var mirror_correction_count: int = 0
var mirror_detail_faded: bool = false
...
func _ready() -> void:
	...
	_setup_airlock()
	if observed_mirror:
		observed_mirror.anchor_state_changed.connect(_on_mirror_anchor_state_changed)
...
func _physics_process(delta: float) -> void:
	...
	if player:
		if observed_mirror:
			observed_mirror.update_player_distance(player.global_position)
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed(&"trigger_correction"):
		run_mirror_correction_pass()
		get_viewport().set_input_as_handled()
		return
	if event.is_action_pressed(&"interact") and observed_mirror and observed_mirror.is_player_in_range:
		if observed_mirror.toggle_anchor():
			mirror_anchor_cost_paid = true
			var state := get_node_or_null("/root/GameStateManager")
			if state:
				state.record_decision(&"station_09_anchored_mirror", true)
		get_viewport().set_input_as_handled()
		return
...
func _on_mirror_anchor_state_changed(_is_anchored: bool) -> void:
	queue_redraw()


func run_mirror_correction_pass() -> void:
	if observed_mirror == null:
		return
	observed_mirror.apply_reality_shift(AnchorableObject.RealityState.STATE_B)
	if not observed_mirror.is_anchored:
		_apply_mirror_correction()


func _apply_mirror_correction() -> void:
	mirror_correction_count += 1
	mirror_detail_faded = true
	var state := get_node_or_null("/root/GameStateManager")
	if state:
		state.record_decision(&"station_09_mirror_corrected", mirror_correction_count)
	if player:
...
func _draw_state_layer() -> void:
	...
	var mirror_col := VectorStageStyle.CORRECTION_OXIDE if not observed_mirror or not observed_mirror.is_anchored else VectorStageStyle.ANCHOR_CYAN
	draw_line(Vector2(342.0, 246.0), Vector2(548.0, 232.0), mirror_col, 3.0)
	draw_rect(Rect2(150.0, 210.0, 40.0, 5.0), mirror_col)
	if inscription_revealed:
		draw_rect(Rect2(204.0, 214.0, 28.0, 4.0), VectorStageStyle.HUMAN_AMBER)
	if mirror_detail_faded:
		draw_rect(Rect2(214.0, 130.0, 8.0, 36.0), VectorStageStyle.shade(VectorStageStyle.CORRECTION_OXIDE, 0.38))
