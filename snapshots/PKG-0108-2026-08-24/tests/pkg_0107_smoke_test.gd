...
const FINAL_SCENE_CASES: Array[Dictionary] = [
	{
		...
		"prop_radii": {"ReturnCups": 48.0, "Station42AExit": 60.0},
		"dialogue_count": 10,
	},
	{
		...
		"prop_radii": {"MartaDoorstep": 48.0, "Station42BExit": 60.0},
		"dialogue_count": 9,
	},
	{
		...
		"prop_radii": {"TramDualTracks": 48.0, "Station42CExit": 60.0},
		"dialogue_count": 13,
	},
	{
...
func _run() -> void:
	...
	if state:
		...
		_expect(int(state_constants.get("CAMPAIGN_TRANSITION_LIMIT", -1)) == 25, "campaign transition limit must remain 25")
	...
	if _failures.is_empty():
		print("PKG-0107 SMOKE PASS: finale Vector-Stage, finale branches, conscious silence and chain boundary 25")
		quit(0)
	else:
...
func _check_finale_scenes(state: Node) -> void:
	for case_data in FINAL_SCENE_CASES:
		...
		if state:
			...
			_expect(state.last_checkpoint_position == CHECKPOINT_POSITION, "cue checkpoint position changed: %s" % path)
		...
		_check_geometry(geometry, path)
		_check_airlock(airlock, path)
...
func _exercise_42a(station: Station42A, props: Node2D, player: PrototypePlayer) -> void:
	...
	if station == null or props == null or player == null:
		return
	_expect(not station.is_exit_unlocked, "Station 42A exit must start locked")
	var cups := props.get_node_or_null("ReturnCups") as MemoryResonancePoint
	var exit_prop := props.get_node_or_null("Station42AExit") as MemoryResonancePoint
	_expect(cups != null, "Station 42A cups prop is missing")
	_expect(exit_prop != null, "Station 42A exit prop is missing")
	if cups:
		cups.trigger_interaction()
	_expect(station.is_cups_inspected, "Station 42A cups inspection must remain available")
	_exhaust_dialogue(station)
	_expect(station.is_dialogue_completed, "Station 42A dialogue must complete")
	_expect(station.is_exit_unlocked, "Station 42A exit must unlock after dialogue")
	_expect(exit_prop != null and exit_prop.is_activated, "Station 42A exit prop must activate")
...
func _exercise_42b(station: Station42B, props: Node2D, player: PrototypePlayer) -> void:
	...
	if station == null or props == null or player == null:
		return
	_expect(not station.is_exit_unlocked, "Station 42B exit must start locked")
	var doorstep := props.get_node_or_null("MartaDoorstep") as MemoryResonancePoint
	var exit_prop := props.get_node_or_null("Station42BExit") as MemoryResonancePoint
	_expect(doorstep != null, "Station 42B doorstep prop is missing")
	_expect(exit_prop != null, "Station 42B exit prop is missing")
	if doorstep:
		doorstep.trigger_interaction()
	_expect(station.is_doorstep_inspected, "Station 42B doorstep inspection must remain available")
	_exhaust_dialogue(station)
	_expect(station.is_dialogue_completed, "Station 42B dialogue must complete")
	_expect(station.is_exit_unlocked, "Station 42B exit must unlock after dialogue")
	_expect(exit_prop != null and exit_prop.is_activated, "Station 42B exit prop must activate")
	await _exercise_existing_airlock(station, player)
...
func _exercise_42c(station: Station42C, props: Node2D, player: PrototypePlayer) -> void:
	...
	if station == null or props == null or player == null:
		return
	_expect(not station.is_exit_unlocked, "Station 42C exit must start locked")
	var tracks := props.get_node_or_null("TramDualTracks") as MemoryResonancePoint
	var exit_prop := props.get_node_or_null("Station42CExit") as MemoryResonancePoint
	_expect(tracks != null, "Station 42C dual-track prop is missing")
	_expect(exit_prop != null, "Station 42C exit prop is missing")
	if tracks:
		tracks.trigger_interaction()
	_expect(station.is_tram_inspected, "Station 42C track inspection must remain available")
	_exhaust_dialogue(station)
	_expect(station.is_dialogue_completed, "Station 42C dialogue must complete")
	_expect(station.is_exit_unlocked, "Station 42C exit must unlock after dialogue")
	_expect(exit_prop != null and exit_prop.is_activated, "Station 42C exit prop must activate")
	await _exercise_existing_airlock(station, player)
...
func _exercise_existing_airlock(station: Node, player: PrototypePlayer) -> void:
	_completion_count = 0
	var completion_callable := Callable(self, "_record_completion")
	station.connect(&"level_completed", completion_callable)
	player.global_position = Vector2(610.0, 248.0)
...
func _check_geometry(geometry: Node2D, path: String) -> void:
	if geometry == null:
		return
	...
	for body_name in ["FloorMain", "Ceiling", "WallLeft", "WallRight"]:
		...
		var expected_size := REQUIRED_FLOOR_SIZE if body_name == "FloorMain" else REQUIRED_CEILING_SIZE if body_name == "Ceiling" else REQUIRED_WALL_SIZE
...
func _check_airlock(airlock: Area2D, path: String) -> void:
	if airlock == null:
		return
	_expect(airlock.position == Vector2(610.0, 248.0), "finale AirlockZone position changed: %s" % path)
	...
	if shape_node:
		...
		if rectangle:
			_expect(rectangle.size == REQUIRED_AIRLOCK_SIZE, "finale AirlockZone shape changed: %s" % path)
...
func _check_campaign_boundary(state: Node) -> void:
	...
	_expect(not state.has_reached_station(&"station_26"), "PKG-0107 must not introduce Station 26")
