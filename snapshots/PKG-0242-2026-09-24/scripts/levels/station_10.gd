...
## Station 10 — P9, dowod relacyjny: pokoj z Marta (S-13, PKG-0230).
## Bialy kubek, dwie wersje wspolnej wyprawy i granica Marty: telefon zostaje
## przy niej, Lena nie dotyka jej tak, jak miejscowa
## (home_task → marta_day → marta_boundary).
## Warstwa dawcy P7 (zamek, klucz, prog) jest nieaktywna w trasie, ale
## callable dla testow — patrz werby nizej.

## PRZESZKODA — dlaczego to tu jest: Marta broni telefonu i bliskosci, bo rozpoznala podmiane wczesniej niz padnie slowo syntezy.
## PRZESZKODA — czego wymaga od Leny: podania kubka, wysluchania jej wersji dnia i przyjecia granicy bez nacisku.
## PRZESZKODA — koszt porażki: nacisk zamiast sluchania zostawia luke w GapLedger, nie blokade.

## PKG-0192 (D-205 §4): Station 10 nie ma własnego diegetycznego elementu R4
## (Sideboard/Balcony/Drawer żyją w Station 11-13). Ten plik dostaje wyłącznie
## retirement namespace'u faktów: `inspect_key_wear()`, `test_key_without_claiming_home()`,
## `commit_cautious_entry()` zostają callable (używane bezpośrednio przez
## `tests/pkg_0100_smoke_test.gd` i inne diagnostyczne fale), ale nie piszą już
## nic pod `p7.foreign_daily_life.*` — patrz `p9.threshold_obstacle.*` niżej.

const NarrativeGuidanceService := preload("res://scripts/core/narrative_guidance_service.gd")
const GuidanceBeat := preload("res://scripts/core/guidance_beat.gd")

## PKG-0192 — retires the callable P7 surface (D-205 §4, second dependency
## step after PKG-0191). These facts kept the `p7.foreign_daily_life.*`
## namespace after PKG-0191 wired the real canonical writers; the methods
## below are still callable (tests exercise them directly, no MRP node
## triggers them) and stay callable, but their bookkeeping now lives under
## `p9.threshold_obstacle.*` — no active writer in Station 10-13 uses
## `p7.foreign_daily_life.*` or `p7.marta_threshold.*` any longer.
const FACT_NEIGHBOUR := &"p9.threshold_obstacle.foreign_daily_life.neighbour_account"
const FACT_KEY_WEAR := &"p9.threshold_obstacle.foreign_daily_life.key_wear_observed"
const FACT_KEY_TEST := &"p9.threshold_obstacle.foreign_daily_life.key_trial_result"
const FACT_ENTRY := &"p9.threshold_obstacle.foreign_daily_life.cautious_entry_committed"
...
var is_marta_boundary_accepted := false
...
func _setup_guidance() -> void:
	...
	_register_beat(&"s10_key_contact", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Marta odsunęła biały kubek.", "Marta moved the white cup aside.", &"", "")
	_register_beat(&"s10_intruder_hypothesis", GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT, &"interpretation", &"fallible", "Może źle zapamiętałam wyjazd. Poproszę Martę o jej wersję.", "Perhaps I remember the trip wrong. I will ask Marta for her account.", &"intruder", "hear_marta_day")
	_register_beat(&"s10_key_wear", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Podam kubek, potem zapytam o deszcz.", "I will pass the cup, then ask about the rain.", &"", "")
	# PKG-0233 (B1, S-03): mysl wyjsciowa jako hipoteza do sprawdzenia, nie
	# stwierdzenie. Biometryka w 11 potwierdza lokalny profil; 10 nie ma
	# prawa go przesadzac (wzor: plan PKG-0231 B1).
	# PKG-0236 (B4, plan 2026-09-15): to samo zdanie nazywa tez wyjscie
	# z domu (telefon zostaje, Lena wychodzi), nie nastepna czynnosc przy
	# stole. Hipoteza i frazy pinowane bramka 0233 nietkniete.
	_register_beat(&"s10_exit_ucp_record", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Marta twierdzi, że pracuję w UCP. Zostawiam jej telefon i wychodzę — zapis sprawdzę w UCP, zanim uznam to za moje.", "Marta claims I work at UCP. I leave her phone and step out — I will check the record at UCP before I take it as mine.", &"", "")
	_register_beat(&"s10_system_hint", GuidanceBeat.Tier.L4_RESCUE_HINT, &"system_hint", &"system", "WSKAZÓWKA: Filiżanki, rozmowa przy stole, telefon Marty.", "HINT: Cups, conversation at the table, Marta's phone.", &"", "")
...
func _draw() -> void:
	...
	# PKG-0198 (ZERO wycinek 2): cień kontaktowy stołu i regału w prawo,
	# 0.48, zgodnie z lampą stołową (230,180) i konwencją 01/06/08.
	draw_colored_polygon(PackedVector2Array([
		Vector2(80.0, 300.0), Vector2(360.0, 296.0),
		Vector2(368.0, 304.0), Vector2(88.0, 308.0),
...
func accept_marta_boundary() -> bool:
	...
	is_marta_boundary_accepted = true
	if guidance_service:
		guidance_service.trigger_beat(&"s10_exit_ucp_record")
	_unlock_exit()
	queue_redraw()
	return true
