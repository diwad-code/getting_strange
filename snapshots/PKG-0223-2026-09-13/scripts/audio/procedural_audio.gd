...
static func drain_playback(node: Node) -> void:
	...
	if node is AudioStreamPlayer:
		_stop_player(node as AudioStreamPlayer)
	elif node is AudioStreamPlayer2D:
		_stop_player_2d(node as AudioStreamPlayer2D)
	elif node is AudioStreamPlayer3D:
		_stop_player_3d(node as AudioStreamPlayer3D)
	for child in node.get_children():
		drain_playback(child)


static func _stop_player(player: AudioStreamPlayer) -> void:
	if player == null:
		return
...
static func _stop_player_2d(player: AudioStreamPlayer2D) -> void:
	if player == null:
		return
	if player.finished.is_connected(player.play):
		player.finished.disconnect(player.play)
...
static func _stop_player_3d(player: AudioStreamPlayer3D) -> void:
	if player == null:
		return
	if player.finished.is_connected(player.play):
		player.finished.disconnect(player.play)
...
static func generate_wav(duration: float, generator_func: Callable) -> AudioStreamWAV:
	...
	for i in range(total_samples):
		...
		var raw: float = float(generator_func.call(t, duration))
		var sample: float = clampf(tanh(raw) * 0.94, -1.0, 1.0)
...
static func create_sensory_galvanometer_tick_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (tick * env + body + click) * 0.80
	)
...
static func create_substructure_strain_groan_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (sub + iron + creak) * env * 0.78
	)
...
static func create_wierzbicka_stamp_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (stamp_body + snap + thud + ink_pad) * 0.85
	)
...
static func create_model_table_resonance_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (tone * shimmer + harmonic + glass_click) * env * 0.80
	)
...
static func create_paper_map_rustle_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (noise * 0.45 + fiber_friction * 0.35 + snap) * env * 0.78
	)
...
static func create_ledger_page_turn_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (scrape + paper_body + fold_snap) * env * 0.82
	)
...
static func create_model_room_door_release_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (bolt_slide + latch_snap + motor_hum + air_bleed) * env * 0.85
	)
...
static func create_crayon_drawing_rustle_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (wax_friction + paper_body + crayon_stroke + erase_rub) * env * 0.80
	)
...
static func create_well_water_drip_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (drip_impulse + sub_water + echo_tone + ripple) * 0.82
	)
...
static func create_szymon_dialogue_blip_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (body + h1 + h2 + breath) * env * 0.76
	)
...
static func create_door_creak_shift_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (iron_groan + friction_screech + structural_pop + wall_scrape) * env * 0.85
	)
...
static func create_anesthetic_hum_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (sub + harmonic + whisper) * env * 0.80
	)
...
static func create_sedation_monitor_blip_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (tone + overtone + low_body) * env * 0.75
	)
...
static func create_erased_name_glitch_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (band_noise + glitch_tone + notch_pulse + vocal_struggle) * env * 0.85
	)
...
static func create_station21_airlock_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (bolt + release_tone + air_burst + latch) * env * 0.85
	)
...
static func create_footstep_steel_grating_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, _dur: float) -> float:
		...
		if t < 0.008:
			click = (1.0 - t / 0.008) * sin(t * TAU * 3400.0) * 0.45
		return (ring * 0.65 + click * 0.35) * env * 0.85
	)
...
static func create_surface_land_sound(surface_type: int) -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, _dur: float) -> float:
		...
		return (low_impact * 0.65 + surface_res * 0.35) * env * 0.90
	)
...
static func generate_looping_wav(duration: float, generator_func: Callable) -> AudioStreamWAV:
	...
	# PKG-0220 (A1): baked 80 ms linear crossfade tail<-head for gapless LOOP_FORWARD.
	# F0/inner AM untouched; only the loop seam is blended so the end flows into the start.
	# Integer-period loops (sustain/drag) stay seamless; one-shot envelopes must not use this.
	var total_frames: int = wav.data.size() / 2
	var fade_frames: int = mini(total_frames / 2, int(SAMPLE_RATE * 0.08))
	if fade_frames > 1:
		var data := wav.data
		for i in range(fade_frames):
			var a := float(i) / float(fade_frames)
			var tail_idx := (total_frames - fade_frames + i) * 2
			var head_idx := i * 2
			var tail_v := float(data.decode_s16(tail_idx))
			var head_v := float(data.decode_s16(head_idx))
			var mixed := int(tail_v * (1.0 - a) + head_v * a)
			data.encode_s16(tail_idx, clampi(mixed, -32768, 32767))
		wav.data = data
...
static func create_anchor_grip_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return (contact * 0.45 + body * 0.55) * env * 0.62
	)
...
static func create_yield_collapse_sound() -> AudioStreamWAV:
	...
	return generate_wav(duration, func(t: float, dur: float) -> float:
		...
		return tanh((tone + oxide) * 1.15) * env * 0.72
	)
...
static func create_outdoor_viaduct_wind_sound() -> AudioStreamWAV:
	var duration := 2.20
	return generate_looping_wav(duration, func(t: float, _dur: float) -> float:
		# PKG-0220 (A1): loop-safe envelope (f0/inner AM unchanged, edge fade removed + baked xfade).
		var env := 1.0
