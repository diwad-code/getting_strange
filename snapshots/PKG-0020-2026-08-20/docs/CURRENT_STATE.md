...
- Ostatni zamkniety pakiet: `PKG-0020`, 2026-08-20
...
- Zamrozenia: `snapshots/PKG-NNNN-DATA/` przez `tools/snapshot.ps1` (D-017);
  ostatnie: `snapshots/PKG-0020-2026-08-20`
...
- Projekt Godot 640x360 uruchamia sceny prototypowe `scenes/prototype/movement_lab.tscn`, `scenes/prototype/anchor_lab.tscn` oraz sceny Vertical Slice: pierwszą lokację `scenes/levels/station_01.tscn` i Komorę Pomiarową `scenes/levels/station_02.tscn`.
...
- Rozszerzono `tests/smoke_test.gd` o automatyczną weryfikację:
  ...
  - syntezy proceduralnego audio (w tym rezonansu pamięci, korelacji, drukarki, igły galwanometru, przełączników i szumu próżni);
...
Potwierdzony wynik finalnej weryfikacji PKG-0020:
...
Kontrakt dokumentacji obejmuje `ADR-001` do `ADR-005` oraz aktualne nagłówki. Bramka weryfikuje istnienie wymaganych dokumentów, brak wiszących placeholderów, poprawne ładowanie klas (`ProceduralAudio`, `AnchorableObject`, `MovableAnchorableProp`, `CinematicCamera`, `AnchorLab`, `PrototypePlayer`, `MemoryResonancePoint`, `Station01`, `Station02`, `DiscontinuousShadow`) oraz test automatyczny kamer, stref komorowych, syntezy dźwięków proceduralnych (w tym kroków na linoleum/metalu, lądowań, uszczelnienia śluzy, rezonansu pamięci, korelacji, drukarki taśmowej, igły galwanometru, przełączników i szumu próżni), węzłów audio/cząsteczek, fizyki i logiki kotwiczenia oraz pełnego przejścia procedur w `station_01.tscn` i `station_02.tscn` (kalibracja, skok wskaźnika powyżej progu, anomalia cienia desynchronizująca klatki, wydruk werdyktu WYNIK ZGODNY, abort i odryglowanie śluzy wyjściowej).
...
Cel: Implementacja Przestrzeni 03 z `FULL_STORY.md` (Puste laboratorium IKP):
- Korytarz i powrót do sterowni: nocna obsada zniknęła.
- Drzwi rozpoznają identyfikator Leny, ale wyświetlają inne zdjęcie i dopisek "urlop przerwany".
- W sterowni stoją dwa kubki zamiast jednego.
- Telefon pokazuje 14 nieodebranych wiadomości od Marty Kurek, której Lena nie zna.
- Interakcja ze śladami niezgodności zasilająca późniejsze dialogi.
...
plik i `NEXT_SESSION_PROMPT.md`, a nastepnie przechodzi do realizacji PKG-0021.
