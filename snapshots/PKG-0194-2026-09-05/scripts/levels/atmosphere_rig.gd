...
func _exit_tree() -> void:
	if is_instance_valid(_fluorescent_hum):
		if _fluorescent_hum.finished.is_connected(_fluorescent_hum.play):
			_fluorescent_hum.finished.disconnect(_fluorescent_hum.play)
		_fluorescent_hum.stop()
		_fluorescent_hum.stream = null
	if is_instance_valid(_substructure_player):
		if _substructure_player.finished.is_connected(_substructure_player.play):
			_substructure_player.finished.disconnect(_substructure_player.play)
