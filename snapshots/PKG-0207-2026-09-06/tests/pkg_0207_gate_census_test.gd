...
##
## Fakty pinowane (stan po tym pakiecie, z self):
## (1) spis: 103 wiersze wywolujace Invoke-GodotGate (102 ze skryptem + import
## bez skryptu), 101 unikalnych res://tests/*.gd == 101 plikow tests/*.gd
...
## (3) handoff: SESSION_LOG konczy sie PKG-0207, CURRENT_STATE nazywa PKG-0207,
## DECISION_LOG zawiera D-222, NEXT_SESSION_PROMPT nazywa PKG-0207 i PKG-0208;
...
const EXPECTED_INVOKE_LINES := 103
const EXPECTED_SCRIPT_ARGS := 102
const EXPECTED_TEST_REFS := 101
const EXPECTED_DISK_TESTS := 101
...
func _finish() -> void:
	if _failures.is_empty():
		print("PKG-0207 GATE CENSUS PASS: 103 invokes / 102 scripts / 101 tests, 640x360, 60 Hz, 10 actions, handoff coherent.")
		quit(0)
	else:
