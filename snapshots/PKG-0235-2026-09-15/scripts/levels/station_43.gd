...
## PKG-0233 (D5): maly koszt z 16 w wyplacie epilogu ("marta_memory" /
## "sample_second" / "" = jawna luka). Czytany z decyzji, nie nowy fakt.
var small_cost_state: String = ""
...
const DEFAULT_DIALOGUE_LINES: Array[Dictionary] = [
	...
	{
		"speaker": "POWRÓT",
		"text": "Radio podaje: »Linia 4 zamknięta do odwołania. Prosimy korzystać z wyznaczonego obejścia.« Odcinek przy torowisku odbudowano po katastrofie; data odbioru wisi obok rozkładu."
	},
	{
		"speaker": "UZGODNIENIE",
...
func _ready() -> void:
	...
	_restore_inspections_from_decisions()
...
func _read_campaign_state() -> void:
	...
	# PKG-0233 (D5): maly koszt z 16 dociera do epilogu ta sama droga co
	# prawda i zgoda — odczyt z decyzji, bez nowych faktow.
	var cost = state.decisions.get("p9.mechanics.small_cost.choice", "")
	small_cost_state = String(cost) if cost != null and (String(cost) == "marta_memory" or String(cost) == "sample_second") else ""
...
## tablica → pełny dialog (5 linii) → napisy → ostatnia czynność → blackout.
## Blackout przed wymaganiami odmawia i NIC nie zapisuje (brak ukończenia).
func _epilogue_requirements_met() -> bool:
	return is_notice_inspected and is_credits_inspected \
		and dialogue_index >= dialogue_lines.size() - 1
...
func inspect_blackout() -> bool:
	...
	if not _epilogue_requirements_met():
		last_feedback = &"epilogue_sequence_incomplete"
		if guidance_service:
			guidance_service.report_failed_attempt(&"s43_epilogue_sequence_incomplete")
			guidance_service.trigger_beat(&"s43_epilogue_action")
		queue_redraw()
		return false
...
func _complete_campaign() -> bool:
	if is_level_completed:
		return false
	# PKG-0232 (D-244, D4): ta sama straż co w inspect_blackout — ścieżka
	# progu (ThresholdBinder) nie omija epilogu. Przed wymaganiami odmawia
	# i nie zapisuje ukończenia kampanii.
	if not _epilogue_requirements_met():
		last_feedback = &"epilogue_sequence_incomplete"
		if guidance_service:
			guidance_service.report_failed_attempt(&"s43_epilogue_sequence_incomplete")
...
## PKG-0232 (D-244, pakiet A): materialne akty trwają w decyzjach; świeża
## instancja po ReturnZone nie wymaga ponownej tablicy ani napisów.
## Dialog zaczyna się od nowa (5 linii do ponownego odczytania).
func _restore_inspections_from_decisions() -> void:
	var state := get_node_or_null("/root/GameStateManager")
	if state == null:
		return
	...
	if not (decisions is Dictionary):
		return
	if bool((decisions as Dictionary).get(FACT_P9_ADMIN_NOTICE, false)) \
			or bool((decisions as Dictionary).get(FACT_NOTICE, false)):
		is_notice_inspected = true
		_activate_prop_by_id("prop_admin_notice_board")
		_activate_prop_by_id("admin_notice_board")
	if bool((decisions as Dictionary).get(FACT_P9_CREDITS, false)) \
			or bool((decisions as Dictionary).get(FACT_CREDITS, false)):
		is_credits_inspected = true
		_activate_prop_by_id("prop_credits_roll")
		_activate_prop_by_id("credits_roll")
...
func _on_return_zone_entered(_body: Node2D) -> void:
...
func _draw_state_layer() -> void:
	...
	# PKG-0233 (D5): prawda, zgoda i maly koszt jako ksztalty na slupkach —
	# odpowiednik pierscienia prawdy i znaczkow z 42A/B/C w jezyku epilogu.
	_draw_payoff_marks()
...
## PKG-0233 (D5): wyplata w obrazie epilogu. Prawda na slupku tablicy
## (pelny okrag / polowa / przerwa), zgoda na slupku napisow (podwojny /
## pojedynczy / X / kropki), koszt na maszcie sygnalizatora (trojkat =
## pamiec Marty, kwadrat = sekunda probki, brak = jawna nieobecnosc).
## Ksztalty, nie sam kolor; ten sam jezyk co znaczki 42A/B/C.
func _draw_payoff_marks() -> void:
	var truth_color := VectorStageStyle.LIGHT_PLANE if is_notice_inspected else VectorStageStyle.shade(VectorStageStyle.HUMAN_AMBER, 0.44)
	match marta_truth_state:
		"full":
			draw_arc(Vector2(160.0, 200.0), 5.0, 0.0, TAU, 16, truth_color, 2.0)
		"partial":
			draw_arc(Vector2(160.0, 200.0), 5.0, -PI * 0.5, PI * 0.5, 12, truth_color, 2.0)
		_:
			draw_arc(Vector2(160.0, 200.0), 5.0, 0.4, TAU - 0.4, 16, truth_color, 2.0)
	var consent_color := VectorStageStyle.ANCHOR_CYAN if is_credits_inspected else VectorStageStyle.shade(VectorStageStyle.ANCHOR_CYAN, 0.46)
	match jakub_consent_state:
		"granted":
			draw_line(Vector2(334.0, 196.0), Vector2(346.0, 196.0), consent_color, 2.0)
			draw_line(Vector2(334.0, 204.0), Vector2(346.0, 204.0), consent_color, 2.0)
		"limited":
			draw_line(Vector2(334.0, 200.0), Vector2(346.0, 200.0), consent_color, 2.0)
		"refused":
			draw_line(Vector2(334.0, 194.0), Vector2(346.0, 206.0), consent_color, 2.0)
			draw_line(Vector2(346.0, 194.0), Vector2(334.0, 206.0), consent_color, 2.0)
		_:
			draw_circle(Vector2(337.0, 200.0), 1.5, consent_color)
			draw_circle(Vector2(343.0, 200.0), 1.5, consent_color)
	var cost_color := VectorStageStyle.LIGHT_PLANE if is_exit_unlocked else VectorStageStyle.MID_PLANE
	match small_cost_state:
		"marta_memory":
			draw_colored_polygon(
				PackedVector2Array([Vector2(514.0, 186.0), Vector2(520.0, 176.0), Vector2(526.0, 186.0)]),
				cost_color,
			)
		"sample_second":
			draw_rect(Rect2(514.0, 174.0, 12.0, 12.0), cost_color, false, 2.0)
		_:
			pass
