...
## PKG-0140 (D-146). Wspolna obwiednia kotwiczenia — ta sama, ktora prowadzi
## `AnchorableObject`, wiec skrzynia i platforma pulsuja identycznie.
var _resonance := AnchorResonance.new()
## Dotyk, ton kotwicy i tarcie ida przez wspoldzielone glosy stacji (D-149).
...
var _drag_sound: AudioStreamWAV
...
func _setup_audio() -> void:
	...
	_drag_sound     = ProceduralAudio.get_cached_sound(&"prop_drag_scrape", ProceduralAudio.create_prop_drag_scrape_sound)
...
func _voice(voice_name: StringName, max_distance: float) -> AudioStreamPlayer2D:
	if not is_inside_tree():
		return null
	return StationAudioVoices.acquire(self, voice_name, max_distance, -80.0)


## Istniejacy glos stacji albo `null`. Miks w stanie spoczynku nigdy nie tworzy
## wezla — glos powstaje dopiero przy pierwszym chwycie albo pierwszym realnym
## przesunieciu, wiec stacja nie placi za dzwiek, ktory nigdy nie zagral.
func _existing_voice(voice_name: StringName) -> AudioStreamPlayer2D:
	if not is_inside_tree():
		return null
	var host := StationAudioVoices.station_root(self)
	if host == null:
		return null
	return host.get_node_or_null(NodePath(String(voice_name))) as AudioStreamPlayer2D


func _start_sustain() -> void:
	var voice := _voice(StationAudioVoices.ANCHOR_SUSTAIN, 420.0)
...
## Miks tonu podtrzymanego i tarcia. Oba kanaly prowadzi obwiednia, wiec nigdy
## nie wlaczaja sie ani nie urywaja skokowo. Glosy sa wspoldzielone przez
## stacje, wiec skrzynia dotyka ich tylko wtedy, gdy realnie ma co grac.
func _update_resonance_mix() -> void:
	_mix_sustain()
	_mix_drag()


func _mix_sustain() -> void:
	if _resonance.grip <= 0.001 and not _resonance.is_held():
		var idle := _existing_voice(StationAudioVoices.ANCHOR_SUSTAIN)
		if idle != null and StationAudioVoices.owns(idle, self):
			idle.volume_db = -80.0
			if idle.playing:
				idle.stop()
			StationAudioVoices.release(idle, self)
		return
	var voice := _voice(StationAudioVoices.ANCHOR_SUSTAIN, 420.0)
	if voice == null:
		return
	if not StationAudioVoices.owns(voice, self):
		return
	voice.global_position = global_position
...
func _mix_drag() -> void:
	if _resonance.drag <= 0.02:
		var idle := _existing_voice(StationAudioVoices.ANCHOR_DRAG)
		if idle != null and StationAudioVoices.owns(idle, self):
			if idle.playing:
				idle.stop()
			StationAudioVoices.release(idle, self)
		return
	var voice := _voice(StationAudioVoices.ANCHOR_DRAG, 360.0)
	if voice == null or _drag_sound == null:
		return
	StationAudioVoices.claim(voice, self)
	voice.global_position = global_position
	voice.stream = _drag_sound
	if not voice.playing:
		voice.play()
	voice.volume_db = lerpf(-40.0, -17.0, _resonance.drag)
	voice.pitch_scale = lerpf(0.82, 1.14, _resonance.drag)
...
func _physics_process(delta: float) -> void:
	...
	# Tarcie karmi obwiednie realnym przemieszczeniem poziomym, nie zamiarem —
	# skrzynia wparta w sciane milczy, tak jak milczalaby naprawde (D-146).
	_resonance.feed_drag(absf(velocity.x) / maxf(1.0, push_speed_max))

	if _resonance.is_active() or _flash_intensity > 0.0 or is_player_in_range:
		_pulse_phase += delta * 4.0
...
func apply_reality_shift(_target_state: AnchorableObject.RealityState, _animate: bool = true) -> void:
	...
	# Uleglosc jest zmiana; inicjalizacyjne ustawienie stanu nie brzmi (D-146).
	var is_real_shift := current_reality != _target_state
...
func _draw() -> void:
	...
	if is_anchored or _resonance.grip > 0.01:
		# PKG-0140 (D-146): ta sama obwiednia co w `AnchorableObject`. Ramka
		# oddycha z tonem harmonicznym i skacze przy uderzeniu fali korekty.
		var cyan_glow := Color(
			_resonance.accent_color(COLOR_AMBER, COLOR_ANCHOR_CYAN, COLOR_CORRECTION),
			maxf(0.35, _resonance.field_alpha())
		)
		draw_rect(body_rect.grow(2.0 + _resonance.field_expansion()), cyan_glow, false, 1.5)
