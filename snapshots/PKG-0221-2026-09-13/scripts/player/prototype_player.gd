...
var _body_echo_audio_player: AudioStreamPlayer2D
var _body_echo_delay := 0.0
var _body_echo_stream: AudioStreamWAV = null
...
func _setup_audio() -> void:
	...
	_body_echo_audio_player = AudioStreamPlayer2D.new()
	_body_echo_audio_player.name = "BodyEchoAudioPlayer"
	_body_echo_audio_player.max_distance = 600.0
	_body_echo_audio_player.bus = &"Master"
	add_child(_body_echo_audio_player)
...
## PKG-0221 (D-234): wylacznie LadderZone wola te metode, po wlasnej bramie
## intencji (interact albo stop+gora). Gracz nigdy nie przypina sie sam.
func begin_climb() -> void:
	if current_ladder == null or not is_instance_valid(current_ladder):
		return
	is_climbing = true
...
func lock_for_threshold() -> void:
	_threshold_locked = true
	velocity = Vector2.ZERO


func unlock_from_threshold() -> void:
	_threshold_locked = false


func is_threshold_locked() -> bool:
	return _threshold_locked


func _physics_process(delta: float) -> void:
	if movement_profile == null:
		return
	if _threshold_locked:
		velocity = Vector2.ZERO
		return
	...
	if _body_echo_delay > 0.0:
		_body_echo_delay -= delta
		if _body_echo_delay <= 0.0 and _body_echo_stream != null:
			if _body_echo_audio_player != null and is_instance_valid(_body_echo_audio_player):
				_body_echo_audio_player.stream = _body_echo_stream
				_body_echo_audio_player.volume_db = -18.0
				_body_echo_audio_player.pitch_scale = 0.92
				_body_echo_audio_player.play()
			_body_echo_stream = null
	if _stepping:
		_advance_curb_step(delta)
		return

	# Ladder climbing. Mounting is owned by LadderZone (PKG-0221/D-234: intent
	# is evaluated in the zone — interact, or stopped + up — so a running
	# graze never pins Lena). The player only sustains the climb. No
	# jump-off: exit is down at the floor or leaving the zone (D-123/§7.5).
	...
	if _was_on_floor and not now_on_floor and not _stepping and not is_climbing:
		var move_dir := signf(horizontal_input) if not is_zero_approx(horizontal_input) else (signf(velocity.x) if absf(velocity.x) > 1.0 else 0.0)
		var drop := measure_drop_height(move_dir if not is_zero_approx(move_dir) else signf(_facing))
		if drop > 1.5 and drop <= MAX_CURB_STEP:
			_begin_step(&"step_down", Vector2(move_dir * 4.0, drop))
			_advance_curb_step(delta)
			return
...
func play_footstep() -> void:
	...
	if _step_audio_player and stream:
		...
		if _should_play_body_echo():
			_body_echo_delay = 0.040
			_body_echo_stream = stream


func _should_play_body_echo() -> bool:
	var gsm := get_node_or_null("/root/GameStateManager")
	if gsm != null and gsm.get("reduced_motion") == true:
		return false
	if is_inside_tree() and get_tree().current_scene != null:
		var sname: String = get_tree().current_scene.name.to_lower()
		for n in range(8, 19):
			if ("%02d" % n) in sname or str(n) in sname:
				return true
	if gsm != null and gsm.has_method("get_current_station_number"):
		var sn: int = gsm.call("get_current_station_number")
		return sn >= 8 and sn <= 18
	return false
...
func measure_drop_height(dir: float = 0.0) -> float:
	if get_world_2d() == null:
		return 0.0
	var space := get_world_2d().direct_space_state
	if space == null:
		return 0.0
	var check_dir := signf(dir) if not is_zero_approx(dir) else signf(_facing)
	var from := global_position + Vector2(check_dir * 6.0, 2.0)
