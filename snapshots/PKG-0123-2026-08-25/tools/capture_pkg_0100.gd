...
func _pose_control_state(station: Node2D, station_id: String) -> void:
	...
	match station_id:
		...
		"station_08":
			...
			station.use_keypad()
		"station_09":
			station.floor_plate_read = true
			station.extinguisher_bracket_read = true
			var planter := station.get_node_or_null("Geometry/StairwellPlanter")
			if planter:
				planter.position.x = 312.0
		"station_10":
