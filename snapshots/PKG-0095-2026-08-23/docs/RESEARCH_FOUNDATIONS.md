...
### Aktualizacja PKG-0093: kierunek plastyczny

- *Another World* (autorstwa Érica Chahiego, wydane w 1991; w Stanach
  Zjednoczonych jako *Out of This World*) jest faktycznym odniesieniem do
  oszczędnej, wielokątnej prezentacji i filmowej inscenizacji, nie biblioteką
  assetów do odtworzenia.
- Dostępne źródła konsekwentnie opisują płaskie wielokąty oraz rotoskopię jako
  cechy oryginału. Materiał konkretnego postmortem GDC był niedostępny podczas
  aktualizacji (bramka logowania), więc nie wyprowadzono z niego dalszych tez.
- Decyzja produkcyjna: własny styl `Rówień Vector-Stage` używa dużych płaszczyzn
  koloru, ograniczonej palety i sylwetek z facetów. Nie kopiuje scen, postaci,
  palety, obcej planety, broni ani ikonografii *Another World*.
- Implementacja Godot: procedury `CanvasItem._draw()`, `draw_colored_polygon()`,
  `draw_polyline()` i jawne `queue_redraw()` pozwalają tworzyć taki obraz bez
  zależności zewnętrznej i bez zastępowania fizyki prezentacją.
