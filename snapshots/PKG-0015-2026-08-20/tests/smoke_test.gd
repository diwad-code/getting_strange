...
func _test_anchor_lab() -> void:
	...
	# ── Test 4: MovableAnchorableProp (crate) ──────────────────────────────
	...
	if crate != null:
		# 4a: Crate starts unanchored with no velocity
		...
		# 4g: Single-anchor exclusivity: anchoring crate releases static anchor and vice versa
		anchor_lab.set_active_anchor(bridge1)
		_expect(bridge1.is_anchored, "bridge1 should be anchored")
		anchor_lab.set_active_prop_anchor(crate)
		...
		_expect(not crate.is_anchored, "crate must be released when bridge1 is anchored")

		# 4h: Respawn resets crate
		anchor_lab._respawn()
