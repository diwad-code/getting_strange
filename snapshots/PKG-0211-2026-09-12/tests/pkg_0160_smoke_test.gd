...
func _renderer_body(renderer: String) -> String:
	# PKG-0199 (D-216): pilot renderers 67..196 live in MrpLegacyRenderer as
	# stateless delegates; the facade keeps a one-call wrapper. Resolve the
	# active body through the delegation so this gate pins the drawn pixels
	# wherever they live — equivalent or stronger than the pre-pilot slice.
	...
	var body := resonance_source.substr(start, end - start) if start >= 0 else ""
	if body.contains("MrpLegacyRenderer.draw_"):
		var helper := FileAccess.get_file_as_string("res://scripts/interactables/mrp_legacy_renderer.gd")
		var hname := renderer.trim_prefix("_draw_")
		var hstart := helper.find("static func draw_%s" % hname)
		var hend := helper.find("\nstatic func ", hstart + 1)
		if hend < 0:
			hend = helper.length()
		_expect(hstart >= 0, "%s must delegate to exactly one MrpLegacyRenderer.draw_%s" % [renderer, hname])
		if hstart >= 0:
			return helper.substr(hstart, hend - hstart)
	return body


func _test_marta_visual_identity() -> void:
	var resonance_source := FileAccess.get_file_as_string("res://scripts/interactables/memory_resonance_point.gd")
	# PKG-0200 (D-218): slice2 moved _draw_marta_interaction (23) and
	# _draw_marta_observation_dialogue (45) verbatim to MrpLegacyRenderer, like
	# the PKG-0199 pilot moved _draw_marta_witness_station (188, D-216). Pin the
	# same pixels wherever they live: combined facade+helper source for prose
	# and arc counts, per-renderer delegation for the pink-hair slice.
	# Equivalent or stronger, never a lowered threshold.
	var helper_source := FileAccess.get_file_as_string("res://scripts/interactables/mrp_legacy_renderer.gd")
	var combined_source := resonance_source + helper_source
	_expect(combined_source.contains("Long pink hair"), "Marta's shared encounter silhouettes must declare long pink hair")
	_expect(combined_source.contains("steel septum"), "Marta's shared encounter silhouettes must declare a steel septum")
	for renderer in ["_draw_marta_interaction", "_draw_marta_observation_dialogue", "_draw_marta_witness_station"]:
		var body := _renderer_body(renderer)
		_expect(body.contains("d45b9a"), "%s must render Marta's pink hair" % renderer)
	_expect(combined_source.count("draw_arc(") >= 2, "Marta's legacy close silhouettes must retain a septum arc")
