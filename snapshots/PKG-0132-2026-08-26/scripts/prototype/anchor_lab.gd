...
var is_sector_completed: bool = false
...
var _airlock_lockdown_progress: float = 0.0
var _airlock_tween: Tween
...
var _airlock_seal_player: AudioStreamPlayer
var _correction_sfx: AudioStreamWAV
var _goal_sfx: AudioStreamWAV
var _airlock_seal_sfx: AudioStreamWAV
...
func _setup_audio() -> void:
	...
	_airlock_seal_player = AudioStreamPlayer.new()
	_airlock_seal_player.name = "AirlockSealAudioPlayer"
	_airlock_seal_player.bus = &"Master"
	add_child(_airlock_seal_player)
	_airlock_seal_sfx = ProceduralAudio.create_airlock_seal_sound()
...
func _respawn() -> void:
	_goal_reached = false
	is_sector_completed = false
	_airlock_lockdown_progress = 0.0
	if _airlock_tween and _airlock_tween.is_valid():
		_airlock_tween.kill()
...
func _on_goal_body_entered(body: Node2D) -> void:
	if body == player and not _goal_reached:
		...
		if _goal_player and _goal_sfx:
			...
			_goal_player.play()
		if _airlock_seal_player and _airlock_seal_sfx:
			_airlock_seal_player.stream = _airlock_seal_sfx
			_airlock_seal_player.play()

		if _airlock_tween and _airlock_tween.is_valid():
			_airlock_tween.kill()

		_airlock_lockdown_progress = 0.0
		_airlock_tween = create_tween()
		_airlock_tween.tween_property(self, "_airlock_lockdown_progress", 1.0, 1.4).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		_airlock_tween.tween_callback(func():
			is_sector_completed = true
...
func _draw() -> void:
	...
	# 7. Sector Transition Telemetry & Fade-out Overlay
	_draw_sector_transition_overlay()

	# 8. Correction Pulse Sweep Wave (sweeps active chamber)
...
func _draw_measurement_airlock() -> void:
	...
	var is_locked := _goal_reached or _airlock_lockdown_progress > 0.0
	...
	var beam_alpha := 0.15 + 0.10 * sin(_beacon_phase * 1.5) if not is_locked else 0.55 + 0.25 * _airlock_lockdown_progress
	...
	if not is_locked:
		...
		draw_line(Vector2(scan_x, 124.0), Vector2(scan_x, 198.0), Color(beam_color, 0.6), 1.0)
	else:
		# Locked containment laser grid bars
		for ly in range(132, 196, 12):
			var bar_y := float(ly)
			var bar_width := 32.0 * minf(1.0, _airlock_lockdown_progress * 1.6)
			var start_bx := gx - bar_width * 0.5
			draw_line(Vector2(start_bx, bar_y), Vector2(start_bx + bar_width, bar_y), Color(COLOR_ANCHOR, 0.85), 1.5)
		# Solid center containment axis
		draw_line(Vector2(gx, 124.0), Vector2(gx, 198.0), Color.WHITE, 2.0)

	# 5. Station Center Alignment Reticle
	draw_circle(Vector2(gx, gy), 3.5 if not is_locked else 5.0, beam_color)
	draw_arc(Vector2(gx, gy), 6.0 if not is_locked else 8.0, 0, TAU, 12, Color(beam_color, 0.85), 1.0)
...
func _draw_sector_transition_overlay() -> void:
	if _airlock_lockdown_progress <= 0.0:
		return

	# Draw institutional telemetry banner in Chamber 3 clear wall area (x=1490..1700, y=24..66)
	var banner_pos := Vector2(1490.0, 24.0)
	var banner_size := Vector2(210.0, 42.0)
	var banner_alpha := minf(1.0, _airlock_lockdown_progress * 1.5)

	# Plate back
	draw_rect(Rect2(banner_pos, banner_size), Color(COLOR_GRID, 0.95 * banner_alpha), true)
	draw_rect(Rect2(banner_pos, banner_size), Color(COLOR_ANCHOR, 0.8 * banner_alpha), false, 1.5)

	# Corner bracket ticks
	var tick := 4.0
	draw_line(banner_pos, banner_pos + Vector2(tick, 0.0), Color.WHITE, 1.5)
	draw_line(banner_pos, banner_pos + Vector2(0.0, tick), Color.WHITE, 1.5)
	draw_line(banner_pos + Vector2(banner_size.x, 0.0), banner_pos + Vector2(banner_size.x - tick, 0.0), Color.WHITE, 1.5)
	draw_line(banner_pos + Vector2(banner_size.x, 0.0), banner_pos + Vector2(banner_size.x, tick), Color.WHITE, 1.5)

	# Stencil status diodes (3 Cyan diodes on top-left)
	draw_circle(banner_pos + Vector2(12.0, 13.0), 2.5, Color(COLOR_ANCHOR, banner_alpha))
	draw_circle(banner_pos + Vector2(22.0, 13.0), 2.5, Color(COLOR_ANCHOR, banner_alpha))
	draw_circle(banner_pos + Vector2(32.0, 13.0), 2.5, Color.WHITE if _airlock_lockdown_progress >= 0.8 else Color(COLOR_ANCHOR, banner_alpha * 0.5))

	# Telemetry header line & stencil lines
	draw_line(banner_pos + Vector2(42.0, 13.0), banner_pos + Vector2(198.0, 13.0), Color(COLOR_INFRASTRUCTURE, 0.6 * banner_alpha), 1.0)
	draw_line(banner_pos + Vector2(12.0, 20.0), banner_pos + Vector2(140.0, 20.0), Color(COLOR_ANCHOR, 0.6 * banner_alpha), 1.0)

	# Consensus progress bar
	var bar_track := Rect2(banner_pos + Vector2(12.0, 26.0), Vector2(186.0, 6.0))
	draw_rect(bar_track, Color(COLOR_PLATFORM, 0.8 * banner_alpha), true)
	var fill_w := 186.0 * clampf(_airlock_lockdown_progress, 0.0, 1.0)
	draw_rect(Rect2(bar_track.position, Vector2(fill_w, 6.0)), Color(COLOR_ANCHOR, banner_alpha), true)
	draw_rect(bar_track, Color(COLOR_INFRASTRUCTURE, 0.5 * banner_alpha), false, 1.0)

	# Smooth fade-out overlay over Chamber 3 screen viewport as progress nears completion
	if _airlock_lockdown_progress > 0.5:
		var fade_alpha := clampf((_airlock_lockdown_progress - 0.5) / 0.5, 0.0, 0.95)
		draw_rect(Rect2(Vector2(1280.0, 0.0), VIEW_SIZE), Color(COLOR_BACKGROUND, fade_alpha), true)
