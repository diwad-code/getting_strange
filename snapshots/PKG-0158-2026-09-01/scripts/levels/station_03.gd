...
func _unlock_exit() -> void:
	...
	if transit_door != null:
		ExitClearance.open_body_tweened(self, transit_door)
...
func _draw_transit_stop() -> void:
	...
	# A scheduled vehicle passes before the reply, then holds its doors after it.
	var vehicle_x := 382.0 if is_marta_reply_sent else 540.0 - _tram_phase
	...
	var door_color := VectorStageStyle.ANCHOR_CYAN if is_exit_unlocked else VectorStageStyle.HUMAN_AMBER
	draw_rect(Rect2(576.0, 178.0, 34.0, 82.0), VectorStageStyle.INK)
	draw_line(Vector2(592.0, 184.0), Vector2(592.0, 254.0), door_color, 2.0)
