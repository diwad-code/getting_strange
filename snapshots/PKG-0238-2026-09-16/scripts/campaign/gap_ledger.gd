...
const CATALOG := {
	...
	"s09.floor_record_unread": {
		...
		"station_flag": "is_private_boundary_respected",
		"close_fact": "p9.mystery.home.trace",
		"thought_pl": "Wyszłam z salonu, zanim obejrzałam dwa życia, fotografię i granicę sypialni.",
		"thought_en": "I left the living room before examining the two lives, the photograph and the bedroom boundary.",
	},
	"s10.key_untried": {
		"opened_from": "station_10",
		"origin_station": "station_10",
		"blocks": ["s11.present_identity_card"],
		"station_flag": "is_marta_boundary_accepted",
	...
	"s11.photograph_unread": {
		...
		"station_flag": "is_minimal_report_requested",
	...
	"s15.signal_unconfirmed": {
		...
		"blocks": ["s16.analyzer"],
		"station_flag": "is_signal_confirmed",
...
const FEEDBACK_TO_GAP := {
	...
	# PKG-0230 (P0-1, S-02): zablokowane zatwierdzenie przy odmowie/braku
	# zakresu mowi luka 17 (powrot i renegocjacja), nie komunikatem systemowym.
...
# PKG-0235 (C2): `station_11|passage_required` no longer maps to a photograph.
# Passage on 11 is the retired sideboard (Pakiet A); the default
# `passage_required` entry names the 09 living-room miss.
...
const NON_GAP_FEEDBACKS := [
	...
	# PKG-0233 (D2): zamrozona migawka metody i rozpoczecie wykonania w 42 —
	# terminalne odmowy commita bez pozniejszego bloku (bramka 0215 fail-closed).
	"method_snapshot_locked",
	"finale_execution_started",
	# PKG-0235 (C1/C2): P7 key verbs stay callable but are not the campaign
	# miss for s10. The table gap opens on depart without Marta's boundary.
	"key_wear_required",
	"key_trial_required",
]
