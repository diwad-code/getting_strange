...
func _test_station_18() -> void:
	...
	root.add_child(station)
	await process_frame
	_expect(not station.commit_force_home(), "Zatwierdzenie bez zestawienia musi pozostać bezpieczne")
	if state:
		_expect(
			state.decisions.get(&"p9.method_commitment.safe_trial_feedback", "") == "forecast_and_consent_inventory_required",
			"Błędna bezpieczna próba musi zostawić fakt"
		)
