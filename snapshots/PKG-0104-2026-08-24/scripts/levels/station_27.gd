...
## dług wdzięczności pozostaje relacyjnym wyborem Jakuba.
## PRZESZKODA — czego wymaga od Leny: wysłuchania jego warunku bez wymuszania
## odpowiedzi ruchem lub timingiem.
## PRZESZKODA — koszt porażki: nie ma fizycznej korekty; nieudowodniona prawda
## pozostaje zamknięta w dialogu, a scena nie udaje testu zręczności.
...
func _draw_state_layer() -> void:
	...
	var signal_color := VectorStageStyle.ANCHOR_CYAN if is_console_inspected else VectorStageStyle.MID_PLANE
	draw_line(Vector2(92.0, 72.0), Vector2(548.0, 72.0), signal_color, 2.0)
	draw_line(Vector2(148.0, 72.0), Vector2(148.0, 214.0), VectorStageStyle.LIGHT_PLANE, 1.0)
	draw_line(Vector2(492.0, 72.0), Vector2(492.0, 214.0), VectorStageStyle.LIGHT_PLANE, 1.0)
	if is_jakub_interacted:
		VectorStageStyle.draw_facet_polygon(self, PackedVector2Array([
			Vector2(232.0, 190.0), Vector2(278.0, 170.0), Vector2(314.0, 190.0), Vector2(278.0, 210.0)
		]), VectorStageStyle.HUMAN_AMBER, 1.0)
	if is_exit_unlocked:
		var exit_color := VectorStageStyle.CORRECTION_OXIDE if _exit_open_progress < 0.5 else VectorStageStyle.ANCHOR_CYAN
		draw_line(Vector2(560.0, 204.0), Vector2(560.0, 276.0), exit_color, 3.0)
