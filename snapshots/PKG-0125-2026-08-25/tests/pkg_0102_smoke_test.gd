...
func _check_station_22_gate(station: Node2D, state: Node) -> void:
	...
	station_22.apply_license_setback()
	_expect(station_22.license_setback_count == 1, "license setback must count the attempt")
	_expect(station_22.reader_locked_in_dock, "license setback must lock reader in dock")
	_expect(not station_22.is_level_completed, "setback must not end Station 22")
	if state:
		_expect(state.decisions.has(&"station_22_dock_locked"), "dock locked must be recorded in decisions")

	station_22.start_offer_dialogue()
	for i in range(station_22.offer_dialogue_lines.size()):
		station_22.advance_offer_dialogue()
	_expect(station_22.is_offer_completed, "offer dialogue must complete")
	if state:
		_expect(state.decisions.has(&"ucp_offer_rejected"), "offer rejection must be recorded")

	station_22.examine_oscilloscope()
	_expect(station_22.is_oscilloscope_checked, "oscilloscope must be examined")
	_expect(station_22.are_modes_named, "modes must be named")
	if state:
		_expect(state.decisions.has(&"anchor_yield_named"), "anchor yield naming must be recorded")
