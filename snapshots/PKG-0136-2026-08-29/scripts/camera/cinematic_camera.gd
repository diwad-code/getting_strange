...
func _physics_process(delta: float) -> void:
	...
	var desired_pos := Vector2(
		...
		_resolve_vertical_focus(delta) + _resolve_dialogue_framing()
	)
...
func _resolve_dialogue_framing() -> float:
	...
	if not _dialogue_surface.has_method("is_presenting"):
		return 0.0
	return dialogue_framing_offset if _dialogue_surface.call("is_presenting") else 0.0
