...
func _run() -> void:
	...
	for path in STATION_PATHS:
		...
		_expect(
			station.get_node_or_null("WorldPixelCompositor") is CanvasLayer,
			"P9 world pixel compositor is missing: %s" % path
		)
	...
	if _failures.is_empty():
		print("PKG-0091 SMOKE PASS: P9 opening scenes, player, atmosphere, CRT dialogue and game state")
		quit(0)
	else:
