...
## Anchor/Yield. Station 21 is the first address allowed to speak the
## recognition, so it is linted only for the method cost.
...
func _test_s06_work_history_and_record(state: Node) -> void:
	...
	_expect(not _call_bool(station, &"request_own_work_record", "Station 15 musi wystawiać prośbę o zapis pracy"), "Próba S06 bez dwóch konkretów musi być bezpiecznie odrzucona")
	_expect(state.decisions.get(&"p7.work_history_and_record.safe_trial_feedback", "") == "expedition_details_incomplete", "Błędna próba S06 musi dać informację")
	_expect(not state.decisions.has(&"marta_memories_conflict"), "Błędna próba nie może nadać kanonicznego faktu")
	_expect(_call_bool(station, &"observe_expedition_notes", "Station 15 musi obserwować notatki terenowe"), "S06 musi otworzyć rozbieżność")
	_expect(_call_bool(station, &"compare_rain_detail", "Station 15 musi porównywać konkret deszczu"), "S06 musi mieć pierwszy konkret")
	_expect(_call_bool(station, &"compare_fence_detail", "Station 15 musi porównywać konkret ogrodzenia"), "S06 musi mieć drugi konkret")
	_expect(_call_bool(station, &"observe_secured_phone", "Station 15 musi obserwować zabezpieczony telefon"), "S06 musi pokazać działanie Marty jako fakt świata")
	_expect(_call_bool(station, &"request_own_work_record", "Station 15 musi wykonać prośbę"), "S06 musi wykonać krok po dwóch konkretach")
	_expect(state.decisions.get(&"marta_memories_conflict", false) == true, "S06 musi utrwalić konflikt wspomnień po wykonanym porównaniu")
	...
	_expect(not _call_bool(station, &"compare_field_biometrics_and_audit", "Station 16 musi wystawiać próbę instytucjonalną"), "Próba S06 bez trzech odczytów musi być odrzucona bezpiecznie")
	_expect(state.decisions.get(&"p7.work_history_and_record.safe_trial_feedback", "") == "institution_sources_incomplete", "Błędna próba S06 na 16 musi dać informację")
	_expect(not state.decisions.has(&"local_lena_ucp_profile_found"), "Błędna próba nie może nadać profilu UCP-4")
	_expect(_call_bool(station, &"scan_biometric_profile", "Station 16 musi skanować profil"), "S06 musi ustanowić źródło biometryczne")
	_expect(_call_bool(station, &"compare_card_number", "Station 16 musi porównywać numer karty"), "S06 musi ustanowić źródło dokumentu")
	_expect(_call_bool(station, &"read_activity_history", "Station 16 musi czytać historię aktywności"), "S06 musi ustanowić źródło chronologii")
	_expect(_call_bool(station, &"compare_field_biometrics_and_audit", "Station 16 musi wykonać próbę"), "S06 musi rozstrzygnąć konkurencyjne hipotezy")
	_expect(state.decisions.get(&"p7.work_history_and_record.institution_trial_result", "") == "persistent_chronology_confirmed", "Wynik próby instytucjonalnej musi być jawny")
	_expect(state.decisions.get(&"local_lena_ucp_profile_found", false) == true, "S06 musi utrwalić profil UCP-4 po wykonanej próbie")
	_expect(_call_bool(station, &"read_roster_signature", "Station 16 musi udostępniać zapis zatwierdzenia"), "S06 musi mieć czytelne źródło podpisu")
	await _close_station(station)
