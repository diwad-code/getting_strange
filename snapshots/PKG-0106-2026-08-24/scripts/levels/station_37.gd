...
## PRZESZKODA — dlaczego to tu jest: Węzeł transmisyjny wzmacnia sygnały
## sektorów, ponieważ Podstruktura przenosi nimi wyparte świadectwa.
## PRZESZKODA — czego wymaga od Leny: Odczytania oscyloskopu, krosownicy,
## anteny i pulpitu oraz uznania, że świadectwo ma odbiorców poza komorą.
## PRZESZKODA — koszt porażki: Nie ma korekty fizycznej; scena buduje
...
func _draw_state_layer() -> void:
	...
	var osc_color := VectorStageStyle.ANCHOR_CYAN if is_oscilloscope_inspected else VectorStageStyle.LIGHT_PLANE
	draw_rect(Rect2(112.0, 132.0, 56.0, 38.0), VectorStageStyle.INK)
	draw_rect(Rect2(112.0, 132.0, 56.0, 38.0), osc_color, false, 1.2)
	draw_line(Vector2(118.0, 152.0), Vector2(128.0, 144.0), osc_color, 1.0)
	draw_line(Vector2(128.0, 144.0), Vector2(140.0, 160.0), osc_color, 1.0)
	draw_line(Vector2(140.0, 160.0), Vector2(153.0, 142.0), osc_color, 1.0)
	var patch_color := VectorStageStyle.HUMAN_AMBER if is_patchbay_inspected else VectorStageStyle.MID_PLANE
	for socket in range(4):
		draw_circle(Vector2(224.0 + float(socket) * 14.0, 150.0), 3.0, patch_color)
	var antenna_color := VectorStageStyle.ANCHOR_CYAN if is_antenna_inspected else VectorStageStyle.LIGHT_PLANE
	draw_line(Vector2(344.0, 92.0), Vector2(344.0, 220.0), antenna_color, 2.0)
	draw_circle(Vector2(344.0, 88.0), 7.0, antenna_color)
	var pulpit_color := VectorStageStyle.CORRECTION_OXIDE if is_pulpit_inspected else VectorStageStyle.MID_PLANE
	draw_colored_polygon(PackedVector2Array([Vector2(414.0, 208.0), Vector2(474.0, 202.0), Vector2(482.0, 232.0), Vector2(420.0, 238.0)]), pulpit_color)
	if is_exit_unlocked:
