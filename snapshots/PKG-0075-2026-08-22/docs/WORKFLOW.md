...
- **Mega-Pakiet Wysokoprzepustowy (High-Throughput Mega-Package, D-085)**: Tryb domyślny dla autonomicznego AI (Lead Programmer & Art Director). Obejmuje 2x do 5x większy zakres funkcjonalny realizowany całościowo w jednym cyklu:
  - Zamiast pojedynczych mikro-kroków, pakiet łączy równolegle 3–5 kompletnych podsystemów (np. wielościeżkowa aparatura syntezy audio + silnik wektorowy CRT + zestaw nowych komór fizyki Canvas 2D + interaktywny wiersz poleceń CLI + pakiety odtajnionych akt).
  - Praca postępuje płynnie krok po kroku bez przerw i bez oczekiwania na potwierdzenia.
  - Zapewnia 100% spójności architektonicznej i natychmiastowe ukończenie szerokich obszarów portalu i silnika.
...
- Dbaj o 100% symetrii językowej (PL/EN) i zgodność kontraktu walidacji `verify_web.ps1`.
