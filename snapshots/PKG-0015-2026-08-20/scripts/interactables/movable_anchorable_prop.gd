...
func _draw() -> void:
	...
	# 2. Structural cross bracing (diagonal stripes, lab equipment look)
	var stripe_color := Color(COLOR_CRATE_STRIPE, 0.7)
	draw_line(Vector2(-half.x, -half.y), Vector2(half.x, half.y), stripe_color, 1.0)
	draw_line(Vector2(half.x, -half.y), Vector2(-half.x, half.y), stripe_color, 1.0)

	# 3. Edge outline — cyan when anchored, standard otherwise
	var edge_color := COLOR_ANCHOR_CYAN if is_anchored else COLOR_CRATE_EDGE
	draw_rect(body_rect, edge_color, false, 1.5)

	# 4. Top walkable surface highlight
	draw_line(Vector2(-half.x, -half.y), Vector2(half.x, -half.y), edge_color, 2.0)

	# 5. Anchor hold indicator (pulsing cyan frame + corner pins)
	if is_anchored:
	...
	# 6. In-range bracket prompt (when not anchored and player is close)
	...
	# 7. Flash on anchor toggle / correction wave
	if _flash_intensity > 0.0:
