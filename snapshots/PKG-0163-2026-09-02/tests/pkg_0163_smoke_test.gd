...
func _test_safe_rejection(state: Node) -> void:
	...
	_expect(not bool(station.get("is_exit_unlocked")), "Wyjście musi pozostać zamknięte bez próby")
...
func _test_control_echo(state: Node) -> void:
	...
	_expect(not bool(station.get("is_exit_unlocked")), "Kontrolne echo nie może otworzyć wyjścia")
...
func _test_corrective_response_and_note(state: Node) -> void:
	...
	_expect(not bool(station.get("is_exit_unlocked")), "Potwierdzenie sygnału samo nie otwiera wyjścia")
