...
func _setup_guidance() -> void:
	...
	if guidance:
		...
		beat.beat_id = &"s04_train_journey"
		beat.scene_id = &"station_04"
		beat.tier = GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT
		beat.thought_kind = &"observation"
		beat.text_pl = "Wagon kołysze na zwrotnicach. Jeszcze dwa przystanki."
		beat.text_en = "Carriage sways across switches. Two stops left."
		beat.cooldown_s = 8.0
...
func _draw_state_layer() -> void:
	...
	var gate_col := VectorStageStyle.CORRECTION_OXIDE
	if is_turnstile_unlocked:
		gate_col = VectorStageStyle.ANCHOR_CYAN
	draw_line(Vector2(500.0, 186.0), Vector2(500.0, 250.0), gate_col, 3.0)
	draw_rect(Rect2(118.0, 224.0, 86.0, 5.0), VectorStageStyle.INK)
	if dialogue_index >= 5:
		draw_rect(Rect2(118.0, 224.0, 72.0, 5.0), VectorStageStyle.CORRECTION_OXIDE)
	if guard_blocks_camera:
		draw_rect(Rect2(374.0, 92.0, 12.0, 18.0), VectorStageStyle.CORRECTION_OXIDE)
...
func _draw_in_world_dialogue(canvas: CanvasItem = null) -> void:
	...
	var is_lena: bool = DIALOGUE_LINES[dialogue_index].get("is_lena", false)
	...
	var box_rect := Rect2(bubble_center.x - 40.0, bubble_center.y - 12.0, 80.0, 24.0)
