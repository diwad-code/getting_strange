...
func _test_scene_contract(state: Node) -> void:
	...
	_expect(not station.is_exit_unlocked, "Wejście bez force_home nie może otworzyć drogi do 43")
...
func _test_donor_entry_requirement(state: Node) -> void:
	...
	_expect(not station.is_exit_unlocked, "Brak force_home nie może odblokować wyjścia")
	await _close_station(station)
...
func _test_wrong_method_rejection(state: Node) -> void:
	...
	_expect(not station.is_exit_unlocked, "Obca metoda nie może otworzyć 42A do 43")
	await _close_station(station)
