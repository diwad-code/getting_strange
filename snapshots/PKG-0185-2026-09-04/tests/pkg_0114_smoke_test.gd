...
func _check_route_contract() -> void:
	...
	_expect(int(constants.get("CAMPAIGN_TRANSITION_LIMIT", -1)) == 18, "campaign route must include Station 01..18")
	...
	_expect(_state.get_all_campaign_scene_ids().size() == 22, "route must map 22 scene resources to 20 spaces and three finale variants")
	_expect(_state.get_selectable_stations(true).size() == 20, "normal selector surface must retain 20 campaign entries")
	...
	_expect(_state.get_next_campaign_station(&"station_18").is_empty(), "Station 18 must wait for a selected operation")
...
func _run_full_route_with_real_signals(operation: String) -> void:
	...
	_expect(chamber != null and _station_id_from_node(chamber) == &"station_18", "full route must arrive at Station 18")
	if chamber:
		...
		_expect(finale != null, "Station 18 must enter the selected 42A variant")
		_expect(_transition_log.size() == branch_transitions_before + 1, "Station 18 must transition once")
...
func _run_branch_from_choice_chamber(operation: String) -> void:
	...
	_expect(_state.has_reached_station(&"station_18"), "branch setup must unlock Station 18 through the campaign API")
	_state.transition_to_station(&"station_18")
	var chamber := await _wait_for_station(&"station_18")
...
## PKG-0176: `Nowa gra` prowadzi teraz przez warstwe A zimnego otwarcia
## (`scenes/shell/cold_open.tscn`), ktora trwa 14,5 s i sama tnie do Station 01.
## Czekanie musi wiec byc liczone w klatkach fizyki, nie w klatkach `process` —
## w trybie headless tych drugich mija znacznie wiecej na sekunde symulacji.
func _wait_for_station(station_id: StringName, max_frames: int = 2400) -> Node:
