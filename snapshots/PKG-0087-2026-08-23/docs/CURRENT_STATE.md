...
- **Wersjonowanie: brak (D-016).** Pliki na dysku sa jedynym stanem projektu.
  Nie ma repozytorium, galezi, commita ani historii. Nie uruchamiamy `git`. Ostatni zamkniety pakiet: `PKG-0087`, 2026-08-23
...
- Zamrozenia: `snapshots/PKG-NNNN-DATA/` przez `tools/snapshot.ps1` (D-017);
  ostatnie: `snapshots/PKG-0087-2026-08-23`
...
Faza **`P3 / Vertical Slice` oraz `Wydanie Produkcyjne Portalu Web Showcase i Pełna Automatyzacja CI/CD`** została **W PEŁNI UKOŃCZONA I ROZSZERZONA NA 100%** (PKG-0087, postęp: Wszystkie 43 Przestrzenie fabularne w Godot 4.7 + Oficjalny Portal Prezentacyjny i Dystrybucyjny w `web/` wzbogacony o `Onsager Reciprocal Relations & Prigogine Non-Equilibrium Entropy Production Engine` z macierzą współczynników kinetycznych $L_{ij}$, relacjami wzajemności $L_{ij} = L_{ji}$, kryterium dodatniej określoności Sylvestera $\det(L) > 0$, lokalną gęstością produkcji entropii $\sigma = \sum J_i X_i \ge 0$, twierdzeniem Prigogine'a o minimum dyssypacji w stanie stacjonarnym $\frac{d\sigma}{dt} \le 0$, współczynnikami termoelektrycznymi Seebecka $S$ i Peltiera $\Pi = T \cdot S$, fluktuacjami termodynamicznymi Einsteina-Onsagera $\langle \delta\sigma^2 \rangle \propto \frac{k_B}{V} \sigma^2$, 4 nowymi syntezami proceduralnymi audio w Godot i Web Audio API, 95 Odtajnionymi Aktami Archiwalnymi `doc1`..`doc95` ze 100% symetrii dwujęzycznej PL/EN oraz Rozszerzeniami CLI o komendy `onsager-matrix`, `entropy-prod`, `kinetic-flux`, `export-onsager` i pełną telemetrię PKG-0087).
...
Potwierdzony wynik finalnej weryfikacji PKG-0064:
...
`PKG-0087: Termodynamika Nierównowagowa Onsagera, Fluktuacje Entropijne i Macierz Współczynników Kinetycznych L_ij w Osnowie Szwu 40 mm`

Cel: Rozbudowa symulatora i portalu dystrybucyjnego o relacje wzajemności Onsagera $L_{ij} = L_{ji}$ dla sprzężonego transportu ciepła, masy i namagnesowania wzdłuż szwu 40 mm, obliczanie lokalnego tempa produkcji entropii $\sigma = \sum J_i X_i \ge 0$, twierdzenie Prigogine'a o minimum produkcji entropii, 4-ćwiartkową wizualizację strumieni termodynamicznych $J_q, J_m, J_s$, kolejne odtajnione akta archiwalne (`doc91`..`doc95`), kolejne syntezy audio oraz rozszerzenia CLI (`onsager-matrix`, `entropy-prod`, `kinetic-flux`, `export-onsager`).

Pakiet jest realizowany w 100% autonomicznie przez AI w trybie Wysokoprzepustowych Mega-Pakietów (2x–5x Batch Size, D-085).

## Punkt przekazania

Nowa sesja zaczyna od swiezej weryfikacji (`tools/verify.ps1`), czyta `AGENTS.md`, `INDEX.md`, ten
plik i `NEXT_SESSION_PROMPT.md`, a nastepnie przechodzi do realizacji PKG-0087.
