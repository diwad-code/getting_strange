...
const COLOR_AMBER := Color("d39a62") # Muted amber (Lena / life / memory)
const COLOR_CYAN := Color("75c7c3") # Cool cyan (Anchor / active signal)
const COLOR_CORRECTION := Color("c65d58") # Oxide cinnabar (Anomaly / sealed)
const COLOR_DARK_STEEL := Color("263943")
const COLOR_FLOOR := Color("151e24")
const COLOR_FLOOR_EDGE := Color("3a4f59")
const COLOR_ASPHALT := Color("131a20")
const COLOR_PUDDLE := Color("1c2832")
const COLOR_RAIL := Color("708590")
...
func _draw() -> void:
	# 1. Distant night sky & city backdrop
	_draw_sky_and_backdrop()
	
	# 2. Chamber 0 Architecture (Institute exterior, Anachronistic Billboard building, Missing Floor Block, Restless Alley)
	_draw_chamber_0_architecture()
	
	# 3. Chamber 1 Architecture (Commercial facade, Tram corridor, Transit shelter)
	_draw_chamber_1_architecture()
	
	# 4. Continuous street roadway, curb, wet asphalt puddles & tram tracks
	_draw_roadway_and_tracks()
	
	# 5. Streetlamps & downward rain illumination cones
	_draw_streetlamps_and_lighting()
	
	# 6. Overhead catenary power lines & suspension cables
	_draw_catenary_wires()
