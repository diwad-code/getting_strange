...
@onready var concrete_stair_flight: StaticBody2D = $Geometry/ConcreteStairFlight
...
var stair_detail_faded: bool = false
...
func _setup_guidance() -> void:
	...
	if guidance:
		...
		beat.beat_id = &"s07_shopkeeper_confusion"
		beat.scene_id = &"station_07"
		beat.tier = GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT
		beat.thought_kind = &"interpretation"
		beat.text_pl = "Pomylił mnie ze swoją stałą klientką. Dziwny zbieg okoliczności."
		beat.text_en = "Mistook me for his regular customer. A strange coincidence."
		beat.cooldown_s = 8.0
...
func _apply_stairwell_correction() -> void:
	if stair_flight_crossed:
		return
	stair_correction_count += 1
	stair_detail_faded = true
...
func _draw_state_layer() -> void:
	...
	var stair_col := VectorStageStyle.MID_PLANE
	if stair_flight_crossed:
		stair_col = VectorStageStyle.ANCHOR_CYAN
	draw_colored_polygon(PackedVector2Array([Vector2(360.0, 310.0), Vector2(480.0, 250.0), Vector2(480.0, 310.0)]), stair_col)
	draw_line(Vector2(360.0, 310.0), Vector2(480.0, 250.0), VectorStageStyle.LIGHT_PLANE, 2.0)
	draw_line(Vector2(474.0, 250.0), Vector2(492.0, 250.0), VectorStageStyle.CORRECTION_OXIDE if not is_door_open else stair_col, 2.0)
	if stair_detail_faded:
		draw_rect(Rect2(508.0, 146.0, 10.0, 42.0), VectorStageStyle.shade(VectorStageStyle.CORRECTION_OXIDE, 0.38))
