...
`move_left`, `move_right`, `move_up`, `move_down`, `jump`, `sprint`,
`interact`, `trigger_correction`, `restart`, `pause`.
`sprint` jest **modyfikatorem chodu** (przytrzymaj Shift / LB), nie nowym
czasownikiem lokomocji (D-127). Domyślny ruch to chód. Bieg tylko przy
wciśniętym `sprint`. **Nie wolno dodawać nowych czasowników ruchu.**
...
## 9. Próg jest czynnością (D-188 / D-189 / D-190, 2026-09-02)

Rozdział dopisany po sesji diagnostycznej właściciela. Rozstrzyga trzy rzeczy,
których kanon przeszkód dotąd nie rozstrzygał: **jak wygląda wejście, jak
wygląda wejście na stopień i jak wygląda wspinaczka**.

Kontrakt szczegółowy: `docs/rebuild/THRESHOLD_AND_ENTRY_CONTRACT.md`.

### 9.1 Zmiana miejsca wymaga czasownika

> Zmiana adresu jest czynnością, nie przekroczeniem niewidzialnej linii.

Zakazane od PHASE-08:

- przełączanie sceny na `body_entered` niewidzialnego `Area2D`;
- wyjście, którego otwór nie jest narysowany w kadrze;
- wejście bez animacji — Lena ma **wejść**, nie zniknąć za krawędzią ekranu.

Nakazane:

- czasownik `interact` przy widocznym otworze (żadnego nowego przycisku —
  rozdział 7.1 pozostaje zamknięty);
- jedna z trzech rodzin wejść: `DOOR` (drzwi zawiasowe), `VEHICLE` (próg
  pojazdu), `HATCH` (właz / otwór techniczny);
- kolejność: zatrzymanie → ustawienie przed otworem → wykonanie → zmiana adresu;
- otwór narysowany z zapasem ≥24 px od krawędzi kadru, żeby animacja się
  zmieściła w obrazie.

To **nie** zmienia reguły kierunku („w prawo albo w górę”) ani dwukierunkowości
z rozdziału 7.6. Zmienia to, że kierunek kończy się konkretnym wejściem, a nie
krawędzią ekranu.

### 9.2 Stopień pokonuje się ruchem, nie teleportem

Rozdział 7.4 ustala **limit** 18 px wejścia bez drabiny. Nie ustalał **sposobu**.

Sposób obowiązujący od PHASE-08:

1. zmierz rzeczywistą wysokość przeszkody przed podniesieniem ciała;
2. podnieś dokładnie o zmierzoną wartość, nie o wartość maksymalną;
3. ruch trwa **0,18–0,24 s** i idzie po łuku, nie w jednej klatce fizyki;
4. w trakcie kroku reakcja lądowania (kurz, dźwięk uderzenia, squash) jest
   wyłączona;
5. animacja to `step_up` / `step_down`, nigdy `jump_fall`.

Powód: podnoszenie o stałe 18 px przy podstopnicy 12 px zostawia 6 px
swobodnego spadku, co przekracza próg reakcji lądowania i produkuje przysiad
na każdym stopniu. Pełny łańcuch przyczynowy:
`docs/rebuild/PRESENTATION_REPAIR_PLAN.md`, DEF-5.

Limit 18 px (D-123) pozostaje nietknięty.

### 9.3 Pion jest umocowany — teraz mierzalnie

Rozdział R7 mówi: „Nic nie wisi w powietrzu”. Od PHASE-08 ma to próg:

| Wielkość | Dopuszczalna różnica |
|---|---|
| rysunek drabiny vs `LadderZone` — pion | ≤ 2 px |
| rysunek drabiny vs `LadderZone` — oś x | ≤ 1 px |
| dół drabiny vs poziom podłogi | ≤ 2 px |
| góra drabiny vs poziom podestu | drabina wystaje 8–14 px ponad podest |

Dodatkowo:

- **drabinę rysuje wyłącznie `LadderZone`.** Żadna stacja nie rysuje drabiny
  drugi raz — Station 02 ma dziś dwie naraz, przesunięte o 62 px;
- **wspinaczka pokazuje plecy postaci**, nie profil boczny; drabina prostopadła
  do kamery wymaga ujęcia tylnego;
- przypięcie do drabiny wymaga intencji (zatrzymanie + ruch w górę albo
  `interact`), nie przypadkowego otarcia się o strefę w biegu.
