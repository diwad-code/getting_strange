...
func compare_address_document() -> bool:
	...
	_present([
		{"speaker": "LENA", "text": "Zaświadczenie: Sadowa 7, mieszkanie 12. Budynek się zgadza. Sprawdzę domofon."},
	])
	queue_redraw()
	return true
...
func inspect_intercom_directory() -> bool:
	...
	_present([
		{"speaker": "LENA", "text": "Lista lokatorów: 12 to Kowalczyk. Pod 14 widnieje: L. Wolska / M. Kurek."},
	])
	queue_redraw()
	return true
...
func _unlock_exit() -> void:
	...
	if entrance_door != null:
		ExitClearance.disable_collision(entrance_door)
...
func _setup_guidance() -> void:
	...
	_register_beat(&"s07_reaction", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Sadowa 7. Moje zaświadczenie podaje mieszkanie 12.", "Sadowa 7. My certificate states flat 12.", &"", "")
...
func _draw() -> void:
	...
	# PKG-0198 (ZERO wycinek 2): cień kontaktowy stopni wejściowych w prawo,
	# 0.48, zgodnie z kinkietami fasady i konwencją 01/06/08. Stopień 14 px
	# ≤ 18 px (kanon 7.4): drabina nie jest wymagana; drabiny w kampanii
	# rysuje wyłącznie LadderZone (02/15/16) — ta stacja nie ma drabiny.
	draw_colored_polygon(PackedVector2Array([
		Vector2(508.0, 304.0), Vector2(604.0, 302.0),
		Vector2(612.0, 310.0), Vector2(516.0, 312.0),
