...
## PKG-0222 (M7): zapis kosztu wymuszonego kroku — swiat pamieta probe
## odpowiedzi bez protokolu (osobny fakt; nie potwierdza sygnalu).
const FACT_COST := &"p9.mechanics.mutual_signal.yield_cost_observed"
...
## PKG-0222 (M7): trwaly stan kosztu po wymuszonym kroku (osobny od feedbacku).
var has_cost_mark := false
...
func _setup_guidance() -> void:
	...
	_register_beat(&"s15_living_response", GuidanceBeat.Tier.L2_CONTEXTUAL_THOUGHT, &"interpretation", &"fallible", "Jeżeli poprawi tylko mój błąd, nie będzie zwykłym odtworzeniem. Sprawdzę, czy odpowiedź się zmienia.", "If it corrects only my error, it is not a simple replay. I will check whether the response changes.", &"living_response", &"corrective_third_impulse")
	_register_beat(&"s15_protocol_plan", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Dwa impulsy kontrolne, potem celowy błąd. Sprawdzę, czy wróci korekta, czy ten sam wzór.", "Two control impulses, then a deliberate error. I will check for a correction rather than a replay.", &"", &"corrective_third_impulse")
	# PKG-0230 (P1-1): zdanie wyjscia — 15 → 16: zywego sygnalu nie czyta sie
	# sprzetem, ktory sam jest czescia petli. Potrzebny przyrzad poza obwodem.
	_register_beat(&"s15_exit_to_analyzer", GuidanceBeat.Tier.L3_DIRECTIONAL_THOUGHT, &"intention", &"procedural", "Mam zmienioną odpowiedź i warunek przerwania. Niosę je do analizatora poza obwodem.", "I have the changed reply and the abort condition. I carry them to the analyzer off the loop.", &"", &"")
	...
	_register_beat(&"s15_log_reconstructed", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Przy późniejszym poleceniu utrzymania wyniku jest podpis Wierzbickiej.", "Wierzbicka signed the later order to maintain the result.", &"", &"")
	_register_beat(&"s15_response_confirmed", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Poprawiła tylko mój błąd. Odpowiedź powstała w tej próbie, nie w zapętlonym nagraniu.", "It corrected only my error. The reply was made in this trial, not in a looped recording.", &"", &"")
	_register_beat(&"s15_abort_note", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Warunek przerwania zapisała przed próbą. Nie zastępuje wcześniejszego pytania obcej osoby o zgodę.", "She wrote an abort condition before the trial. It does not replace asking the other person beforehand.", &"", &"")
	# PKG-0223 (N8/K4): margines miejscowej Leny — dopisek ołówkiem na wydruku,
	# proceduralny ślad winy bez monologu-ducha (obserwacja faktu, L1).
	_register_beat(&"s15_local_margin", GuidanceBeat.Tier.L1_REACTION, &"observation", &"factual", "Na marginesie wydruku dopisek ołówkiem: przepraszam M. — 3 s.", "Pencilled aside on the printout margin: sorry M. — 3 s.", &"", &"")
...
func _apply_response() -> void:
	...
	if _pending_corrective:
		...
		if not is_signal_confirmed:
			...
			# PKG-0222 (M5): potwierdzenie dorecza notatke — sciezka krytyczna
			# to 3 czasowniki (log / kontrola / korekta); jawny odczyt zostaje
			# idempotentny (0194/0163: przed potwierdzeniem nadal falsz).
			read_abort_note()
	else:
...
## PKG-0222 (M7): wymuszony krok = stan kosztu + linia L2 + zapis kosztu.
## Nie potwierdza sygnalu, nie zamyka drogi (kontrakt §5: brak softlocka).
func _mark_forced_cost() -> void:
	if has_cost_mark:
		return
	has_cost_mark = true
	_record(FACT_COST, "corrective_forced_without_controls")
	if guidance_service != null:
		guidance_service.trigger_beat(&"s15_living_response", true)
	queue_redraw()
...
func read_abort_note() -> bool:
	...
	if guidance_service != null:
		...
		guidance_service.trigger_beat(&"s15_exit_to_analyzer")
...
func _draw_station_props() -> void:
	...
	# PKG-0223 (N4/N8): pokaz w obrazie zamiast akapitu. Dwa tiki na dzienniku:
	# kontakt przy pierwszym odczycie i powtorka z pelna probka (po odczycie;
	# domyslny kadr: sama ramka). Obok, na marginesie wydruku, olowkowy slad
	# miejscowej (K4: "przepraszam M. — 3 s") — stara warstwa, zawsze widoczna.
	draw_line(Vector2(84.0, 248.0), Vector2(104.0, 248.0), Color(0.55, 0.56, 0.58, 0.55), 1.0)
	draw_line(Vector2(88.0, 252.0), Vector2(104.0, 252.0), Color(0.55, 0.56, 0.58, 0.55), 1.0)
	draw_line(Vector2(84.0, 256.0), Vector2(100.0, 256.0), Color(0.55, 0.56, 0.58, 0.55), 1.0)
	if is_log_reconstructed:
		draw_line(Vector2(118.0, 244.0), Vector2(118.0, 250.0), VectorStageStyle.ANCHOR_CYAN, 2.0)
		draw_line(Vector2(126.0, 244.0), Vector2(126.0, 250.0), VectorStageStyle.ANCHOR_CYAN, 2.0)
	# PKG-0222 (M7): slad kosztu — przygaszona komora i ostry akcent szwu.
	# Wylacznie po wymuszonym kroku; domyslny kadr nietkniety (HOLD 0198).
	if has_cost_mark:
		draw_rect(Rect2(0.0, 84.0, 640.0, 190.0), Color(0.0, 0.0, 0.0, 0.18))
		draw_line(Vector2(352.0, 40.0), Vector2(352.0, 296.0), Color(VectorStageStyle.SEAM_RED, 0.85), 3.0)
