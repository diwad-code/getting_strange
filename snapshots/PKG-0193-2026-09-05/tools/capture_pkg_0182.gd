...
## PKG-0182 normal-driver evidence capture. This script is intentionally not a
## gameplay route proof; player-verb route evidence is produced by pkg_0177.
...
func _run() -> void:
	...
	if _failures.is_empty():
		print("PKG-0182 CAPTURE PASS: %d normal-driver frames on %s." % [_rows.size() - 1, DisplayServer.get_name()])
		quit(0)
	else:
		for failure in _failures:
			push_error("PKG-0182 CAPTURE: " + failure)
		quit(1)
